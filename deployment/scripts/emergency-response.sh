...
enable_emergency_mode() {
    ...
    if command -v pm2 &> /dev/null; then
        pm2 restart pharmacare-backend || log ERROR "Failed to restart service"
    elif command -v systemctl &> /dev/null; then
        systemctl restart pharmacare-backend || log ERROR "Failed to restart service"
    fi
...
collect_incident_data() {
    ...
    if [[ -f "/var/log/pharmacare-backend.log" ]]; then
        tail -n 1000 /var/log/pharmacare-backend.log > "$incident_dir/backend.log"
    fi
    ...
    # Collect system status
    {
        ...
        if command -v pm2 &> /dev/null; then
            ...
            systemctl status pharmacare-backend
        fi
        echo ""
