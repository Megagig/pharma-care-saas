...
const API_BASE_URL =
   process.env.PLAYWRIGHT_API_BASE_URL || 'http://localhost:5000';
...
test.describe('Clinical Interventions E2E Tests', () => {
   test.beforeEach(async ({ page }) => {
      ...
      await page.route(
         `${API_BASE_URL}/api/clinical-interventions**`,
         async (route) => {
      ...
      await page.route(
         `${API_BASE_URL}/api/clinical-interventions/analytics/summary**`,
         async (route) => {
            await route.fulfill({
               status: 200,
               contentType: 'application/json',
   ...
   test.describe('Dashboard Navigation and Display', () => {
      test('should display clinical interventions dashboard', async ({
         page,
      }) => {
         ...
         await expect(
            page.locator('[data-testid="total-interventions"]')
         ).toContainText('25');
         await expect(
            page.locator('[data-testid="active-interventions"]')
         ).toContainText('8');
         await expect(
            page.locator('[data-testid="success-rate"]')
         ).toContainText('85.5%');

         // Check interventions table
         await expect(
            page.locator('[data-testid="interventions-table"]')
         ).toBeVisible();
         await expect(
            page.locator('[data-testid="intervention-row"]')
         ).toHaveCount(1);
      ...
      test('should display charts and analytics', async ({ page }) => {
         ...
         await expect(
            page.locator('[data-testid="priority-distribution-chart"]')
         ).toBeVisible();
         await expect(
            page.locator('[data-testid="monthly-trends-chart"]')
         ).toBeVisible();
      });
   });
   ...
   test.describe('Intervention Creation Workflow', () => {
      test('should create new intervention successfully', async ({ page }) => {
         ...
         await expect(
            page.locator('[data-testid="intervention-form"]')
         ).toBeVisible();

         // Fill out the form
         await page.selectOption('[data-testid="patient-select"]', {
            label: 'John Doe (MRN123456)',
         });
         ...
         await page.fill(
            ...
            'Patient experiencing significant side effects from current medication regimen that requires immediate attention and intervention'
         );
         ...
         ).toContainText('Intervention created successfully');
      ...
      test('should validate required fields', async ({ page }) => {
         ...
         await expect(
            page.locator('[data-testid="patient-error"]')
         ).toContainText('Patient is required');
         await expect(
            page.locator('[data-testid="category-error"]')
         ).toContainText('Category is required');
         await expect(
            page.locator('[data-testid="priority-error"]')
         ).toContainText('Priority is required');
         await expect(
            page.locator('[data-testid="description-error"]')
         ).toContainText('Issue description is required');
      });
      ...
      test('should validate issue description length', async ({ page }) => {
         ...
         await expect(
            page.locator('[data-testid="description-error"]')
         ).toContainText('Issue description must be at least 10 characters');
      });
      ...
      test('should add and remove strategies', async ({ page }) => {
         ...
         await expect(
            page.locator('[data-testid="strategy-form"]')
         ).toBeVisible();
         ...
         await page.fill(
            ...
            'Patient experiencing dose-related side effects'
         );
         await page.fill(
            '[data-testid="strategy-outcome"]',
         ...
         await expect(
            page.locator('[data-testid="strategy-list"]')
         ).toContainText('Dose Adjustment');
         await expect(
            page.locator('[data-testid="strategy-list"]')
         ).toContainText('Reduce dose by 50%');

         // Remove strategy
         await page.click('[data-testid="remove-strategy-0"]');
         await expect(
            page.locator('[data-testid="strategy-list"]')
         ).not.toContainText('Dose Adjustment');
      });
   });
   ...
   test.describe('Filtering and Search', () => {
      test('should filter interventions by category', async ({ page }) => {
         ...
         ).toHaveValue('drug_therapy_problem');
      });
      ...
      test('should combine multiple filters', async ({ page }) => {
         ...
         await page.waitForResponse((response) => {
            ...
            return (
               url.includes('/api/clinical-interventions') &&
   ...
   test.describe('Pagination', () => {
      test('should handle pagination controls', async ({ page }) => {
         ...
         await expect(
            page.locator('[data-testid="pagination-info"]')
   ...
   test.describe('Accessibility', () => {
      test('should be keyboard navigable', async ({ page }) => {
         ...
         await expect(
            page.locator('[data-testid="create-intervention-button"]')
         ).toBeFocused();
      ...
      test('should have proper ARIA labels', async ({ page }) => {
         ...
         await expect(
            page.locator('[aria-label*="Search interventions"]')
         ).toBeVisible();
         await expect(
            page.locator('[aria-label*="Filter by category"]')
         ).toBeVisible();
         await expect(
            page.locator('[aria-label*="Filter by priority"]')
         ).toBeVisible();
      });
   ...
   test.describe('Error Handling', () => {
      test('should handle network errors gracefully', async ({ page }) => {
         ...
         ).toContainText('Error loading interventions');
