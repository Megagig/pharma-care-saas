...
describe('Clinical Notes System Integration Tests', () => {
   ...
   describe('Feature Flag Integration', () => {
      ...
      it('conditionally shows file upload based on feature flag', async () => {
         ...
         await waitFor(() => {
            expect(
               screen.getByText('Create Clinical Note')
            ).toBeInTheDocument();
         });
   ...
   describe('Patient Management Integration', () => {
      it('displays clinical notes in patient profile', async () => {
         ...
         await waitFor(() => {
            ...
            expect(
               screen.getByText('Confidential Assessment')
            ).toBeInTheDocument();
         });
      });
   ...
   describe('End-to-End Workflow Integration', () => {
      ...
      it('handles bulk operations workflow', async () => {
         ...
         await waitFor(() => {
            expect(
               mockClinicalNoteService.bulkUpdateNotes
            ).toHaveBeenCalledWith(['note1', 'note2'], { priority: 'high' });
         });
      });
   });
   ...
   describe('Error Handling Integration', () => {
      ...
      it('handles subscription limit errors', async () => {
         ...
         await waitFor(() => {
            expect(
               screen.getByText('Usage Limit Exceeded')
            ).toBeInTheDocument();
