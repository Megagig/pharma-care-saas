...
class FrontendClinicalInterventionTestRunner {
      ...
      const testSuites = [
         {
            ...
            command:
               'vitest run src/components/__tests__/*ClinicalIntervention*.test.tsx src/components/__tests__/Intervention*.test.tsx',
         },
         {
            name: 'Unit Tests - Services',
            command:
               'vitest run src/services/__tests__/clinicalInterventionService*.test.ts',
         },
         {
            name: 'Unit Tests - Stores',
            command:
               'vitest run src/stores/__tests__/clinicalInterventionStore*.test.ts',
         },
         {
            name: 'Unit Tests - Hooks',
            command:
               'vitest run src/hooks/__tests__/*clinicalIntervention*.test.ts',
         },
         {
            name: 'Unit Tests - Utils',
            command:
               'vitest run src/utils/__tests__/clinicalInterventionValidation*.test.ts',
         },
         {
            name: 'Integration Tests',
            command:
               'vitest run src/__tests__/integration/clinicalInterventionWorkflow*.test.tsx',
         },
         {
            name: 'Accessibility Tests',
            command:
               'vitest run src/__tests__/accessibility/clinicalInterventionAccessibility*.test.tsx',
         },
      ];
   ...
   private async runTestSuite(
      ...
      try {
         ...
         const failedTests = this.parseVitestOutput(
   ...
   private async runE2ETests(): Promise<void> {
      ...
      try {
         ...
         const playwrightCommand =
            'npx playwright test src/__tests__/e2e/clinicalInterventionE2E*.test.ts --reporter=line';
         ...
         console.log(
            '   Make sure the application is running and Playwright is installed'
         );
   ...
   private generateCoverageSummary(): void {
      ...
      try {
         ...
         const summaryLines = coverageLines.filter(
            (line) =>
               line.includes('%') &&
               (line.includes('Lines') ||
   ...
   private generateQualityMetrics(): void {
      ...
      console.log(
         `Average Test Duration: ${(avgTestDuration / 1000).toFixed(3)}s`
      );
      ...
      if (avgTestDuration > 10000) {
         ...
         console.log(
            '🚀 Excellent: Tests are running very fast (<500ms average)'
         );
      } else {
      ...
      this.results.forEach((suite) => {
         const percentage = ((suite.tests.length / totalTests) * 100).toFixed(
            1
         );
         console.log(
            `   ${suite.name}: ${suite.tests.length} tests (${percentage}%)`
         );
      });

      // Component test coverage
      const componentSuite = this.results.find((s) =>
         s.name.includes('Components')
      );
      const serviceSuite = this.results.find((s) =>
         s.name.includes('Services')
      );
   ...
   private generateAccessibilitySummary(): void {
      ...
      const accessibilitySuite = this.results.find((s) =>
         s.name.includes('Accessibility')
      );
      ...
      if (accessibilitySuite) {
         ...
         if (
            accessibilitySuite.passedCount === accessibilitySuite.tests.length
         ) {
            console.log(
               '✅ All accessibility tests passed - WCAG compliance verified'
            );
         } else {
            console.log(
               '⚠️  Some accessibility tests failed - please review WCAG compliance'
            );
         }
      } else {
   ...
   async runWithWatch(): Promise<void> {
      ...
      try {
         const command =
            'vitest src/components/__tests__/*ClinicalIntervention* src/services/__tests__/clinicalIntervention* src/stores/__tests__/clinicalIntervention*';
         execSync(command, {
            stdio: 'inherit',
            cwd: process.cwd(),
         });
   ...
   async runE2EOnly(): Promise<void> {
      ...
      try {
         const command =
            'npx playwright test src/__tests__/e2e/clinicalInterventionE2E*.test.ts --headed';
         execSync(command, {
            stdio: 'inherit',
            cwd: process.cwd(),
         });
