...
export interface DeepLinkParams {
    ...
    tab?: 'messages' | 'notifications' | 'audit' | 'settings';
}
...
export class CommunicationDeepLinks {
     ...
     */
    toNewConversation(patientId?: string, type?: 'direct' | 'group' | 'patient_query') {
        const params = new URLSearchParams();
     ...
     */
    toPatientQueries(status?: 'open' | 'pending' | 'resolved', priority?: 'low' | 'normal' | 'high' | 'urgent') {
        const params = new URLSearchParams();
     ...
     */
    static generateShareableLink(conversationId: string, messageId?: string): string {
     ...
     */
    static async validateAccess(params: DeepLinkParams, userPermissions: string[]): Promise<boolean> {
        // Basic validation - in real implementation, this would check against backend
        if (params.conversationId && !userPermissions.includes('communication_access')) {
            return false;
        }
        ...
        if (params.tab === 'settings' && !userPermissions.includes('communication_settings')) {
            return false;
        }
    ...
    static handleExternalLink(url: string, navigate: NavigateFunction): boolean {
        try {
            ...
            if (urlObj.pathname === '/pharmacy/communication') {
                const params = CommunicationDeepLinks.parseUrlParams(urlObj.searchParams);
