...
import type { DrugTherapyProblem, TherapyPlan, MTRIntervention, MTRFollowUp } from '../types/mtr';
...
class OfflineStorage {
        ...
        return new Promise((resolve, reject) => {
            ...
            request.onupgradeneeded = (event) => {
                ...
                if (!db.objectStoreNames.contains(STORES.PATIENTS)) {
                    const patientsStore = db.createObjectStore(STORES.PATIENTS, { keyPath: '_id' });
                    patientsStore.createIndex('mrn', 'mrn', { unique: false });
                    patientsStore.createIndex('name', ['firstName', 'lastName'], { unique: false });
                }
                ...
                if (!db.objectStoreNames.contains(STORES.MEDICATIONS)) {
                    ...
                    medicationsStore.createIndex('patientId', 'patientId', { unique: false });
                }

                if (!db.objectStoreNames.contains(STORES.PROBLEMS)) {
                    const problemsStore = db.createObjectStore(STORES.PROBLEMS, { keyPath: '_id' });
                    problemsStore.createIndex('patientId', 'patientId', { unique: false });
                    problemsStore.createIndex('reviewId', 'reviewId', { unique: false });
                }

                if (!db.objectStoreNames.contains(STORES.PLANS)) {
                    const plansStore = db.createObjectStore(STORES.PLANS, { keyPath: 'id' });
                    plansStore.createIndex('patientId', 'patientId', { unique: false });
                }

                if (!db.objectStoreNames.contains(STORES.INTERVENTIONS)) {
                    const interventionsStore = db.createObjectStore(STORES.INTERVENTIONS, { keyPath: '_id' });
                    interventionsStore.createIndex('reviewId', 'reviewId', { unique: false });
                }

                if (!db.objectStoreNames.contains(STORES.FOLLOWUPS)) {
                    const followupsStore = db.createObjectStore(STORES.FOLLOWUPS, { keyPath: '_id' });
    ...
    async getMedicationsByPatient(patientId: string): Promise<MTRMedication[]> {
        ...
        return new Promise((resolve, reject) => {
            const transaction = this.db!.transaction([STORES.MEDICATIONS], 'readonly');
    ...
    async getInterventionsByReview(reviewId: string): Promise<MTRIntervention[]> {
        ...
        return new Promise((resolve, reject) => {
            const transaction = this.db!.transaction([STORES.INTERVENTIONS], 'readonly');
    ...
    async autoSaveDraft(type: MTRDraft['type'], data: unknown, patientId?: string, reviewId?: string): Promise<void> {
