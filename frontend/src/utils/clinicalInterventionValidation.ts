            ...
            formData.strategies.forEach((strategy: any, index: number) => {
               if (strategy.type === 'custom') {
                  if (
                     ...
                     strategy.description.length < 20
                  ) {
                     errors.push({
                        field: `strategies.${index}.description`,
                        message:
                           'Custom strategies require detailed description (minimum 20 characters)',
                  ...
                  if (!strategy.rationale || strategy.rationale.length < 20) {
                     errors.push({
                        ...
                        message:
                           'Custom strategies require detailed rationale (minimum 20 characters)',
         ...
         if (formData.strategies && Array.isArray(formData.strategies)) {
            if (formData.strategies.length === 0) {
               warnings.push({
                  ...
                  code: 'NO_STRATEGIES',
                  suggestion:
                     'Add strategies to improve intervention effectiveness',
               });
            }
            ...
            if (formData.strategies.length > 5) {
               warnings.push({
                  ...
                  suggestion:
                     'Consider focusing on the most important strategies',
               });
            }
         }
...
export const useFieldValidation = (
   ...
   React.useEffect(() => {
      const result = interventionValidator.validateField(
         fieldName,
         value,
         formData
      );
