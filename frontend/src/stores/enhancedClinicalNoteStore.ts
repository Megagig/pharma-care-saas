...
interface EnhancedClinicalNoteStore {
   ...
   setLoading: (
      key: keyof EnhancedClinicalNoteStore['loading'],
      loading: boolean
   ) => void;
   setError: (
      key: keyof EnhancedClinicalNoteStore['errors'],
      error: string | null
   ) => void;
   clearErrors: () => void;
         ...
         immer((set, get) => ({
            ...
            fetchNotes: async (filters) => {
               ...
               try {
                  ...
                  const response =
                     await clinicalNoteService.getNotes(currentFilters);
            ...
            fetchNotesByPatient: async (patientId, filters = {}) => {
               ...
               try {
                  const response = await clinicalNoteService.getPatientNotes(
                     patientId,
                     filters
                  );
                  ...
                  set((state) => {
                     state.errors.fetchNotes =
                        error.message || 'Failed to fetch patient notes';
                  });
               } finally {
                  set((state) => {
                     state.loading.fetchNotes = false;
            ...
            searchNotes: async (query, filters = {}) => {
               ...
               try {
                  const response = await clinicalNoteService.searchNotes(
                     query,
                     filters
                  );
            ...
            createNote: async (noteData) => {
               ...
               try {
                  const response =
                     await clinicalNoteService.createNote(noteData);

                  set((state) => {
                     // Replace optimistic note with real note
                     const tempNoteIndex = state.notes.findIndex(
                        (n) => n._id === tempId
                     );
                     if (tempNoteIndex !== -1) {
            ...
            updateNote: async (id, noteData) => {
               ...
               // Optimistic update
               get().optimisticUpdateNote(
                  id,
                  noteData as Partial<ClinicalNote>
               );
               ...
               try {
                  const response = await clinicalNoteService.updateNote(
                     id,
                     noteData
                  );
            ...
            bulkUpdateNotes: async (noteIds, updates) => {
               ...
               try {
                  await clinicalNoteService.bulkUpdateNotes({
                     noteIds,
                     updates,
                  });
                  ...
                  set((state) => {
                     state.errors.bulkOperations =
                        error.message || 'Failed to bulk update notes';
                  });
                  return false;
               } finally {
                  set((state) => {
               ...
               try {
                  ...
                  set((state) => {
                     state.notes = state.notes.filter(
                        (note) => !noteIds.includes(note._id)
                     );
                     state.selectedNotes = [];
                     state.pagination.total = Math.max(
                        0,
                        state.pagination.total - noteIds.length
                     );
                  });
                  ...
                  set((state) => {
                     state.errors.bulkOperations =
                        error.message || 'Failed to bulk delete notes';
                  });
                  return false;
               } finally {
                  set((state) => {
            ...
            deleteAttachment: async (noteId, attachmentId) => {
               try {
                  const response = await clinicalNoteService.deleteAttachment(
                     noteId,
                     attachmentId
                  );
            ...
            updateFileProgress: (fileIndex, progress) => {
               set((state) => {
                  if (state.fileUpload.files[fileIndex]) {
                     ...
                     state.fileUpload.files[fileIndex].status =
            ...
            getPatientNoteSummary: (patientId) => {
               const patientNotes = get().notes.filter(
                  (note) => note.patient._id === patientId
               );

               return {
                  consultation: patientNotes.filter(
                     (n) => n.type === 'consultation'
                  ).length,
                  medicationReview: patientNotes.filter(
                     (n) => n.type === 'medication_review'
                  ).length,
                  followUp: patientNotes.filter((n) => n.type === 'follow_up')
                     .length,
                  adverseEvent: patientNotes.filter(
                     (n) => n.type === 'adverse_event'
                  ).length,
                  other: patientNotes.filter((n) => n.type === 'other').length,
                  total: patientNotes.length,
                  confidential: patientNotes.filter((n) => n.isConfidential)
                     .length,
                  withFollowUp: patientNotes.filter((n) => n.followUpRequired)
                     .length,
                  withAttachments: patientNotes.filter(
                     (n) => n.attachments && n.attachments.length > 0
                  ).length,
               };
            },
         })),
