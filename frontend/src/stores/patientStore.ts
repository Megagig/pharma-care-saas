      ...
      (set, get) => ({
         ...
         fetchPatients: async (filters) => {
            ...
            try {
               ...
               const response =
                  await patientService.getPatients(currentFilters);
               ...
               if (response.success && response.data) {
                  ...
                  set({
                     ...
                     pagination: {
                        ...
                        limit:
                           response.meta?.limit || currentFilters.limit || 10,
                        total: response.meta?.total || apiPatients.length,
                        pages:
                           response.meta?.totalPages ||
                           Math.ceil(
                              apiPatients.length / (currentFilters.limit || 10)
                           ),
                     },
                  });
               } else {
         ...
         createPatient: async (patientData) => {
            ...
            try {
               ...
               const response =
                  await patientService.createPatient(apiPatientData);
               ...
               if (successful.length === ids.length) {
                  ...
                  set((state) => ({
                     ...
                     selectedPatient:
                        ...
                        ids.includes(state.selectedPatient._id)
                           ? null
                           : state.selectedPatient,
                  }));
                  return true;
