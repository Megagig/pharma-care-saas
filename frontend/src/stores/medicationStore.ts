...
interface MedicationStore {
   ...
   // Bulk operations
   updateMultipleMedicationStatus: (
      ids: string[],
      status: string
   ) => Promise<boolean>;
   ...
   persist(
      (set, get) => ({
         ...
         fetchMedications: async (filters) => {
            ...
            if (!isProduction) {
               console.warn(
                  'Skipping medication API call - no token found in development mode'
               );
               return;
            }
            ...
            try {
               ...
               const response =
                  await medicationService.getMedications(currentFilters);
         ...
         fetchMedicationsByPatient: async (_patientId) => {
            ...
            try {
               const response =
                  await medicationService.getMedicationsByPatient(_patientId);
         ...
         updateMedicationStatus: async (id, status) => {
            ...
            try {
               const response = await medicationService.updateMedicationStatus(
                  id,
                  status
               );

               if (response.success) {
                  updateMedicationInState(id, {
         ...
         updateMultipleMedicationStatus: async (ids, status) => {
            ...
            try {
               ...
               if (successful.length === ids.length) {
                  ...
                  setError(
                     'updateMultiple',
                     `Only ${successful.length} of ${ids.length} medications were updated`
                  );
                  return false;
               }
            } catch (error) {
               ...
               if (successful.length === ids.length) {
                  ...
                  set((state) => ({
                     ...
                     selectedMedication:
                        ...
                        ids.includes(state.selectedMedication._id)
                           ? null
                           : state.selectedMedication,
                  }));
                  return true;
                  ...
                  setError(
                     'deleteMultiple',
                     `Only ${successful.length} of ${ids.length} medications were deleted`
                  );
                  return false;
               }
            } catch (error) {
         ...
         getPatientMedicationSummary: (patientId) => {
            ...
            return {
               ...
               completed: patientMeds.filter((m) => m.status === 'completed')
                  .length,
               discontinued: patientMeds.filter(
...
export const useSelectedMedication = () =>
   useMedicationStore((state) => ({
      selectedMedication: state.selectedMedication,
