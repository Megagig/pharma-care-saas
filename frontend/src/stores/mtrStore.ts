      ...
      (set, get) => ({
         ...
         createReview: async (patientId: string) => {
            ...
            try {
               ...
               if (!patientId) {
                  throw new Error(
                     'Patient ID is required to create an MTR review'
                  );
               }
               ...
               console.log(
                  '✅ Permissions validated, proceeding with MTR creation'
               );
               ...
               console.log(
                  'Checking all possible locations for MTR review ID...'
               );
               ...
               if (
                  ...
                  if (
                     ...
                     if (session._id || session.id) {
                        ...
                        console.log(
                           'Found ID in response.review.session:',
                           reviewId
                        );
                     }
                  }
               ...
               if (
                  ...
                  if (
                     ...
                     if (session._id || session.id) {
                        reviewId =
                           (session._id as string) || (session.id as string);
                        console.log(
                           'Found ID in response.data.session:',
                           reviewId
                        );
                     }
                  }
                  ...
                  if (!reviewId && (data._id || data.id)) {
                     ...
                     console.log(
                        'Found ID directly in response.data:',
                        reviewId
                     );
                  }
               }
               ...
               if (response.review) {
                  ...
                  if (
                     ...
                     if (sessionId) {
                        console.log(
                           'Found session ID, applying to review:',
                           sessionId
                        );
                        ...
                        reviewObj = {
                           ...
                           workplaceId:
                              (sessionData.workplaceId as string) || 'unknown',
                           patientId:
                              (sessionData.patientId as string) || patientId,
                           pharmacistId:
                              (sessionData.pharmacistId as string) ||
                              'current-user',
                           reviewNumber:
                              (sessionData.reviewNumber as string) ||
                              `MTR-${Date.now()}`,
                           status:
                           ...
                           confidentialityAgreed:
                              !!sessionData.confidentialityAgreed,
                           startedAt:
                           ...
                           updatedAt:
                              ...
                              new Date().toISOString(),
                           createdBy:
                              (sessionData.createdBy as string) ||
                              'current-user',
                  ...
                  if (reviewObj && reviewObj._id) {
                     ...
                     console.log(
                        'Using response.review format. Found ID:',
                        reviewId
                     );
                  }
               }
               // Case 2: Nested in response.data.session (new API format)
                  ...
                  const responseData = response.data as Record<string, unknown>;
                  const sessionData = responseData.session as Record<
                     string,
                     unknown
                  >;
                  ...
                  if (reviewId) {
                     ...
                     if (sessionData.completedAt) {
                        reviewObj.completedAt =
                           sessionData.completedAt as string;
                     }
               ...
               // Case 3: Directly in response.data (alternative format)
                  ...
                  if (reviewId) {
                     ...
                     reviewObj = {
                        _id: reviewId,
                        workplaceId:
                           (responseData.workplaceId as string) || 'unknown',
                        patientId: (responseData.patientId as string) || '',
                        pharmacistId:
                           (responseData.pharmacistId as string) || '',
                        reviewNumber:
                           (responseData.reviewNumber as string) ||
                           `MTR-${Date.now()}`,
                        status:
                        ...
                        interventions: (responseData.interventions ||
                           []) as string[],
                        ...
                        confidentialityAgreed:
                           !!responseData.confidentialityAgreed,
                        startedAt:
                     ...
                     if (responseData.completedAt) {
                        reviewObj.completedAt =
                           responseData.completedAt as string;
                     }
               ...
               if (reviewId && !reviewObj && response.review) {
                  ...
                  const reviewAny =
                     response.review as MedicationTherapyReview & {
                        session?: {
                           _id?: string;
                           id?: string;
                        };
         ...
         getNextStep: () => {
            ...
            try {
               ...
               for (let i = 0; i < stepOrder.length; i++) {
                  const stepName = stepOrder[
                     i
                  ] as keyof typeof currentReview.steps;
                  if (
         ...
         updateIntervention: (
            id: string,
            updates: Partial<MTRIntervention>
         ) => {
            set((state) => ({
         ...
         canCompleteReview: () => {
            ...
            try {
               ...
               const allStepsCompleted = Object.values(
                  currentReview.steps
               ).every((step) => step && step.completed);
               ...
               console.error(
                  'Error checking if review can be completed:',
                  error
               );
               return false;
            }
...
export const useMTRPatient = () =>
   useMTRStore((state) => ({
      ...
      loading:
         state.loading.searchPatients ||
         state.loading.createNewPatient ||
         false,
      error:
         state.errors.searchPatients || state.errors.createNewPatient || null,
   }));
...
export const useMTRFollowUps = () =>
   useMTRStore((state) => ({
      ...
      loading:
         state.loading.scheduleFollowUp ||
         state.loading.completeFollowUp ||
         false,
      error:
