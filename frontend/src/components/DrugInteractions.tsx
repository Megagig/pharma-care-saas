...
const DrugInteractions: React.FC<DrugInteractionsProps> = ({
   rxcui,
   rxcuis,
   drugName,
}) => {
   const {
      ...
      isLoading,
      error,
   } = useDrugInteractions(rxcui, rxcuis);
      ...
      group.interactionType.forEach((type) => {
         type.interactionPair.forEach((pair) => {
            interactionPairs.push({
               ...
               drug2:
                  pair.interactionConcept[1]?.minConceptItem?.name || 'Unknown',
         ...
         {interactionPairs.length === 0 ? (
            ...
            <Box>
               {interactionPairs.map((interaction, index) => (
                  <Paper
                     key={index}
                     className="p-3 mb-3 border-l-4 border-blue-500"
                  >
                     <Box
                        display="flex"
