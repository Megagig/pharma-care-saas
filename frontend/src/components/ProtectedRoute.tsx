...
const AccessDenied: React.FC<AccessDeniedProps> = ({
   ...
   const getIcon = () => {
      switch (reason) {
         case 'subscription':
            return (
               <CreditCardIcon sx={{ fontSize: 64, color: 'warning.main' }} />
            );
         case 'license':
         ...
         >
            ...
            {reason === 'license' && licenseStatus === 'pending' && (
               <Alert severity="warning" sx={{ mt: 2, mb: 2 }}>
                  Your license is currently under review. You'll be notified
                  once it's approved.
               </Alert>
            )}
...
const ProtectedRoute: React.FC<ProtectedRouteProps> = ({
   ...
   if (requiredFeature && !hasFeature(requiredFeature)) {
      return (
         <AccessDenied reason="feature" requiredFeature={requiredFeature} />
      );
   }
