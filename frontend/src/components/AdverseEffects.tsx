...
const AdverseEffects: React.FC<AdverseEffectsProps> = ({
   drugId,
   drugName,
}) => {
   const {
   ...
   const reactions: {
      [key: string]: { count: number; seriousness: string[] };
   } = {};
      ...
      if (report.patient && report.patient.reaction) {
         report.patient.reaction.forEach((reaction) => {
            ...
            if (report.seriousnesslifethreatening === '1')
               seriousness.push('Life Threatening');
            if (report.seriousnesshospitalization === '1')
               seriousness.push('Hospitalization');
   ...
   return (
      <Paper elevation={2} className="p-4">
         ...
         {sortedReactions.length === 0 ? (
            <Alert severity="info">No adverse effects reported</Alert>
         ) : (
            <List>
               {sortedReactions.map(
                  ({ reaction, count, seriousness }, index) => (
