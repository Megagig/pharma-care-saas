      ...
      const handleServiceWorkerMessage = (event: MessageEvent) => {
         ...
         switch (type) {
            case 'INTERVENTION_SYNCED':
               setSyncStatus((prev) => ({
                  ...prev,
                  pendingInterventions: Math.max(
                     0,
                     prev.pendingInterventions - 1
                  ),
      ...
      try {
         ...
         for (const intervention of offlineInterventions) {
            try {
               ...
               if (response.ok) {
                  await offlineStorage.removeOfflineIntervention(
                     intervention.id
                  );
                  setSyncStatus((prev) => ({
                     ...prev,
            ...
            <Collapse in={showDetailsPanel}>
               ...
               >
                  ...
                  {syncStatus.pendingInterventions > 0 && (
                     <Box sx={{ mb: 1 }}>
                        <Typography variant="body2" color="warning.main">
                           {syncStatus.pendingInterventions} intervention(s)
                           pending sync
                        </Typography>
                     </Box>
                  )}
                  ...
                  <Box sx={{ mb: 2 }}>
                     ...
                     >
                        <Typography variant="caption">
                           • {storageStats.offlineInterventions} interventions
                           queued
                        </Typography>
                        <Typography variant="caption">
