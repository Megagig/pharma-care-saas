...
const STRATEGY_TYPES = {
   ...
   custom: {
      ...
      color: '#607d8b',
      defaultRationale:
         'Unique intervention approach tailored to patient needs',
      defaultOutcome:
...
const StrategyRecommendationStep: React.FC<StrategyRecommendationStepProps> = ({
   ...
   const [expandedStrategy, setExpandedStrategy] = useState<number | null>(
      null
   );
   ...
   const handleStrategyTypeChange = (index: number, newType: string) => {
      ...
      setValue(
         `strategies.${index}.rationale`,
         strategyConfig.defaultRationale
      );
      setValue(
               ...
               {loadingRecommendations ? (
                  ...
                  <Grid container spacing={2}>
                     {recommendedStrategies.map((strategyType) => {
                        const config =
                           STRATEGY_TYPES[
                              strategyType as keyof typeof STRATEGY_TYPES
                           ];
                        const isSelected =
               ...
               >
                  ...
                  <Box>
                     <IconButton
                        size="small"
                        onClick={() =>
                           setExpandedStrategy(isExpanded ? null : index)
                        }
                     >
                           ...
                           <Controller
                              ...
                              render={({ field }) => (
                                 <TextField
                                    ...
                                    error={
                                       !!errors.strategies?.[index]?.rationale
                                    }
                                    helperText={
                                       errors.strategies?.[index]?.rationale
                                          ?.message ||
      ...
      <Card sx={{ mb: 3 }}>
         <CardContent>
            ...
            {fields.length === 0 ? (
               ...
               <Box>
                  {fields.map((field, index) => renderStrategyForm(index))}
               </Box>
            )}
         </CardContent>
      </Card>
   ...
   return (
      <Box>
         ...
         <Typography variant="body1" color="text.secondary" sx={{ mb: 3 }}>
            Select and customize intervention strategies based on the clinical
            issue
         </Typography>
