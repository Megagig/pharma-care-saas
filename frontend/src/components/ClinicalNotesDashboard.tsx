...
const ClinicalNotesDashboard: React.FC<ClinicalNotesDashboardProps> = ({
   ...
   const handleBulkToggleConfidential = async (isConfidential: boolean) => {
      try {
         const success = await bulkUpdateNotes(selectedNotes, {
            isConfidential,
         });
         if (success) {
            setSnackbar({
   ...
   return (
      <Box
         sx={{ height: maxHeight || (embedded ? 600 : '100%'), width: '100%' }}
      >
         {!embedded && (
            <Box sx={{ mb: 3 }}>
               <Typography
               ...
               {isLoading || loading.fetchNotes ? (
                  ...
                  <>
                     ...
                     {data && data.totalPages > 1 && (
                        ...
                        >
                           <Typography variant="body2" color="text.secondary">
                              Showing {data.notes.length} of {data.total} total
                              rows
                           </Typography>
                           <Stack
                              direction="row"
            ...
            <DialogActions>
               ...
               <Button
                  onClick={() =>
                     ...
                     handleDeleteNote(selectedNoteForAction)
                  }
                  color="error"
                  variant="contained"
               >
         ...
         >
            ...
            <DialogContent>
               <Typography>
                  ...
                  selected notes? This action cannot be undone.
               </Typography>
            </DialogContent>
            <DialogActions>
