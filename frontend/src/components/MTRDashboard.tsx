...
const MTR_STEPS: MTRStepConfig[] = [
   ...
   },
   {
      ...
      description:
         'Collect comprehensive medication history and current regimen',
...
const MTRDashboard: React.FC<MTRDashboardProps> = ({
   ...
   const autoSave = useCallback(async () => {
      ...
      if (!currentReview._id) {
         console.error(
            'Auto-save skipped: Review ID is missing',
            currentReview
         );
         return;
      }
   ...
   const handleComplete = async () => {
      ...
      if (
         ...
         try {
            ...
            if (latestReview && latestReview._id) {
               ...
               console.log(
                  'Attempting to reload review using URL ID:',
                  reviewId
               );

               try {
                  await loadReview(reviewId);
                  const { currentReview: reloadedReview } =
                     useMTRStore.getState();
                  ...
                  setSnackbarMessage(
                     'Cannot complete - Failed to reload review'
                  );
                  setSnackbarSeverity('error');
                  setSnackbarOpen(true);
                  return;
            ...
            setSnackbarMessage(
               'Cannot complete - Error during recovery attempt'
            );
            setSnackbarSeverity('error');
            setSnackbarOpen(true);
            return;
      ...
      try {
         ...
         if (result) {
            ...
            throw new Error(
               'Complete operation did not return expected result'
            );
         }
      } catch (error) {
                           ...
                           ? {
                                ...
                                state:
                                   (selectedPatient.address
                                      ?.state as NigerianState) || undefined,
                             }
                           : null
                     }
                  />
      ...
      <MTRErrorBoundary>
         ...
         >
                  ...
                  >
                     ...
                     {currentStep < MTR_STEPS.length - 1 ? (
                        ...
                        <Button
                           ...
                           disabled={
                              !canCompleteReview() || loading.completeReview
                           }
                           size={isMobile ? 'large' : 'medium'}
                           sx={{ minWidth: isMobile ? 140 : 'auto' }}
                        >
                           {loading.completeReview
                              ? 'Completing...'
                              : 'Complete Review'}
                        </Button>
                     )}
            ...
            >
               ...
               <DialogContent>
                  <Typography>
                     Are you sure you want to cancel this MTR session? Any
