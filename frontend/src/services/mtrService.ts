      ...
      Object.keys(transformed.steps).forEach((stepKey) => {
         ...
         if (step?.completedAt && typeof step.completedAt === 'string') {
            try {
               ...
               console.warn(
                  `Failed to transform step completedAt date:`,
                  error
               );
            }
         }
...
export const mtrService = {
            ...
            (mtr: MedicationTherapyReview) => {
               ...
               return {
                  ...
                  completionPercentage:
                     calculateCompletionPercentage(transformed),
   ...
   async createMTRSession(
      ...
      try {
         ...
         if (!transformed.steps) {
            console.warn(
               'MTR steps not initialized, creating default structure'
            );
            transformed.steps = {
               patientSelection: {
                  completed: false,
   ...
   async createIntervention(
      ...
      try {
         ...
         if (!interventionData.documentation?.trim()) {
            throw new MTRValidationError(
               'Intervention documentation is required'
            );
         }
   ...
   async getMTRSummary(sessionId: string): Promise<{
      ...
      try {
         const [session, problems, interventions, followUps] =
            await Promise.all([
