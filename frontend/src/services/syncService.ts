      ...
      try {
         ...
         for (const item of syncQueue) {
            try {
               ...
               result.failed++;
               result.errors.push(
                  `${item.entity} ${item.type}: ${error instanceof Error ? error.message : 'Unknown error'}`
               );
               ...
               if (item.retryCount < 3) {
                  ...
                  await offlineStorage.removeSyncQueueItem(item.id);
                  result.errors.push(
                     `${item.entity} ${item.type}: Max retries exceeded, item removed`
                  );
               }
            }
         }
   ...
   private async syncMedication(item: unknown): Promise<void> {
      ...
      switch (item.type) {
         ...
         case 'update':
            ...
            console.log(
               'Medication sync not fully implemented - requires MTR context'
            );
            break;
         case 'delete':
