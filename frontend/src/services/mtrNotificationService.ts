...
class MTRNotificationService {
   ...
   async getFollowUpReminders(followUpId: string): Promise<FollowUpReminder[]> {
      const response = await apiClient.get(
         `${this.baseUrl}/follow-up/${followUpId}/reminders`
      );
      return response.data;
   }
   ...
   async cancelScheduledReminder(
      ...
      await apiClient.delete(
         `${this.baseUrl}/follow-up/${followUpId}/reminder/${reminderId}`
      );
   }
   ...
   async checkDrugInteractions(
      ...
      const response = await apiClient.post(
         `${this.baseUrl}/alert/drug-interactions`,
         {
            patientId,
            medications,
         }
   ...
   async notifyHighSeverityDTP(problemId: string): Promise<void> {
      await apiClient.post(
         `${this.baseUrl}/alert/high-severity-dtp/${problemId}`
      );
   }
