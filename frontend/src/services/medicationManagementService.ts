   ...
   getAdherenceAnalytics: async (
      ...
      try {
         ...
         return {
            ...
            adherenceByTimeOfDay: timeOfDay.map((time) => ({
               time,
               adherence: 0,
            })),
            costsData: {
      ...
      try {
         ...
         return {
            reminderSettings: {
               ...
               reminderMethod:
                  settings.reminderSettings?.reminderMethod ?? 'email',
               ...
               repeatReminders:
                  settings.reminderSettings?.repeatReminders ?? false,
               repeatInterval: settings.reminderSettings?.repeatInterval ?? 0,
               smartReminders:
                  settings.reminderSettings?.smartReminders ?? false,
               allowSnooze: settings.reminderSettings?.allowSnooze ?? false,
               snoozeOptions: settings.reminderSettings?.snoozeOptions ?? [],
               notifyCaregiver:
                  settings.reminderSettings?.notifyCaregiver ?? false,
               caregiverContact:
                  settings.reminderSettings?.caregiverContact ?? '',
            },
            monitoringSettings: {
               adherenceMonitoring:
                  settings.monitoringSettings?.adherenceMonitoring ?? true,
               refillReminders:
                  settings.monitoringSettings?.refillReminders ?? true,
               interactionChecking:
                  settings.monitoringSettings?.interactionChecking ?? true,
               refillThreshold:
                  settings.monitoringSettings?.refillThreshold ?? 20,
               missedDoseThreshold:
