...
class ClinicalNoteService {
   ...
   async getNote(id: string): Promise<{ note: ClinicalNote }> {
      try {
         const response = await api.get<{ note: ClinicalNote }>(
            `${this.baseUrl}/${id}`
         );
         return response.data;
      } catch (error: any) {
   ...
   ): Promise<{ note: ClinicalNote }> {
      try {
         const response = await api.post<{ note: ClinicalNote }>(
            `${this.baseUrl}`,
            data
         );
         return response.data;
   ...
   async updateNote(
      ...
      try {
         const response = await api.put<{ note: ClinicalNote }>(
            `${this.baseUrl}/${id}`,
            data
         );
         return response.data;
   ...
   async downloadAttachment(
      ...
      try {
         const response = await api.get(
            `${this.baseUrl}/${noteId}/attachments/${attachmentId}/download`,
            {
               responseType: 'blob',
            }
         );
...
export const clinicalNoteUtils = {
   ...
   formatNoteContent(content: ClinicalNote['content']): string {
      ...
      if (content.subjective)
         sections.push(`**Subjective:** ${content.subjective}`);
      if (content.objective)
         sections.push(`**Objective:** ${content.objective}`);
      if (content.assessment)
         sections.push(`**Assessment:** ${content.assessment}`);
    ...
    */
   getPriorityInfo(priority: ClinicalNote['priority']): {
      name: string;
      color: string;
   } {
      const priorityMap = {
    ...
    */
   validateNoteData(data: ClinicalNoteFormData): {
      isValid: boolean;
      errors: string[];
   } {
      const errors: string[] = [];
