   ...
   return useMutation({
      ...
      onSuccess: (data: unknown) => {
         ...
         queryClient.invalidateQueries({
            queryKey: queryKeys.clinicalNotes.lists(),
         });
      ...
      onError: (error: unknown) => {
         addNotification({
            ...
            message:
               error.message ||
               'Failed to create clinical note. Please try again.',
            duration: 5000,
         });
      },
   });
   ...
   return useMutation({
      ...
      onError: (error: unknown) => {
         addNotification({
            ...
            message:
               error.message ||
               'Failed to update clinical note. Please try again.',
            duration: 5000,
         });
      },
   });
...
export const useToggleNotePrivacy = () => {
   ...
   return useMutation({
      mutationFn: (noteId: string) =>
         clinicalNoteService.toggleNotePrivacy(noteId),
      onMutate: async (noteId) => {
         ...
         queryClient.setQueryData(
            queryKeys.clinicalNotes.detail(noteId),
            (old: unknown) =>
               old ? { ...old, isPrivate: !old.isPrivate } : old
         );
      ...
      onError: (error, noteId, context) => {
         ...
         addNotification({
            ...
            message:
               error.message ||
               'Failed to update note privacy. Please try again.',
            duration: 5000,
         });
      },
      onSuccess: (data: unknown) => {
...
export const useDeleteClinicalNote = () => {
   ...
   return useMutation({
      mutationFn: (noteId: string) =>
         clinicalNoteService.deleteClinicalNote(noteId),
