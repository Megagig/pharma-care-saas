...
export const useSearchPatients = (searchQuery: string) => {
   return useQuery({
      ...
      enabled:
         !!searchQuery && searchQuery.length >= 2 && searchQuery.trim() !== '', // Only search with 2+ characters and non-empty
   ...
   return useMutation({
      ...
      onError: (error: ApiError) => {
         addNotification({
            ...
            message:
               error.message || 'Unable to record allergy. Please try again.',
            duration: 5000,
         });
      },
   });
...
export const useUpdateAllergy = () => {
   ...
   return useMutation({
      ...
      onSuccess: (data, variables) => {
         ...
         queryClient.invalidateQueries({
            queryKey: queryKeys.allergies.lists(),
         });
...
export const useUpdateCondition = () => {
   ...
   return useMutation({
      ...
      onSuccess: (data, variables) => {
         ...
         queryClient.invalidateQueries({
            queryKey: queryKeys.conditions.lists(),
         });
...
export const usePastMedications = (patientId: string) => {
   return useQuery({
      ...
      queryFn: () =>
         patientService.getMedications(patientId, { phase: 'past' }),
      enabled: !!patientId,
   });
};
