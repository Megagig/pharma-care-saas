...
export const clinicalNoteKeys = {
    ...
    patient: (patientId: string) => [...clinicalNoteKeys.all, 'patient', patientId] as const,
...
export const useClinicalNotes = (filters: ClinicalNoteFilters = {}) => {
    return useQuery({
        ...
        queryFn: () => clinicalNoteService.getNotes(filters),
...
export const usePatientNotes = (patientId: string, filters: Omit<ClinicalNoteFilters, 'patientId'> = {}) => {
    return useQuery({
...
export const useSearchNotes = (query: string, filters: Omit<ClinicalNoteFilters, 'search'> = {}) => {
    return useQuery({
...
export const useNoteStatistics = (filters: { dateFrom?: string; dateTo?: string } = {}) => {
    return useQuery({
...
export const useCreateNote = () => {
    ...
    return useMutation({
        mutationFn: (data: ClinicalNoteFormData) => clinicalNoteService.createNote(data),
        onSuccess: (response, variables) => {
            ...
            queryClient.setQueryData(
                clinicalNoteKeys.detail(response.note._id),
                { note: response.note }
            );
        },
        onError: (error) => {
...
export const useUpdateNote = () => {
    ...
    return useMutation({
        mutationFn: ({ id, data }: { id: string; data: Partial<ClinicalNoteFormData> }) =>
            clinicalNoteService.updateNote(id, data),
        onSuccess: (response, variables) => {
            // Update the specific note in cache
            queryClient.setQueryData(
                clinicalNoteKeys.detail(variables.id),
                { note: response.note }
            );
...
export const useDeleteNote = () => {
    ...
    return useMutation({
        ...
        onSuccess: (_, deletedId) => {
            ...
            queryClient.removeQueries({ queryKey: clinicalNoteKeys.detail(deletedId) });
            queryClient.invalidateQueries({ queryKey: clinicalNoteKeys.lists() });
...
export const useBulkUpdateNotes = () => {
    ...
    return useMutation({
        mutationFn: (data: BulkUpdateData) => clinicalNoteService.bulkUpdateNotes(data),
        onSuccess: () => {
...
export const useBulkDeleteNotes = () => {
    ...
    return useMutation({
        mutationFn: (noteIds: string[]) => clinicalNoteService.bulkDeleteNotes(noteIds),
...
export const useUploadAttachment = () => {
    ...
    return useMutation({
        ...
        onSuccess: (response, variables) => {
            ...
            queryClient.setQueryData(
                clinicalNoteKeys.detail(variables.noteId),
                { note: response.note }
            );
