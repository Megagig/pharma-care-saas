...
test.describe('MTR Performance E2E Tests', () => {
   ...
   test('should handle large medication lists efficiently', async ({
      ...
      for (let i = 0; i < 50; i++) {
         ...
         await page.fill(
            ...
            `Performance test indication ${i + 1}`
         );
      ...
      console.log(
         `⏱️ Added 50 medications in ${totalTime}ms (${totalTime / 50}ms per medication)`
      );
      ...
      await page.evaluate(() => {
         const container = document.querySelector(
            '[data-testid="medication-list-container"]'
         );
         if (container) {
      ...
      await expect(
         page.locator('[data-testid="medication-item-Performance Med 25"]')
      ).toBeVisible();
   ...
   test('should maintain performance with multiple concurrent operations', async ({
      ...
      const medicationItems = await page
         .locator('[data-testid^="medication-item-Concurrent Med"]')
         .all();
   ...
   test('should handle rapid user interactions without lag', async ({
      ...
      for (let i = 0; i < 20; i++) {
         ...
         // Rapid typing
         await page.type('[data-testid="strength-input"]', `${10 + i} mg`, {
            delay: 10,
         });
      ...
      await expect(
         page.locator('[data-testid="medication-list-container"]')
      ).toBeVisible();
   ...
   test('should optimize memory usage with large datasets', async ({
      ...
      const visibleItems = await listContainer
         .locator('[data-testid^="medication-item-"]')
         .all();
   ...
   test('should load initial data quickly', async ({ page }) => {
      ...
      await expect(
         page.locator('[data-testid="new-mtr-button"]')
      ).toBeVisible();
   ...
   test('should handle network latency gracefully', async ({ page }) => {
      ...
      console.log(
         `🐌 MTR session started with 2s latency in ${sessionStartTime}ms`
      );
      ...
      await expect(
         page.locator('[data-testid="medication-history-step"]')
      ).toBeVisible();

      console.log('✅ Network latency test passed!');
   });

   test('should maintain performance during auto-save operations', async ({
      page,
   }) => {
      ...
      for (let i = 0; i < 20; i++) {
         ...
         await expect(
            page.locator('[data-testid="add-medication-button"]')
         ).toBeEnabled();
      }
      ...
      console.log(
         `💾 Completed 20 operations with auto-save in ${totalTime}ms`
      );
      expect(totalTime).toBeLessThan(60000); // Should complete within 1 minute
