...
test.describe('MTR Complete Workflow E2E Tests', () => {
   ...
   test('should handle complex MTR with multiple medications and problems', async ({
      page,
   }) => {
      ...
      await page.fill(
         ...
         complexMTRData.patient.mrn
      );
      await page.click(
         `[data-testid="view-mtr-${complexMTRData.patient.mrn}"]`
      );
      ...
      const problemItems = await page
         .locator('[data-testid^="problem-item-"]')
         .all();
      expect(problemItems.length).toBeGreaterThanOrEqual(
         complexMTRData.problems.length
      );
   ...
   test('should auto-save progress during MTR session', async ({ page }) => {
      ...
      await expect(
         page.locator('[data-testid="medication-item-Auto-save Test Med"]')
      ).toBeVisible();
   ...
   test('should validate required fields in each step', async ({ page }) => {
      ...
      await expect(
         page.locator('[data-testid="patient-required-error"]')
      ).toBeVisible();
   ...
   test('should handle concurrent MTR sessions', async ({ page, context }) => {
      ...
      await expect(
         page2.locator('[data-testid="medication-history-step"]')
      ).toBeVisible();
      ...
      await expect(
         page.locator('[data-testid="medication-item-Session 1 Med"]')
      ).toBeVisible();
      await expect(
         page2.locator('[data-testid="medication-item-Session 2 Med"]')
      ).toBeVisible();
