...
test.describe('MTR Error Scenarios and Recovery E2E Tests', () => {
   ...
   test('should handle network failures gracefully', async ({
      ...
      await expect(
         page.locator('[data-testid="medication-item-Network Test Med"]')
      ).toBeVisible();
   ...
   test('should handle server errors with retry mechanism', async ({
      ...
      // Should show error message
      await expect(
         page.locator('[data-testid="server-error-message"]')
      ).toBeVisible();
      ...
      await expect(
         page.locator('[data-testid="patient-selection-step"]')
      ).toBeVisible();
   ...
   test('should validate and handle invalid input data', async ({ page }) => {
      ...
      await page.click('[data-testid="save-medication-button"]');
      await expect(
         page.locator('[data-testid="drug-name-required-error"]')
      ).toBeVisible();
      ...
      await page.click('[data-testid="save-medication-button"]');
      await expect(
         page.locator('[data-testid="strength-format-error"]')
      ).toBeVisible();
      ...
      await expect(
         page.locator('[data-testid="medication-item-Test Med"]')
      ).toBeVisible();
   ...
   test('should recover from browser crashes and data loss', async ({
      page,
   }) => {
      ...
      if (
         await page
            .locator('[data-testid="recover-session-button"]')
            .isVisible()
      ) {
         ...
         await expect(
            page.locator('[data-testid="medication-item-Crash Test Med"]')
         ).toBeVisible();
      }
   ...
   test('should handle concurrent user conflicts', async ({ context }) => {
      ...
      await page2.fill(
         ...
         sampleMTRData.patient.mrn
      );
      await page2.waitForTimeout(1000);
      await page2.click(
         `[data-testid="patient-result-${sampleMTRData.patient.mrn}"]`
      );
      await page2.click('[data-testid="confirm-patient-button"]');

      // Should show conflict warning
      await expect(
         page2.locator('[data-testid="session-conflict-warning"]')
      ).toBeVisible();
   ...
   test('should handle large data sets without performance degradation', async ({
      page,
   }) => {
      ...
      const medicationItems = await page
         .locator('[data-testid^="medication-item-Performance Med"]')
         .all();
      expect(medicationItems).toHaveLength(20);

      console.log(
         `✅ Large data performance test passed! (${totalTime}ms for 20 medications)`
      );
   });
   ...
   test('should handle malicious input safely', async ({ page }) => {
      ...
      for (const maliciousInput of maliciousInputs) {
         ...
         const medicationItem = page.locator(
            `[data-testid="medication-item-${maliciousInput}"]`
         );
         ...
         if (
            await page
               .locator('[data-testid="delete-medication-button"]')
               .isVisible()
         ) {
