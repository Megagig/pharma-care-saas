        ...
        socket.on("notification:get_unread_count", async () => {
            try {
                const unreadCount = await Notification_1.default.countDocuments({
                    userId: new mongoose_1.default.Types.ObjectId(socket.userId),
                    workplaceId: new mongoose_1.default.Types.ObjectId(socket.workplaceId),
                    status: "unread",
                });
    ...
    async sendNotificationToUser(userId, notification) {
        ...
        if (userSockets && userSockets.size > 0) {
            ...
            const unreadCount = await Notification_1.default.countDocuments({
                ...
                workplaceId: new mongoose_1.default.Types.ObjectId(notification.workplaceId.toString()),
                status: "unread",
            });
            this.io
                .to(`user:${userId}`)
