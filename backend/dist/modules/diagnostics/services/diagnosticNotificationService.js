...
exports.diagnosticNotificationService = void 0;
...
class DiagnosticNotificationService {
    ...
    async scheduleFollowUpReminder(followUpId, reminderType = 'email', scheduledFor) {
        try {
            ...
            const diagnosticResult = followUp.diagnosticResultId;
            ...
            const notificationData = {
                ...
                diagnoses: diagnosticResult?.diagnoses?.map((d) => d.condition) || []
            };
            const channels = [];
    ...
    async scheduleAdherenceAlert(adherenceTrackingId, alert) {
        try {
            const tracking = await AdherenceTracking_1.default.findById(adherenceTrackingId)
                .populate('patientId', 'firstName lastName mrn workplaceId');
            if (!tracking) {
                throw new Error('Adherence tracking not found');
            }
            const patient = tracking.patientId;
            ...
            logger_1.default.info(`Adherence alert sent for patient ${patient.mrn}: ${alert.message}`);
        }
        catch (error) {
            logger_1.default.error('Error scheduling adherence alert:', error);
            throw error;
        }
    }
    async sendMissedRefillReminder(adherenceTrackingId, medicationName, daysOverdue) {
        try {
            const tracking = await AdherenceTracking_1.default.findById(adherenceTrackingId)
            ...
            logger_1.default.info(`Missed refill reminder sent for patient ${patient.mrn}: ${medicationName}`);
        }
        catch (error) {
            logger_1.default.error('Error sending missed refill reminder:', error);
            throw error;
        }
    }
    async checkOverdueFollowUps() {
        try {
            const overdueFollowUps = await DiagnosticFollowUp_1.default.find({
    ...
    async checkAdherenceIssues() {
        try {
            const trackingRecords = await AdherenceTracking_1.default.find({
                monitoringActive: true,
                'alerts.acknowledged': false
            }).populate('patientId', 'firstName lastName mrn');
            for (const tracking of trackingRecords) {
                const unacknowledgedAlerts = tracking.alerts.filter(alert => !alert.acknowledged && !alert.resolved);
                for (const alert of unacknowledgedAlerts) {
                    const alertAge = Math.floor((Date.now() - alert.triggeredAt.getTime()) / (1000 * 60 * 60 * 24));
                ...
                for (const medication of tracking.medications) {
                    if (medication.expectedRefillDate && medication.expectedRefillDate < new Date()) {
                        ...
                        if (daysOverdue >= tracking.alertPreferences.reminderDaysBefore) {
                            await this.sendMissedRefillReminder(tracking._id, medication.medicationName, daysOverdue);
                        }
                    }
                }
            }
            logger_1.default.info(`Checked adherence issues for ${trackingRecords.length} patients`);
        }
        catch (error) {
    ...
    generateAdherenceAlertEmail(data) {
    ...
    generateMissedRefillEmail(data) {
        return `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
    ...
    generateOverdueFollowUpEmail(data) {
        return `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                <div style="background-color: #fef3c7; border-left: 4px solid #d97706; padding: 20px; margin-bottom: 20px;">
    ...
    async processPendingNotifications() {
        try {
            ...
            const pendingFollowUps = await DiagnosticFollowUp_1.default.find({
            ...
            logger_1.default.info(`Processed pending notifications`);
        }
        catch (error) {
...
exports.diagnosticNotificationService = new DiagnosticNotificationService();
exports.default = exports.diagnosticNotificationService;
//# sourceMappingURL=diagnosticNotificationService.js.map