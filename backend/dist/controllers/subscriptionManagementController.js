...
exports.subscriptionManagementController = exports.SubscriptionManagementController = void 0;
...
class SubscriptionManagementController {
    async getWorkspaceSubscription(req, res) {
        try {
            ...
            const isTeamMember = workspace.teamMembers.some((memberId) => memberId.toString() === req.user._id.toString());
    ...
    async createTrialSubscription(req, res) {
        try {
            const { workspaceId, trialDurationDays = 14 } = req.body;
            if (!workspaceId) {
                return res.status(400).json({
                    success: false,
                    message: 'Workspace ID is required',
            ...
            const trialPlan = await SubscriptionPlan_1.default.findOne({
                tier: 'free_trial',
                isActive: true,
            });
            if (!trialPlan) {
            ...
            if (owner) {
                await emailService_1.emailService.sendTrialActivation(owner.email, {
    ...
    async createWorkspaceCheckout(req, res) {
        try {
            ...
            if (process.env.NODE_ENV === 'development' && !paystackService_1.paystackService.isConfigured()) {
    ...
    async handleWorkspacePaymentSuccess(req, res) {
        try {
    ...
    async activateWorkspaceSubscription(paymentRecord) {
        const workspaceId = paymentRecord.metadata.workspaceId;
        ...
        await Subscription_1.default.updateMany({ workspaceId: workspaceId, status: { $in: ['active', 'trial'] } }, { status: 'canceled' });
        const startDate = new Date();
        const endDate = new Date();
        ...
        const owner = await User_1.default.findById(workspace.ownerId);
        if (owner) {
            await emailService_1.emailService.sendWorkspaceSubscriptionConfirmation(owner.email, {
                firstName: owner.firstName,
                workspaceName: workspace.name,
                planName: plan.name,
                amount: plan.priceNGN,
    ...
    async upgradeWorkspaceSubscription(req, res) {
        try {
            const { workspaceId, planId, billingInterval = 'monthly' } = req.body;
            if (!workspaceId || !planId) {
                return res.status(400).json({
            ...
            const newPlan = await SubscriptionPlan_1.default.findOne({
                _id: planId,
                billingInterval: billingInterval,
                isActive: true,
            });
            ...
            if (upgradeAmount <= 100) {
                const result = await this.applySubscriptionUpgrade(currentSubscription, newPlan, workspace, billingInterval, 0);
                return res.json({
                    success: true,
    ...
    async downgradeWorkspaceSubscription(req, res) {
        try {
            ...
            if (owner) {
                const currentPlan = currentSubscription.planId;
                await emailService_1.emailService.sendSubscriptionDowngrade(owner.email, {
                    firstName: owner.firstName,
            ...
            res.json({
                ...
                data: {
                    currentPlan: currentSubscription.planId.name,
    ...
    async handleUpgradePaymentSuccess(req, res) {
        try {
            const { paymentReference } = req.body;
            if (!paymentReference) {
                return res.status(400).json({
            ...
            const currentSubscription = await Subscription_1.default.findById(metadata.currentSubscriptionId);
            const newPlan = await SubscriptionPlan_1.default.findById(paymentRecord.planId);
            const workspace = await Workplace_1.default.findById(metadata.workspaceId);
            ...
            const result = await this.applySubscriptionUpgrade(currentSubscription, newPlan, workspace, metadata.billingInterval || 'monthly', metadata.upgradeAmount || 0);
            paymentRecord.status = 'completed';
            paymentRecord.completedAt = new Date();
            await paymentRecord.save();
            res.json({
    ...
    async applySubscriptionUpgrade(currentSubscription, newPlan, workspace, billingInterval, upgradeAmount) {
        ...
        currentSubscription.features = Object.keys(newPlan.features).filter(key => newPlan.features[key] === true);
        ...
        const owner = await User_1.default.findById(workspace.ownerId);
        if (owner) {
            await emailService_1.emailService.sendSubscriptionUpgrade(owner.email, {
                firstName: owner.firstName,
    ...
    async checkTrialExpiry(req, res) {
        try {
            const { workspaceId } = req.params;
            if (!workspaceId) {
                return res.status(400).json({
    ...
    async extendTrialPeriod(req, res) {
        try {
            ...
            const owner = await User_1.default.findById(workspace.ownerId);
            if (owner) {
                await emailService_1.emailService.sendTrialExtension(owner.email, {
                    firstName: owner.firstName,
                    workspaceName: workspace.name,
                    extensionDays,
                    newEndDate,
    ...
    async handleSubscriptionExpiry(req, res) {
        try {
    ...
    async enablePaywallMode(req, res) {
        try {
            const { workspaceId } = req.params;
            if (!workspaceId) {
                return res.status(400).json({
    ...
    async expireTrialSubscription(subscription, workspace) {
        subscription.status = 'expired';
        await subscription.save();
        workspace.subscriptionStatus = 'expired';
        await workspace.save();
    ...
    async sendStatusChangeNotification(user, workspace, subscription, newStatus) {
        try {
            switch (newStatus) {
                case 'past_due':
                    await emailService_1.emailService.sendSubscriptionPastDue(user.email, {
                    ...
                    break;
                case 'expired':
                    await emailService_1.emailService.sendSubscriptionExpired(user.email, {
                        firstName: user.firstName,
                        workspaceName: workspace.name,
                    });
                    break;
                case 'canceled':
                    await emailService_1.emailService.sendSubscriptionCanceled(user.email, {
                        firstName: user.firstName,
                        workspaceName: workspace.name,
                    });
                    break;
                case 'suspended':
                    await emailService_1.emailService.sendSubscriptionSuspended(user.email, {
                        firstName: user.firstName,
                        workspaceName: workspace.name,
                    });
                    break;
...
exports.subscriptionManagementController = new SubscriptionManagementController();
