...
exports.validatePatientAccess = exports.deleteReferral = exports.sendReferralElectronically = exports.downloadReferralDocument = exports.getFollowUpCases = exports.updateReferralDocument = exports.generateCaseReferralDocument = exports.markCaseAsCompleted = exports.markCaseForFollowUp = exports.compareDiagnosticHistories = exports.generateReferralDocument = exports.exportDiagnosticHistoryPDF = exports.getDiagnosticReferrals = exports.getAllDiagnosticCases = exports.getDiagnosticAnalytics = exports.addDiagnosticHistoryNote = exports.getPatientDiagnosticHistory = exports.saveDiagnosticNotes = exports.debugDatabaseCounts = exports.testAIConnection = exports.checkDrugInteractions = exports.getDiagnosticCase = exports.getDiagnosticHistory = exports.saveDiagnosticDecision = exports.generateDiagnosticAnalysis = void 0;
...
const generateDiagnosticAnalysis = async (req, res) => {
    try {
        ...
        const patientExists = await Patient_1.default.findById(patientId);
        logger_1.default.info('Patient existence check for diagnostic analysis', {
            patientId,
        ...
        if (!patient) {
            ...
            logger_1.default.warn('Patient access denied for diagnostic analysis', {
                patientId,
                userId,
                workplaceId,
                debugInfo,
...
const getDiagnosticHistory = async (req, res) => {
    try {
        ...
        let patient;
        if (isSuperAdmin) {
            patient = await Patient_1.default.findById(patientId);
            if (patient && patient.workplaceId.toString() !== workplaceId.toString()) {
                logger_1.default.info('Super admin cross-workplace patient access', {
                    patientId,
                    userId,
        ...
        const caseFilter = { patientId };
        if (!isSuperAdmin) {
            caseFilter.workplaceId = workplaceId;
        }
        const diagnosticCases = await DiagnosticCase_1.default.find(caseFilter)
            .populate('pharmacistId', 'firstName lastName')
            .sort({ createdAt: -1 })
            .skip(skip)
            .limit(Number(limit))
            .select('-aiRequestData -pharmacistDecision.modifications');
        const total = await DiagnosticCase_1.default.countDocuments(caseFilter);
        res.status(200).json({
            success: true,
            data: {
...
const getDiagnosticCase = async (req, res) => {
    try {
        ...
        const diagnosticCase = await DiagnosticCase_1.default.findOne(caseFilter)
        ...
        if (isSuperAdmin && diagnosticCase.workplaceId.toString() !== workplaceId.toString()) {
            logger_1.default.info('Super admin cross-workplace case access', {
                caseId,
                userId,
                userWorkplaceId: workplaceId,
...
const getDiagnosticReferrals = async (req, res) => {
    ...
    try {
        ...
        const referrals = await DiagnosticCase_1.default.find(filter)
            .populate('patientId', 'firstName lastName age gender')
            .populate('pharmacistId', 'firstName lastName')
...
const markCaseForFollowUp = async (req, res) => {
    try {
        const { caseId } = req.params;
        ...
        diagnosticCase.pharmacistDecision.reviewedBy = userId;
        await diagnosticCase.save();
        logger_1.default.info('Case marked for follow-up', {
            caseId,
            followUpDate,
            reason,
            userId,
        ...
        logger_1.default.error('Failed to mark case for follow-up', {
            error: error instanceof Error ? error.message : 'Unknown error',
            caseId: req.params.caseId,
            userId: req.user?._id,
        });
...
exports.markCaseForFollowUp = markCaseForFollowUp;
const markCaseAsCompleted = async (req, res) => {
    try {
        const { caseId } = req.params;
        ...
        await diagnosticCase.save();
        logger_1.default.info('Case marked as completed', {
            caseId,
            userId,
        ...
        logger_1.default.error('Failed to mark case as completed', {
            error: error instanceof Error ? error.message : 'Unknown error',
            caseId: req.params.caseId,
            userId: req.user?._id,
        });
...
exports.markCaseAsCompleted = markCaseAsCompleted;
const generateCaseReferralDocument = async (req, res) => {
    try {
        const { caseId } = req.params;
...
${diagnosticCase.aiAnalysis.differentialDiagnoses.slice(0, 3).map((dx, index) => `${index + 1}. ${dx.condition} (${Math.round(dx.probability)}% probability)
     Reasoning: ${dx.reasoning}`).join('\n')}

Recommended Tests:
${diagnosticCase.aiAnalysis.recommendedTests.map((test, index) => `${index + 1}. ${test.testName} (${test.priority})
     Reasoning: ${test.reasoning}`).join('\n')}

Red Flags Identified:
${diagnosticCase.aiAnalysis.redFlags.length > 0
            ? diagnosticCase.aiAnalysis.redFlags.map((flag, index) => `${index + 1}. ${flag.flag} (${flag.severity})
         Action: ${flag.action}`).join('\n')
            : 'None identified'}
...
This referral was generated with AI assistance and reviewed by a licensed pharmacist.
    `.trim();
        ...
        diagnosticCase.pharmacistDecision.reviewedBy = userId;
        await diagnosticCase.save();
        logger_1.default.info('Referral document generated', {
            caseId,
            userId,
...
exports.generateCaseReferralDocument = generateCaseReferralDocument;
const updateReferralDocument = async (req, res) => {
    try {
        const { caseId } = req.params;
        ...
        logger_1.default.error('Failed to update referral document', {
            error: error instanceof Error ? error.message : 'Unknown error',
            caseId: req.params.caseId,
            userId: req.user?._id,
        });
...
exports.updateReferralDocument = updateReferralDocument;
const getFollowUpCases = async (req, res) => {
    try {
        ...
        const followUpCases = await DiagnosticCase_1.default.find(matchFilter)
            .populate('patientId', 'firstName lastName age gender')
            .populate('pharmacistId', 'firstName lastName')
            .sort({ 'followUp.scheduledDate': 1 })
            .skip(skip)
            .limit(Number(limit));
        const total = await DiagnosticCase_1.default.countDocuments(matchFilter);
        res.status(200).json({
            success: true,
            data: {
                cases: followUpCases,
        ...
        logger_1.default.error('Failed to get follow-up cases', {
            error: error instanceof Error ? error.message : 'Unknown error',
            userId: req.user?._id,
        });
        res.status(500).json({
...
exports.getFollowUpCases = getFollowUpCases;
const downloadReferralDocument = async (req, res) => {
    try {
        ...
        logger_1.default.error('Failed to download referral document', {
            error: error instanceof Error ? error.message : 'Unknown error',
            caseId: req.params.caseId,
            userId: req.user?._id,
        });
...
exports.downloadReferralDocument = downloadReferralDocument;
const sendReferralElectronically = async (req, res) => {
    try {
        const { caseId } = req.params;
        ...
        await diagnosticCase.save();
        logger_1.default.info('Referral sent electronically', {
            caseId,
            physicianEmail,
            trackingId,
            userId,
        ...
        logger_1.default.error('Failed to send referral', {
            error: error instanceof Error ? error.message : 'Unknown error',
            caseId: req.params.caseId,
            userId: req.user?._id,
        });
...
exports.sendReferralElectronically = sendReferralElectronically;
const deleteReferral = async (req, res) => {
    try {
        ...
        await diagnosticCase.save();
        logger_1.default.info('Referral deleted', {
            caseId,
            userId,
        });
        res.status(200).json({
        ...
        logger_1.default.error('Failed to delete referral', {
            error: error instanceof Error ? error.message : 'Unknown error',
            caseId: req.params.caseId,
            userId: req.user?._id,
        });
...
const validatePatientAccess = async (req, res) => {
    try {
        const { patientId } = req.body;
        const userId = req.user._id;
        const workplaceId = req.user.workplaceId;
        logger_1.default.info('Validating patient access', {
            patientId,
            userId,
            workplaceId,
        });
        ...
        const patientExists = await Patient_1.default.findById(patientId);
        logger_1.default.info('Patient existence check', {
            patientId,
            exists: !!patientExists,
            patientWorkplaceId: patientExists?.workplaceId,
            userWorkplaceId: workplaceId,
        ...
        if (!patient) {
            ...
            logger_1.default.warn('Patient access denied', {
                patientId,
                userId,
                workplaceId,
                debugInfo,
        ...
        logger_1.default.error('Failed to validate patient access', {
            error: error instanceof Error ? error.message : 'Unknown error',
            patientId: req.body.patientId,
            userId: req.user?._id,
