        ...
        for (let attempt = 1; attempt <= this.retryAttempts; attempt++) {
            try {
                ...
                if (attempt === this.retryAttempts) {
                    logger_1.logger.error(`API call failed after ${this.retryAttempts} attempts:`, error);
                    throw error;
                }
                const delay = this.retryDelay * Math.pow(2, attempt - 1);
                logger_1.logger.warn(`API call failed (attempt ${attempt}/${this.retryAttempts}), retrying in ${delay}ms:`, error.message);
