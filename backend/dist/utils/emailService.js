...
class EmailService {
    ...
    async sendTrackedEmail(options) {
        return await emailDeliveryService_1.emailDeliveryService.sendTrackedEmail({
            to: options.to,
    ...
    async sendInvitationAcceptedNotification(inviterEmail, data) {
        try {
            const frontendUrl = process.env.FRONTEND_URL || 'http://localhost:3000';
    ...
    async sendInvitationExpiredNotification(inviterEmail, data) {
        try {
            const frontendUrl = process.env.FRONTEND_URL || 'http://localhost:3000';
            const workspaceUrl = `${frontendUrl}/workspace/settings/team`;
            const supportUrl = `${frontendUrl}/support`;
    ...
    async sendInvitationExpiredToInvitee(invitation) {
        try {
            const frontendUrl = process.env.FRONTEND_URL || 'http://localhost:3000';
    ...
    async sendInvitationReminderEmail(invitation) {
        try {
            const frontendUrl = process.env.FRONTEND_URL || 'http://localhost:3000';
            const acceptUrl = `${frontendUrl}/accept-invitation/${invitation.code}`;
            const supportUrl = `${frontendUrl}/support`;
    ...
    async sendTrialActivation(email, data) {
        const template = {
    ...
    async sendWorkspaceSubscriptionConfirmation(email, data) {
        const template = {
    ...
    async sendSubscriptionPastDue(email, data) {
        const template = {
    ...
    async sendSubscriptionExpired(email, data) {
        const template = {
    ...
    async sendTrialExpiryWarning(email, data) {
        try {
            const frontendUrl = process.env.FRONTEND_URL || 'http://localhost:3000';
    ...
    async sendUsageLimitWarning(email, data) {
        try {
            const frontendUrl = process.env.FRONTEND_URL || 'http://localhost:3000';
            const upgradeUrl = `${frontendUrl}/workspace/subscription/upgrade`;
            const workspaceUrl = `${frontendUrl}/workspace/settings`;
    ...
    async sendSubscriptionStatusChange(email, data) {
        try {
            const frontendUrl = process.env.FRONTEND_URL || 'http://localhost:3000';
            const workspaceUrl = `${frontendUrl}/workspace/settings`;
            const supportUrl = `${frontendUrl}/support`;
            ...
            const config = statusConfig[data.newStatus] || statusConfig.active;
            const templateVariables = {
                firstName: data.firstName,
                workspaceName: data.workspaceName,
    ...
    async sendSubscriptionSuspended(email, data) {
        const template = {
    ...
    async sendTrialExtension(email, data) {
        const template = {
    ...
    async sendSubscriptionExpiryWarning(email, data) {
        const template = {
    ...
    async sendSubscriptionDowngradeApplied(email, data) {
        const template = {
