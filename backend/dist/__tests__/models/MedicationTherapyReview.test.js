...
describe('MedicationTherapyReview Model', () => {
    ...
    describe('Model Creation', () => {
        it('should create a valid MTR session with required fields', async () => {
            ...
            const mtr = new MedicationTherapyReview_1.default(mtrData);
    ...
    describe('Workflow Steps', () => {
        ...
        beforeEach(async () => {
            ...
            mtr = new MedicationTherapyReview_1.default(mtrData);
            await mtr.save();
        });
        ...
        it('should calculate completion percentage correctly', () => {
            ...
            expect(mtr.getCompletionPercentage()).toBe(17);
            mtr.markStepComplete('medicationHistory');
            expect(mtr.getCompletionPercentage()).toBe(33);
    ...
    describe('Virtual Properties', () => {
        ...
        it('should determine overdue status for routine priority', () => {
            mtr.startedAt = new Date(Date.now() - 8 * 24 * 60 * 60 * 1000);
            expect(mtr.isOverdue).toBe(true);
            mtr.startedAt = new Date(Date.now() - 6 * 24 * 60 * 60 * 1000);
            expect(mtr.isOverdue).toBe(false);
        });
        it('should determine overdue status for urgent priority', () => {
            mtr.priority = 'urgent';
            mtr.startedAt = new Date(Date.now() - 2 * 24 * 60 * 60 * 1000);
            expect(mtr.isOverdue).toBe(true);
            mtr.startedAt = new Date(Date.now() - 12 * 60 * 60 * 1000);
            expect(mtr.isOverdue).toBe(false);
        });
        it('should not be overdue if completed', () => {
            mtr.status = 'completed';
            mtr.startedAt = new Date(Date.now() - 10 * 24 * 60 * 60 * 1000);
            expect(mtr.isOverdue).toBe(false);
        });
    });
    describe('Static Methods', () => {
        beforeEach(async () => {
            ...
            await MedicationTherapyReview_1.default.create([mtrData1, mtrData2]);
        });
        it('should find active reviews', async () => {
            const activeReviews = await MedicationTherapyReview_1.default.findActive(workplaceId);
            expect(activeReviews).toHaveLength(1);
            expect(activeReviews[0].status).toBe('in_progress');
        });
        it('should find overdue reviews', async () => {
            await MedicationTherapyReview_1.default.updateOne({ status: 'in_progress' }, { startedAt: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000) });
            const overdueReviews = await MedicationTherapyReview_1.default.findOverdue(workplaceId);
    ...
    describe('Indexes and Performance', () => {
        it('should have proper indexes defined', () => {
            const indexes = MedicationTherapyReview_1.default.collection.getIndexes();
            expect(MedicationTherapyReview_1.default.schema.indexes()).toBeDefined();
        });
    });
...
//# sourceMappingURL=MedicationTherapyReview.test.js.map