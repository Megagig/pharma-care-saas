...
router.post('/patient/validate', diagnosticRateLimit, auth_1.auth, (0, auth_1.requireFeature)('clinical_decision_support'), (0, auditMiddleware_1.auditLogger)('VIEW_DIAGNOSTIC_HISTORY', 'data_access'), diagnosticValidators_1.validatePatientAccessRequest, diagnosticController_1.validatePatientAccess);
...
router.post('/cases/:caseId/follow-up', diagnosticRateLimit, auth_1.auth, (0, auth_1.requireFeature)('clinical_decision_support'), (0, auditMiddleware_1.auditLogger)('MARK_CASE_FOLLOW_UP', 'clinical_documentation'), diagnosticController_1.markCaseForFollowUp);
router.post('/cases/:caseId/complete', diagnosticRateLimit, auth_1.auth, (0, auth_1.requireFeature)('clinical_decision_support'), (0, auditMiddleware_1.auditLogger)('MARK_CASE_COMPLETED', 'clinical_documentation'), diagnosticController_1.markCaseAsCompleted);
