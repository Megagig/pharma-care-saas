   ...
   private async showStatus(): Promise<void> {
      try {
         ...
         if (status.lastMigration) {
            console.log(
               `Last Migration: ${status.lastMigration.toISOString()}`
            );
         }
         ...
         if (sampleNote) {
            const needsMigration =
               !sampleNote.createdBy || !sampleNote.lastModifiedBy;
            console.log(
               `Migration Needed: ${needsMigration ? '⚠️  Yes' : '✅ No'}`
            );
         } else {
   ...
   private async runMigrations(): Promise<void> {
      try {
         if (this.options.dryRun) {
            ...
            const collection =
               mongoose.connection.db.collection('clinicalnotes');
            ...
            console.log(
               `Notes Needing Migration: ${notesNeedingMigration.toLocaleString()}`
            );
