...
class AdminUtils {
  ...
  async getWorkspaceDetails(workspaceId) {
    try {
      ...
      const subscription = await Subscription.findOne({ workspaceId }).populate(
        'planId'
      );
  ...
  async listUsers(options = {}) {
    try {
      ...
      const users = await User.find(query).limit(limit).sort({ createdAt: -1 });
    ...
    try {
      ...
      const report = {
        ...
        summary: {
          ...
          invitations: { total: totalInvitations, pending: pendingInvitations },
        },
        growth: {
      ...
      log('\n📊 System Report:', 'cyan');
      log('─'.repeat(50));
      log(`Workspaces: ${totalWorkspaces} total, ${activeWorkspaces} active`);
  ...
  while (true) {
    ...
    switch (choice) {
      ...
      case '3':
        ...
        const endDate = await question('New end date (YYYY-MM-DD, optional): ');
        const updates = {};
      ...
      case '4':
        ...
        await adminUtils.listInvitations({ status: invStatus || undefined });
        break;
      ...
      case '7':
        ...
        await adminUtils.updateUserStatus(userId, newUserStatus, statusReason);
        break;
