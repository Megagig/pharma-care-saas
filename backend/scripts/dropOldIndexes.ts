...
const dropOldIndexes = async (): Promise<void> => {
   try {
      ...
      for (const indexName of userIndexesToDrop) {
         try {
            ...
            if (error.codeName === 'IndexNotFound' || error.code === 27) {
               ...
               console.error(
                  `❌ Error dropping ${indexName} index from "users":`,
                  error.message
               );
            }
         }
      ...
      console.log(
         'Updated "users" indexes:',
         updatedUserIndexes.map((idx) => idx.name)
      );
      ...
      console.log(
         'Current "patients" indexes:',
         patientIndexes.map((idx) => idx.name)
      );

      const patientIndexesToDrop = ['pharmacyId_1_mrn_1'];
      ...
      for (const indexName of patientIndexesToDrop) {
         try {
            ...
            if (error.codeName === 'IndexNotFound' || error.code === 27) {
               console.log(
                  `ℹ️ ${indexName} index on "patients" does not exist`
               );
            } else {
               console.error(
                  `❌ Error dropping ${indexName} index from "patients":`,
                  error.message
               );
            }
         }
      ...
      console.log(
         'Updated "patients" indexes:',
         updatedPatientIndexes.map((idx) => idx.name)
      );
