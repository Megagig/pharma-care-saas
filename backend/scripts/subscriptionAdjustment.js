...
class SubscriptionAdjustmentUtils {
  ...
  async extendTrial(workspaceId, additionalDays, reason) {
    try {
      ...
      success(`Extended trial for ${workspace.name} by ${additionalDays} days`);
  ...
  async resumeSubscription(workspaceId, reason) {
    try {
      ...
      workspace.subscriptionStatus = subscription.pausedStatus.originalStatus;
    ...
    try {
      ...
      if (subscription.status === 'past_due') {
        ...
        subscription.endDate = new Date(
          subscription.endDate.getTime() + billingDays * 24 * 60 * 60 * 1000
        );
  ...
  while (true) {
    ...
    switch (choice) {
      ...
      case '3':
        ...
        const effectiveDate = await question('Effective date (YYYY-MM-DD): ');
      ...
      case '7':
        const startDate = new Date(await question('Start date (YYYY-MM-DD): '));
