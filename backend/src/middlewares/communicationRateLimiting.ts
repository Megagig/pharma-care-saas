...
const communicationRateLimitStore = new Map<string, {
    count: number;
    resetTime: number;
    messageCount: number;
...
export const createCommunicationRateLimiter = (options: {
    ...
    return rateLimit({
        ...
        max: (req: AuthRequest) => {
            ...
            return options.limits[userRole as keyof typeof options.limits] || options.limits.default;
        },
        message: {
            success: false,
            code: 'COMMUNICATION_RATE_LIMIT_EXCEEDED',
            message: options.message || 'Too many communication requests. Please try again later.',
            retryAfter: Math.ceil(options.windowMs / 1000),
        },
        standardHeaders: true,
        legacyHeaders: false,
        skipSuccessfulRequests: options.skipSuccessfulRequests || false,
        keyGenerator: options.keyGenerator || ((req: AuthRequest) => {
    ...
    return (req: AuthRequest, res: Response, next: NextFunction) => {
        ...
        if (exceeded) {
            ...
            return res.status(429).json({
                success: false,
                code: `USER_${limitType.toUpperCase().replace(' ', '_')}_RATE_LIMIT_EXCEEDED`,
 ...
 */
export const burstProtection = (req: AuthRequest, res: Response, next: NextFunction) => {
    if (!req.user) {
        return next();
    }
    ...
    userBurstData.count++;
    const burstLimit = req.user.role === 'patient' ? 3 : 5; // Lower limit for patients
 ...
 */
export const spamDetection = (req: AuthRequest, res: Response, next: NextFunction) => {
    if (!req.user) {
        return next();
    }
