...
const sessionSecurityStore = new Map<string, {
...
export const validateSession = async (
    ...
    try {
        ...
        if (security?.isLocked && security.lockExpires && Date.now() < security.lockExpires) {
            res.status(423).json({
                success: false,
                code: 'ACCOUNT_LOCKED',
        ...
        const validation = validateUserSession(req.user._id.toString(), sessionId, req);
 ...
 */
export const terminateSession = (userId: string, sessionId: string): boolean => {
    const userSessions = activeSessionsStore.get(userId);
