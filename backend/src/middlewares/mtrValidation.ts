...
import { MTRValidationError, MTRAuthorizationError, MTRBusinessLogicError } from '../utils/mtrErrors';
...
export const handleValidationErrors = (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
...
export const validateMedicationHistory = (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
        ...
        if (validationErrors.length > 0) {
            ...
            throw new MTRValidationError('Medication history validation failed', validationErrors);
        }
...
export const validateTherapyPlan = (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
        const { plan } = req.body;
        ...
        if (validationErrors.length > 0) {
            ...
            throw new MTRValidationError('Therapy plan validation failed', validationErrors);
        }
...
export const validateMTRAccess = (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
        const user = req.user;
        ...
        if (!user.role || !['pharmacist', 'pharmacy_team', 'pharmacy_outlet', 'super_admin'].includes(user.role)) {
            ...
            throw new MTRAuthorizationError('Pharmacist credentials required for MTR operations');
        }
...
export const validateMTRBusinessLogic = (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
        ...
        if (validationErrors.length > 0) {
            throw new MTRBusinessLogicError('MTR business logic validation failed', validationErrors);
        }
