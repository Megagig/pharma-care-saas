 ...
 */
export const generateCSRFToken = (userId: string, sessionId?: string): string => {
 ...
 * Validate CSRF token
 */
export const validateCSRFToken = (userId: string, token: string, sessionId?: string): boolean => {
    const key = `${userId}_${sessionId || 'default'}`;
 ...
 */
export const requireCSRFToken = (req: AuthRequest, res: Response, next: NextFunction): void => {
    try {
        ...
        const isValid = validateCSRFToken(req.user._id.toString(), csrfToken as string, sessionId);
        ...
        if (!isValid) {
            logger.warn('Invalid CSRF token', {
                ...
                ip: req.ip,
                userAgent: req.get('User-Agent'),
                providedToken: typeof csrfToken === 'string' ? csrfToken.substring(0, 8) + '...' : 'invalid',
                service: 'communication-csrf',
            });
 ...
 */
export const doubleSubmitCSRF = (req: AuthRequest, res: Response, next: NextFunction): void => {
    try {
 ...
 */
export const enforceSameSite = (req: Request, res: Response, next: NextFunction): void => {
    try {
 ...
 */
export const lightweightCSRFProtection = [
    validateOrigin,
    doubleSubmitCSRF,
];
