   ...
   try {
      ...
      await loadWorkspaceContext(req, res, async () => {
         ...
         if (
            ...
            subscriptionValidation.blockAccess
         ) {
            res.status(402).json({
               success: false,
               message: subscriptionValidation.message,
               subscriptionStatus:
                  req.workspaceContext?.subscription?.status || 'none',
...
export const authWithWorkspaceOptionalSubscription = async (
   ...
   try {
      ...
      if (error instanceof jwt.TokenExpiredError) {
         ...
         logger.error(
            'Enhanced auth optional subscription middleware error:',
            error
         );
         res.status(401).json({
            success: false,
...
async function validateUserStatus(user: IUser): Promise<{
   ...
   if (
      ...
      return {
         valid: false,
         message:
            'License verification was rejected. Please resubmit valid license information.',
...
async function validateSubscriptionStatus(req: AuthRequest): Promise<{
   ...
   if (context.isTrialExpired && !context.isSubscriptionActive) {
      return {
         ...
         message:
            'Trial period has expired. Please upgrade to continue using the service.',
      };
   }
