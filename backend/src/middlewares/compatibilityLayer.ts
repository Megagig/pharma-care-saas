      ...
      if (user.workplaceId) {
         ...
         if (workplace && workplace.currentSubscriptionId) {
            subscription = await Subscription.findById(
               workplace.currentSubscriptionId
            ).populate('planId');
         ...
         logger.info(
            'Compatibility auth - User has no workspace, checking user subscription',
            {
               userId: user._id,
            }
         );
...
export const migrationStatusChecker = (
   ...
   if (req.user) {
      const migrationStatus = {
         ...
         needsMigration:
            !req.user.workplaceId || !req.workplace?.currentSubscriptionId,
      };
