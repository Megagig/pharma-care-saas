 ...
 */
const sanitizeContent = (content: string, config: SanitizationOptions): string => {
 ...
 */
export const sanitizeMessageContent = (req: AuthRequest, res: Response, next: NextFunction): void => {
    try {
        if (req.body.content) {
            ...
            const allowedTypes = ['text', 'file', 'image', 'clinical_note', 'voice_note'];
            if (req.body.content.type && !allowedTypes.includes(req.body.content.type)) {
                res.status(400).json({
                    success: false,
            ...
            if (req.body.content.attachments && Array.isArray(req.body.content.attachments)) {
                req.body.content.attachments = req.body.content.attachments.map((attachment: any) => ({
                    ...attachment,
 ...
 */
export const sanitizeConversationData = (req: AuthRequest, res: Response, next: NextFunction): void => {
    try {
        ...
        const allowedTypes = ['direct', 'group', 'patient_query', 'clinical_consultation'];
        ...
        if (req.body.tags && Array.isArray(req.body.tags)) {
            req.body.tags = req.body.tags
                .map((tag: string) => sanitizeContent(tag, { maxLength: 50, stripHtml: true }))
        ...
        // Validate case ID
        if (req.body.caseId) {
            req.body.caseId = sanitizeContent(req.body.caseId, { maxLength: 100, stripHtml: true });
        }
 ...
 */
export const sanitizeSearchQuery = (req: AuthRequest, res: Response, next: NextFunction): void => {
    try {
        ...
        const stringParams = ['conversationId', 'senderId', 'participantId', 'fileType'];
        stringParams.forEach(param => {
            if (req.query[param]) {
                req.query[param] = sanitizeContent(req.query[param] as string, sanitizationConfigs.search);
            }
        });
        ...
        if (req.query.tags) {
            if (Array.isArray(req.query.tags)) {
                req.query.tags = req.query.tags.map((tag: string) =>
                    sanitizeContent(tag, { maxLength: 50, stripHtml: true })
                );
            } else {
                req.query.tags = sanitizeContent(req.query.tags as string, { maxLength: 50, stripHtml: true });
            }
        }
 ...
 */
export const validateFileUpload = (req: AuthRequest, res: Response, next: NextFunction): void => {
    try {
 ...
 */
export const preventNoSQLInjection = (req: AuthRequest, res: Response, next: NextFunction): void => {
    try {
 ...
 */
export const setCommunicationCSP = (req: Request, res: Response, next: NextFunction): void => {
 ...
 */
export const validateEmojiReaction = (req: AuthRequest, res: Response, next: NextFunction): void => {
    try {
 ...
 */
export const validateCommunicationInput = (req: AuthRequest, res: Response, next: NextFunction): void => {
    try {
