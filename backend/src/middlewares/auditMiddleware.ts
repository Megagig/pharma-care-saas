      ...
      res.on('finish', async () => {
         try {
            ...
            const complianceCategory =
               req.auditData?.complianceCategory || options.complianceCategory;
            ...
            const auditData = {
               ...
               patientId: req.auditData?.patientId
                  ? new mongoose.Types.ObjectId(req.auditData.patientId)
                  : req.params.patientId
                    ? new mongoose.Types.ObjectId(req.params.patientId)
                    : undefined,
               reviewId: req.auditData?.reviewId
                  ? new mongoose.Types.ObjectId(req.auditData.reviewId)
                  : req.params.id
               ...
               errorMessage:
                  statusCode >= 400 ? getErrorMessage(responseData) : undefined,
               duration,
               complianceCategory: complianceCategory as
