      ...
      try {
         ...
         if (!req.workspaceContext) {
            res.status(500).json({
               success: false,
               message:
                  'Workspace context not loaded. Ensure authWithWorkspace middleware is used.',
            });
            return;
         }
         ...
         if (usageResult.isAtWarning) {
            ...
            res.setHeader(
               'X-Usage-Current',
               usageResult.currentUsage.toString()
            );
            res.setHeader(
               'X-Usage-Limit',
               (usageResult.limit || 'unlimited').toString()
            );
            res.setHeader(
               'X-Usage-Percentage',
               Math.round(
                  (usageResult.currentUsage / (usageResult.limit || 1)) * 100
               ).toString()
            );
         }
 ...
 */
export const updateUsageAfterCreation = (
   resource: string,
   delta: number = 1
) => {
   return async (
      ...
      res.json = function (body: any) {
         ...
         if (
            ...
            WorkspaceStatsService.updateUsageStats({
               ...
               logger.error(
                  `Failed to update usage stats for ${resource}:`,
                  error
               );
            });
         }
 ...
 */
export const updateUsageAfterDeletion = (
   resource: string,
   delta: number = 1
) => {
   return async (
...
export const attachUsageStats = async (
   ...
   try {
      if (req.workspaceContext?.workspace && req.workspaceContext?.limits) {
         ...
         const warningResources = Object.values(usageStats).filter(
            (stat) => stat.isAtWarning
         ).length;
         const limitedResources = Object.values(usageStats).filter(
            (stat) => stat.isAtLimit
         ).length;
