...
setInterval(() => {
    ...
    if (initialLength !== auditLogs.length) {
        logger.info(`Cleaned up ${initialLength - auditLogs.length} old audit logs`);
    }
 ...
 */
export const createAuditLog = async (logData: Partial<SystemAuditLog>): Promise<void> => {
    try {
    ...
    return async (req: AuthRequest, res: Response, next: NextFunction) => {
        ...
        res.on('finish', async () => {
            ...
            await createAuditLog({
                ...
                resourceId: req.params.id ? new mongoose.Types.ObjectId(req.params.id) : undefined,
                details: {
...
export const auditOperations = {
    ...
    invitationAccepted: async (req: AuthRequest, invitation: any, newUser: any) => {
        await createAuditLog({
    ...
    // Subscription events
    subscriptionChanged: async (req: AuthRequest, oldSubscription: any, newSubscription: any) => {
    ...
    permissionDenied: async (req: AuthRequest, requiredPermission: string, reason: string) => {
        await createAuditLog({
    ...
    // Data access events
    dataAccess: async (req: AuthRequest, resourceType: string, resourceId: string, action: string) => {
        await createAuditLog({
...
function determineSeverity(statusCode: number, category: SystemAuditLog['category']): SystemAuditLog['severity'] {
...
function detectSuspiciousRequest(req: AuthRequest, res: Response): boolean {
    ...
    const suspiciousPatterns = [
        ...
        req.get('User-Agent')?.includes('bot') || req.get('User-Agent')?.includes('crawler'), // Bot activity
    ];
...
async function detectSuspiciousActivity(auditLog: SystemAuditLog): Promise<void> {
    ...
    const failedAttempts = recentLogs.filter(log =>
        log.errorMessage && log.category === 'authentication'
    ).length;

    if (failedAttempts > 5) {
        logger.warn('Suspicious activity: Multiple failed authentication attempts', {
            userId: auditLog.userId,
            ipAddress: auditLog.ipAddress,
            failedAttempts,
