...
/**
 * RBAC middleware that checks if user has any of the specified system roles
 */
...
export const requireWorkspaceOwner = (
   ...
   const isOwner =
      req.workspaceContext.workspace.ownerId.toString() ===
      ...
      try {
         ...
         if (!hasAnyPermission) {
            res.status(403).json({
               success: false,
               message:
                  'Insufficient permissions - requires any of the specified actions',
               requiredActions: actions,
            });
            return;
         }
...
export const requireSubscriptionOrTrial = (
   ...
   const isTrialActive =
      req.workspaceContext.workspace?.subscriptionStatus === 'trial' &&
