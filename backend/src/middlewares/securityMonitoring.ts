 ...
 */
export const monitorSuspiciousUsers = (
   req: AuthRequest,
   res: Response,
   next: NextFunction
) => {
   ...
   const suspiciousScore = securityMonitoringService.getUserSuspiciousScore(
...
export const adaptiveRateLimit = (baseLimit: number) => {
   return (req: AuthRequest, res: Response, next: NextFunction) => {
      ...
      const adjustedLimit = Math.max(
         1,
         Math.floor(baseLimit * (1 - suspiciousScore / 10))
      );
 ...
 */
export const monitorPermissionChanges = (
   req: AuthRequest,
   res: Response,
   next: NextFunction
) => {
