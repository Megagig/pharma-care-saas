...
export class PlanSeeder {
   ...
   private async upsertPlan(planConfig: PlanConfig): Promise<void> {
      try {
         ...
         if (result) {
            logger.info(
               `Upserted plan: ${planConfig.name} (${planConfig.tier})`
            );
         }
      } catch (error: any) {
   ...
   private mapConfigToPlanModel(
      ...
      return {
         ...
         features: this.mapFeaturesToModel(
            planConfig.features,
            planConfig.limits
         ),
      };
    ...
    */
   private async deactivateObsoletePlans(
      currentPlans: PlanConfig[]
   ): Promise<void> {
      try {
         ...
         if (obsoletePlans.length > 0) {
            logger.info(
               `Found ${obsoletePlans.length} obsolete plans to deactivate`
            );
   ...
   public async getSeedingStats(): Promise<{
      ...
      try {
         ...
         const activePlansInDatabase = await SubscriptionPlan.countDocuments({
            isActive: true,
         });
