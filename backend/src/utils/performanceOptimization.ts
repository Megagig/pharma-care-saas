...
export class CacheManager {
      ...
      try {
         ...
         if (!value) {
            ...
            if (compressedValue) {
               const zlib = require('zlib');
               value = zlib
                  .gunzipSync(Buffer.from(compressedValue, 'base64'))
                  .toString();
            }
         }
   ...
   static async invalidateInterventionCaches(
      ...
      try {
         ...
         if (interventionId) {
            patterns.push(
               this.generateKey('intervention', interventionId, '*')
            );
         }

         if (patientId) {
            patterns.push(
               this.generateKey('patient_interventions', patientId, '*')
            );
         ...
         if (workplaceId) {
            ...
            patterns.push(
               this.generateKey('interventions_list', '*', workplaceId)
            );
         }
...
export class QueryOptimizer {
    ...
    */
   static optimizeInterventionQuery(
      baseQuery: any,
      options: {
    ...
    */
   static createDashboardAggregation(
      workplaceId: string,
    ...
    */
   static createUserAssignmentsQuery(
      userId: string,
      workplaceId: string,
      status?: string[]
   ) {
      ...
      return [
         ...
         },
         {
            $lookup: {
               ...
               foreignField: '_id',
               as: 'patient',
               pipeline: [{ $project: { firstName: 1, lastName: 1, mrn: 1 } }],
            },
         },
         { $unwind: '$patient' },
         {
...
export class MemoryOptimizer {
   ...
   static optimizeInterventionData(intervention: any): any {
      ...
      if (optimized.assignments) {
         optimized.assignments = optimized.assignments.map(
            (assignment: any) => ({
               _id: assignment._id,
      ...
      if (
         optimized.issueDescription &&
         optimized.issueDescription.length > 100
      ) {
         optimized.issueDescriptionPreview =
            optimized.issueDescription.substring(0, 100) + '...';
      }
