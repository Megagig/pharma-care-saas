...
export class FeatureFlagManager {
   ...
   static async initializeDefaultFlags(): Promise<void> {
      ...
      for (const flagData of defaultFlags) {
         try {
            ...
            logger.error(
               `Failed to initialize feature flag ${flagData.name}:`,
               error
            );
         }
      }
   ...
   static async createFlag(
      ...
      try {
         ...
         logger.info(
            `Feature flag ${flagData.name} created by ${flagData.createdBy}`
         );
         return flagObj;
      } catch (error) {
   ...
   static async isEnabled(
      ...
      try {
         ...
         if (
            ...
            flag.conditions.environment.length > 0
         ) {
         ...
         if (
            flag.conditions?.workplaceIds &&
            flag.conditions.workplaceIds.length > 0
         ) {
            if (
               !context.workplaceId ||
               !flag.conditions.workplaceIds.includes(context.workplaceId)
            ) {
               return {
                  flagName,
                  enabled: false,
         ...
         if (
            ...
            flag.conditions.userRoles.length > 0
         ) {
            if (
               !context.userRole ||
         ...
         if (
            flag.conditions?.subscriptionPlans &&
            flag.conditions.subscriptionPlans.length > 0
         ) {
            if (
               !context.subscriptionPlan ||
               !flag.conditions.subscriptionPlans.includes(
                  context.subscriptionPlan
               )
            ) {
               return {
         ...
         if (flag.rolloutPercentage < 100) {
            const hash = this.hashString(
               flagName + (context.workplaceId || context.userId || '')
            );
   ...
   static async refreshCache(): Promise<void> {
      try {
         ...
         logger.debug(
            `Feature flag cache refreshed with ${flags.length} flags`
         );
      } catch (error) {
...
export const featureFlagMiddleware = async (req: any, res: any, next: any) => {
   try {
      ...
      req.featureFlags = await FeatureFlagManager.evaluateFlags(
         commonFlags,
         context
      );
      req.evaluateFlag = (flagName: string) =>
         FeatureFlagManager.isEnabled(flagName, context);
