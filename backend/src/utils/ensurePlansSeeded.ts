...
export async function ensurePlansSeeded(): Promise<void> {
   try {
      ...
      if (planCount === 0) {
         logger.info(
            'No subscription plans found in database, seeding from configuration...'
         );
         ...
         if (stats.totalPlansInConfig > stats.activePlansInDatabase) {
            logger.warn(
               `Configuration has ${stats.totalPlansInConfig} plans but database has only ${stats.activePlansInDatabase} active plans`
            );
            logger.info(
               'Consider running plan seeding to sync configuration with database'
            );
         }
      }
   } catch (error) {
...
export async function getPlanSeedingStatus(): Promise<{
   ...
   try {
      ...
      const needsSeeding =
         stats.totalPlansInConfig > stats.activePlansInDatabase;
