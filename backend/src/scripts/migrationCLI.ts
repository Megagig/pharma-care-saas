   ...
   .action(async () => {
      try {
         ...
         if (status.estimatedCompletion) {
            console.log(
               `Estimated Completion: ${status.estimatedCompletion.toLocaleString()}`
            );
         }
         ...
         console.log(
            `Users Remaining: ${metrics.totalUsers - metrics.migratedUsers}`
         );
         console.log(`Total Workspaces: ${metrics.totalWorkspaces}`);
         console.log(
            `Workspaces with Subscriptions: ${metrics.workspacesWithSubscriptions}`
         );
         console.log(`Legacy User Subscriptions: ${metrics.userSubscriptions}`);
         console.log(
            `Workspace Subscriptions: ${metrics.workspaceSubscriptions}`
         );
      ...
      try {
         ...
         if (result.issues.length > 0) {
            ...
            result.issues.forEach((issue) => {
               console.log(
                  `${getIssueEmoji(issue.type)} ${issue.category}: ${issue.description}`
               );
               ...
               if (options.detailed && issue.affectedIds.length > 0) {
                  console.log(
                     `   Affected IDs: ${issue.affectedIds.slice(0, 5).join(', ')}${issue.affectedIds.length > 5 ? '...' : ''}`
                  );
               }
               console.log('');
            });
         ...
         if (result.warnings.length > 0) {
            ...
            result.warnings.forEach((warning) => {
               console.log(
                  `${getWarningEmoji(warning.impact)} ${warning.type}: ${warning.description}`
               );
               console.log(
                  `   Count: ${warning.count}, Impact: ${warning.impact}`
               );
            });
         }
         ...
         console.log(
            `Users with Workspace: ${result.stats.usersWithWorkspace}`
         );
         console.log(
            `Users without Workspace: ${result.stats.usersWithoutWorkspace}`
         );
         console.log(`Total Workspaces: ${result.stats.totalWorkspaces}`);
         console.log(
            `Workspaces with Subscription: ${result.stats.workspacesWithSubscription}`
         );
         console.log(
            `Legacy User Subscriptions: ${result.stats.userSubscriptions}`
         );
         console.log(
            `Workspace Subscriptions: ${result.stats.workspaceSubscriptions}`
         );
         console.log(
            `Data Consistency Score: ${result.stats.dataConsistencyScore}/100`
         );
      } catch (error) {
...
program
   ...
   .action(async (options: any) => {
      try {
         ...
         console.log(
            `Backup: ${options.backup !== false ? 'Enabled' : 'Disabled'}`
         );
         console.log(
            `Progress Tracking: ${options.progress !== false ? 'Enabled' : 'Disabled'}`
         );
         console.log(
            `Integrity Checks: ${options.integrityChecks !== false ? 'Enabled' : 'Disabled'}`
         );
         console.log('');
         ...
         if (result.results.migration) {
            ...
            console.log(
               `Subscriptions Migrated: ${migration.subscriptionsMigrated}`
            );
         ...
         if (result.results.validation) {
            ...
            console.log(
               `Validation Score: ${validation.stats.dataConsistencyScore}/100`
            );
         ...
         if (result.integrityCheck) {
            console.log(
               `Orphaned Records: ${result.integrityCheck.orphanedUsers + result.integrityCheck.orphanedSubscriptions}`
            );
         }
...
program
   ...
   .action(async (options: any) => {
      try {
         ...
         console.log(
            '⚠️  This will revert workspace-based subscriptions to user-based subscriptions.'
         );
         console.log('');
         ...
         if (result.results.rollback) {
            ...
            console.log(
               `Subscriptions Rolled Back: ${rollback.subscriptionsMigrated}`
            );
      ...
      try {
         ...
         const monitor = async () => {
            try {
               ...
               console.log(
                  `Progress: ${'█'.repeat(Math.floor(status.progress / 5))}${'░'.repeat(20 - Math.floor(status.progress / 5))} ${status.progress}%`
               );
               console.log(`Validation Score: ${status.validationScore}/100`);
               console.log(`Critical Issues: ${status.criticalIssues}`);
               console.log('');
               console.log(
                  `Users: ${metrics.migratedUsers}/${metrics.totalUsers} migrated`
               );
               console.log(
                  `Workspaces: ${metrics.workspacesWithSubscriptions}/${metrics.totalWorkspaces} with subscriptions`
               );
               console.log(
                  `Subscriptions: ${metrics.workspaceSubscriptions} workspace, ${metrics.userSubscriptions} legacy`
               );
...
program
   ...
   .option(
      '--type <type>',
      'Report type (daily, weekly, on_demand)',
      'on_demand'
   )
   ...
   .action(async (options: any) => {
      try {
         ...
         if (options.output) {
            ...
            console.log(
               `Migration Progress: ${report.metrics.migrationProgress}%`
            );
            console.log(
               `Validation Score: ${report.metrics.validationScore}/100`
            );
