...
export async function migrateToWorkspaceSubscriptions(): Promise<MigrationResult> {
    ...
    try {
        ...
        logger.info(`Found ${stats.usersWithoutWorkspace} users without workspace associations`);
        ...
        for (const user of usersWithoutWorkspace) {
            try {
                ...
                logger.info(`Created workspace for user ${user.email}: ${workspace._id}`);

                // Step 3: Migrate user subscription to workspace subscription if exists
                if (user.currentSubscriptionId) {
                    const userSubscription = await Subscription.findById(user.currentSubscriptionId);
                    if (userSubscription) {
                        ...
                        logger.info(`Migrated subscription for user ${user.email}: ${userSubscription._id} -> ${workspaceSubscription._id}`);
                    }
                }
 ...
 */
function mapSubscriptionStatus(oldStatus: string): 'trial' | 'active' | 'past_due' | 'expired' | 'canceled' {
    switch (oldStatus) {
        case 'trial':
            return 'trial';
        case 'active':
    ...
    try {
        ...
        for (const subscription of workspaceSubscriptions) {
            try {
                ...
                stats.subscriptionsMigrated++;
                logger.info(`Rolled back subscription for workspace ${workspace._id}`);
        ...
        try {
            switch (command) {
                ...
                case 'validate':
                    ...
                    break;
                default:
                    logger.info('Usage: npm run migrate:workspace [migrate|rollback|validate]');
            }
        } catch (error) {
