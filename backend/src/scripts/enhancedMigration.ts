...
export class EnhancedMigrationOrchestrator {
    ...
    constructor(options: Partial<EnhancedMigrationOptions> = {}) {
        ...
        this.progressTracker = new MigrationProgressTracker('workspace_subscription');
        ...
        try {
            ...
            if (this.options.enableIntegrityChecks) {
                ...
                if (!consistencyCheck.isConsistent) {
                    logger.error('Data consistency issues detected after migration:', consistencyCheck.inconsistencies);
                }
            }
        ...
        try {
            switch (command) {
                ...
                case 'validate':
                    ...
                    break;
                default:
                    logger.info('Usage: npm run migrate:enhanced [migrate|rollback|dry-run|validate] [options]');
                    logger.info('Options:');
                    logger.info('  --dry-run              Run without making changes');
                    logger.info('  --batch-size <number>  Set batch size for processing');
                    ...
                    logger.info('  --continue-on-error   Continue processing on errors');
            }
        } catch (error) {
