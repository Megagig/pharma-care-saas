...
async function testMTRAuditIntegration() {
   ...
   try {
      ...
      if (auditLogs.length > 0) {
         ...
         console.log(
            '   Compliance Category:',
            auditLogs[0]?.complianceCategory
         );
      } else {
      ...
      const allAuditLogs = await MTRAuditLog.find({
         workplaceId: workplace._id,
      });
      ...
      for (const log of allAuditLogs) {
         console.log(
            `   - ${log.action} (${log.riskLevel} risk, ${log.complianceCategory})`
         );
         ...
         const missingFields = requiredFields.filter(
            (field) => !log[field as keyof typeof log]
         );
      ...
      if (sampleLog) {
         ...
         console.log(
            `   Compliance Category Display: ${sampleLog.complianceCategoryDisplay}`
         );
      }
