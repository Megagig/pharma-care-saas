...
class MigrationIntegrityValidator {
         ...
         if (!mongoose.connection.readyState) {
            await mongoose.connect(
               process.env.MONGODB_URI ||
                  'mongodb://localhost:27017/pharma-care-test'
            );
         }

         // Run validation checks
         const workspaceValidation =
            await this.validateWorkspaceSubscriptions();
         const subscriptionValidation =
            await this.validateSubscriptionReferences();
   ...
   private async validateWorkspaceSubscriptions() {
      ...
      for (const workspace of workspaces) {
         if (workspace.currentSubscriptionId) {
            ...
            if (!subscription) {
               this.errors.push(
                  `Workspace ${workspace._id} references non-existent subscription ${workspace.currentSubscriptionId}`
               );
            } else if (
               subscription.workspaceId.toString() !== workspace._id.toString()
            ) {
               this.errors.push(
                  `Workspace ${workspace._id} subscription ${subscription._id} has mismatched workspaceId`
               );
            }
         } else {
            this.errors.push(
               `Workspace ${workspace._id} (${workspace.name}) has no subscription`
            );
         }
      }
   ...
   private async validateSubscriptionReferences() {
      ...
      for (const subscription of subscriptions) {
         ...
         if (!workspace) {
            this.errors.push(
               `Subscription ${subscription._id} references non-existent workspace ${subscription.workspaceId}`
            );
            orphaned++;
         }
   ...
   private async validateUserWorkspaceReferences() {
      ...
      for (const user of users) {
         if (user.workplaceId) {
            ...
            if (!workspace) {
               this.errors.push(
                  `User ${user._id} (${user.email}) references non-existent workspace ${user.workplaceId}`
               );
               invalid++;
            } else {
               ...
               if (!isTeamMember) {
                  this.warnings.push(
                     `User ${user._id} (${user.email}) is not in workspace ${workspace._id} team members`
                  );
               }
            }
         }
   ...
   private async validateSubscriptionPlans() {
      ...
      for (const plan of plans) {
         if (!plan.isActive) {
            this.warnings.push(
               `Plan ${plan._id} (${plan.name}) is inactive but may be referenced by subscriptions`
            );
         }
      }
