   ...
   async processUserMigration(
      ...
      try {
         ...
         this.progress.totalBatches = Math.ceil(
            totalUsers / this.options.batchSize
         );

         logger.info(
            `Starting batch migration for ${totalUsers} users in ${this.progress.totalBatches} batches`
         );
         ...
         while (skip < totalUsers) {
            ...
            logger.info(
               `Processing batch ${batchNumber}/${this.progress.totalBatches} (items ${skip + 1}-${Math.min(skip + this.options.batchSize, totalUsers)})`
            );
            ...
            const progressPercent = Math.round(
               (this.progress.processedItems / totalUsers) * 100
            );
            logger.info(
               `Batch ${batchNumber} completed. Progress: ${progressPercent}% (${this.progress.processedItems}/${totalUsers})`
            );
...
export class MigrationIntegrityChecker {
   ...
   async checkOrphanedRecords(): Promise<{
      ...
      try {
         ...
         if (orphanedUsers > 0) {
            issues.push(
               `Found ${orphanedUsers} users without workspace associations`
            );
         }
         ...
         if (orphanedSubscriptions > 0) {
            issues.push(
               `Found ${orphanedSubscriptions} old user-based subscriptions`
            );
         }
         ...
         if (workspacesWithoutOwners > 0) {
            issues.push(
               `Found ${workspacesWithoutOwners} workspaces without valid owners`
            );
         }
         ...
         if (invalidOwnerCount > 0) {
            issues.push(
               `Found ${invalidOwnerCount} workspaces with non-existent owners`
            );
         }
   ...
   async checkDataConsistency(): Promise<{
      ...
      try {
         ...
         const invalidSubscriptionCount =
            workspacesWithInvalidSubscriptions[0]?.count || 0;
         ...
         const subscriptionsWithInvalidWorkspaces =
         ...
         const subscriptionWorkspaceMismatch =
            subscriptionsWithInvalidWorkspaces[0]?.count || 0;
...
export class MigrationRollbackManager {
   ...
   getBackupStats(): {
      ...
      for (const [key, backup] of this.backupData.entries()) {
         ...
         backupsByCollection[collection] =
            (backupsByCollection[collection] || 0) + 1;
      }
