...
class MTRPerformanceOptimizer {
   ...
   async optimizeDTPQueries(): Promise<void> {
      ...
      for (const index of dtpIndexes) {
         try {
            ...
            console.log(
               `⚠️  DTP Index already exists: ${JSON.stringify(index)}`
            );
         }
      }
   }
   ...
   async optimizeInterventionQueries(): Promise<void> {
      ...
      for (const index of interventionIndexes) {
         try {
            ...
            console.log(
               `✅ Created intervention index: ${JSON.stringify(index)}`
            );
         } catch (error) {
            console.log(
               `⚠️  Intervention index already exists: ${JSON.stringify(index)}`
            );
         }
      }
      ...
      for (const index of followUpIndexes) {
         try {
            ...
            console.log(
               `⚠️  Follow-up index already exists: ${JSON.stringify(index)}`
            );
         }
      }
   }
   ...
   generateReport(): void {
      ...
      const avgQueryTime =
         this.metrics.reduce((sum, m) => sum + m.queryTime, 0) /
         this.metrics.length;
      const avgMemoryUsage =
         this.metrics.reduce((sum, m) => sum + m.memoryUsage, 0) /
         this.metrics.length;
      const totalDocuments = this.metrics.reduce(
      ...
      if (avgQueryTime > 1000) {
         console.log(
            '⚠️  Consider adding more specific indexes for slow queries'
         );
      }
      if (avgMemoryUsage > 50 * 1024 * 1024) {
         // 50MB
         console.log(
            '⚠️  Consider implementing result pagination for large datasets'
         );
      }
