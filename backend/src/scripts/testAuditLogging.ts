...
async function testAuditLogging() {
   ...
   try {
      ...
      const authLog = await AuditService.logAuthEvent(testContext, 'LOGIN', {
         loginMethod: 'email',
      ...
      const exportLogs = await MTRAuditLog.find({
         workplaceId: testWorkplaceId,
      })
         .select('-__v')
         .lean();
      ...
      const sampleLog = await MTRAuditLog.findOne({
         workplaceId: testWorkplaceId,
      });
      if (sampleLog) {
