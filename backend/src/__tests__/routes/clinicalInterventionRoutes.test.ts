...
jest.mock('../../controllers/clinicalInterventionController', () => ({
   getClinicalInterventions: jest.fn((req, res) =>
      res.json({ success: true, data: [] })
   ),
   getClinicalIntervention: jest.fn((req, res) =>
      res.json({ success: true, data: {} })
   ),
   createClinicalIntervention: jest.fn((req, res) =>
      res.status(201).json({ success: true, data: {} })
   ),
   updateClinicalIntervention: jest.fn((req, res) =>
      res.json({ success: true, data: {} })
   ),
   deleteClinicalIntervention: jest.fn((req, res) =>
      res.json({ success: true })
   ),
   addInterventionStrategy: jest.fn((req, res) =>
      res.json({ success: true, data: {} })
   ),
   updateInterventionStrategy: jest.fn((req, res) =>
      res.json({ success: true, data: {} })
   ),
   assignTeamMember: jest.fn((req, res) =>
      res.json({ success: true, data: {} })
   ),
   updateAssignment: jest.fn((req, res) =>
      res.json({ success: true, data: {} })
   ),
   recordOutcome: jest.fn((req, res) => res.json({ success: true, data: {} })),
   scheduleFollowUp: jest.fn((req, res) =>
      res.json({ success: true, data: {} })
   ),
   searchClinicalInterventions: jest.fn((req, res) =>
      res.json({ success: true, data: [] })
   ),
   getPatientInterventions: jest.fn((req, res) =>
      res.json({ success: true, data: [] })
   ),
   getAssignedInterventions: jest.fn((req, res) =>
      res.json({ success: true, data: [] })
   ),
   getInterventionAnalytics: jest.fn((req, res) =>
      res.json({ success: true, data: {} })
   ),
   getInterventionTrends: jest.fn((req, res) =>
      res.json({ success: true, data: {} })
   ),
   getOutcomeReports: jest.fn((req, res) =>
      res.json({ success: true, data: {} })
   ),
   exportInterventionData: jest.fn((req, res) =>
      res.json({ success: true, data: {} })
   ),
   getStrategyRecommendations: jest.fn((req, res) =>
      res.json({ success: true, data: [] })
   ),
   linkToMTR: jest.fn((req, res) => res.json({ success: true })),
   sendInterventionNotifications: jest.fn((req, res) =>
      res.json({ success: true })
   ),
}));
...
describe('Clinical Intervention Routes', () => {
   ...
   describe('GET /api/clinical-interventions', () => {
      ...
      it('should accept valid query parameters', async () => {
         const response = await request(app)
            .get(
               '/api/clinical-interventions?page=1&limit=10&category=drug_therapy_problem&priority=high'
            )
            .set('Authorization', `Bearer ${authToken}`)
            .expect(200);
         ...
         strategies: [
            {
               ...
               expectedOutcome:
                  'Reduced side effects and improved medication adherence',
            },
         ],
      };
   ...
   describe('POST /api/clinical-interventions/:id/strategies', () => {
      const validStrategyData = {
         ...
         expectedOutcome:
            'Reduced side effects while maintaining therapeutic effect',
         priority: 'primary',
      };
   ...
   describe('Analytics and Reporting Routes', () => {
      ...
      it('should validate analytics query parameters', async () => {
         const response = await request(app)
            .get(
               '/api/clinical-interventions/analytics/summary?dateFrom=invalid-date'
            )
            .set('Authorization', `Bearer ${authToken}`)
            .expect(400);
   ...
   describe('Integration Routes', () => {
      it('should return strategy recommendations', async () => {
         const response = await request(app)
            .get(
               '/api/clinical-interventions/recommendations/drug_therapy_problem'
            )
            .set('Authorization', `Bearer ${authToken}`)
            .expect(200);
