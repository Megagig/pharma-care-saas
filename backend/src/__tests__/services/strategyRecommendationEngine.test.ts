...
describe('Strategy Recommendation Engine', () => {
   describe('getRecommendedStrategies', () => {
      ...
      it('should return strategies for adverse drug reaction category', () => {
         ...
         const discontinuation = strategies.find(
            (s) => s.type === 'discontinuation'
         );
      ...
      it('should return strategies for medication nonadherence category', () => {
         ...
         const counseling = strategies.find(
            (s) => s.type === 'patient_counseling'
         );
      ...
      it('should return strategies for dosing issue category', () => {
         ...
         const doseAdjustment = strategies.find(
            (s) => s.type === 'dose_adjustment'
         );
      ...
      it('should sort strategies by priority (primary first)', () => {
         ...
         const primaryStrategies = strategies.filter(
         ...
         if (primaryStrategies.length > 0 && secondaryStrategies.length > 0) {
            const firstPrimaryIndex = strategies.findIndex(
               (s) => s.priority === 'primary'
            );
            const firstSecondaryIndex = strategies.findIndex(
               (s) => s.priority === 'secondary'
            );
   ...
   describe('getAllStrategies', () => {
      ...
      it('should sort strategies alphabetically by label', () => {
         ...
         for (let i = 1; i < allStrategies.length; i++) {
            ...
            if (current && previous) {
               expect(
                  current.label.localeCompare(previous.label)
   ...
   describe('getStrategiesForCategories', () => {
      it('should return strategies applicable to multiple categories', () => {
         ...
         const strategies =
            ClinicalInterventionService.getStrategiesForCategories(categories);
   ...
   describe('validateCustomStrategy', () => {
      it('should validate correct custom strategy', () => {
         const validStrategy = {
            ...
            expectedOutcome:
               'This is a valid expected outcome that is long enough',
            priority: 'primary' as const,
         };

         const result =
            ClinicalInterventionService.validateCustomStrategy(validStrategy);
      ...
      it('should reject strategy with wrong type', () => {
         ...
         const result =
            ClinicalInterventionService.validateCustomStrategy(invalidStrategy);

         expect(result.isValid).toBe(false);
         expect(result.errors).toContain(
            'Custom strategy must have type "custom"'
         );
      });
      ...
      it('should reject strategy with short description', () => {
         ...
         expect(result.isValid).toBe(false);
         expect(result.errors).toContain(
            'Strategy description must be at least 10 characters'
         );
      });
      ...
      it('should reject strategy with short rationale', () => {
         ...
         expect(result.isValid).toBe(false);
         expect(result.errors).toContain(
            'Strategy rationale must be at least 10 characters'
         );
      });
      ...
      it('should reject strategy with too long fields', () => {
         ...
         expect(result.isValid).toBe(false);
         expect(result.errors).toContain(
            'Strategy description cannot exceed 500 characters'
         );
         expect(result.errors).toContain(
            'Strategy rationale cannot exceed 500 characters'
         );
         expect(result.errors).toContain(
            'Expected outcome cannot exceed 500 characters'
         );
      });
   ...
   describe('generateRecommendations', () => {
      ...
      it('should prioritize counseling for adherence issues', () => {
         ...
         const counselingRec = recommendations.find(
            (r) => r.type === 'patient_counseling'
         );
   ...
   describe('getStrategyByType', () => {
      it('should return strategy for valid type', () => {
         const strategy =
            ClinicalInterventionService.getStrategyByType('medication_review');
      ...
      it('should return null for invalid type', () => {
         const strategy =
            ClinicalInterventionService.getStrategyByType('invalid_type');
      ...
      it('should return strategy for all valid types', () => {
         ...
         validTypes.forEach((type) => {
            const strategy =
               ClinicalInterventionService.getStrategyByType(type);
            expect(strategy).toBeDefined();
