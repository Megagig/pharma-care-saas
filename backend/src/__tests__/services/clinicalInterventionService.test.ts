...
describe('ClinicalInterventionService', () => {
   ...
   describe('generateInterventionNumber', () => {
      it('should delegate to model static method', async () => {
         const ClinicalIntervention =
            require('../../models/ClinicalIntervention').default;
         ClinicalIntervention.generateNextInterventionNumber = jest
            .fn()
            .mockResolvedValue('CI-202412-0001');
         ...
         expect(
            ClinicalIntervention.generateNextInterventionNumber
         ).toHaveBeenCalledWith(mockWorkplaceId);
      });
   });
   ...
   describe('isValidStatusTransition', () => {
      ...
      const isValidStatusTransition = (ClinicalInterventionService as any)
      ...
      it('should allow valid status transitions', () => {
         ...
         expect(isValidStatusTransition('in_progress', 'implemented')).toBe(
            true
         );
      ...
      it('should reject invalid status transitions', () => {
         ...
         expect(isValidStatusTransition('completed', 'in_progress')).toBe(
            false
         );
         expect(isValidStatusTransition('cancelled', 'planning')).toBe(false);
         expect(isValidStatusTransition('cancelled', 'in_progress')).toBe(
            false
         );
         expect(isValidStatusTransition('identified', 'completed')).toBe(false);
         expect(isValidStatusTransition('identified', 'implemented')).toBe(
            false
         );
   ...
   describe('Service Integration', () => {
      it('should have all required static methods', () => {
         expect(typeof ClinicalInterventionService.createIntervention).toBe(
            'function'
         );
         expect(typeof ClinicalInterventionService.updateIntervention).toBe(
            'function'
         );
         expect(typeof ClinicalInterventionService.getInterventions).toBe(
            'function'
         );
         expect(typeof ClinicalInterventionService.getInterventionById).toBe(
            'function'
         );
         expect(typeof ClinicalInterventionService.deleteIntervention).toBe(
            'function'
         );
         expect(
            typeof ClinicalInterventionService.generateInterventionNumber
         ).toBe('function');
         expect(
            typeof ClinicalInterventionService.checkDuplicateInterventions
         ).toBe('function');
      });
