...
describe('ClinicalInterventionService - Comprehensive Tests', () => {
   ...
   describe('createIntervention', () => {
      ...
      it('should create intervention with valid data', async () => {
         const intervention =
            await ClinicalInterventionService.createIntervention(
         ...
         expect(intervention.patientId.toString()).toBe(
            testPatientId.toString()
         );
         expect(intervention.identifiedBy.toString()).toBe(
            testUserId.toString()
         );
         expect(intervention.workplaceId.toString()).toBe(
            ...
            strategies: [
               {
                  ...
                  rationale:
                     'Patient experiencing dose-related adverse effects',
                  expectedOutcome:
                     'Reduction in side effects while maintaining therapeutic efficacy',
      ...
      it('should throw error for non-existent patient', async () => {
         ...
         await expect(
            ClinicalInterventionService.createIntervention(invalidData)
      ...
      it('should generate unique intervention numbers', async () => {
         ...
         expect(intervention1.interventionNumber).not.toBe(
            intervention2.interventionNumber
         );
      ...
      it('should handle related MTR and DTP IDs', async () => {
         ...
         expect(intervention.relatedMTRId?.toString()).toBe(
            relatedMTRId.toString()
         );
         expect(intervention.relatedDTPIds).toHaveLength(2);
         expect(intervention.relatedDTPIds[0]?.toString()).toBe(
            relatedDTPIds[0]?.toString()
         );
      });
   });
   ...
   describe('updateIntervention', () => {
      ...
      beforeEach(async () => {
         testIntervention = await ClinicalIntervention.create({
            ...
            priority: 'medium',
            issueDescription:
               'Initial issue description that meets minimum length requirements',
            identifiedBy: testUserId,
            createdBy: testUserId,
            status: 'identified',
         });
      ...
      it('should update intervention with valid data', async () => {
         const updates: UpdateInterventionDTO = {
            priority: 'high',
            issueDescription:
               'Updated issue description with more detailed information about the problem',
            implementationNotes:
               'Added implementation notes for tracking progress',
         };

         const updatedIntervention =
            await ClinicalInterventionService.updateIntervention(
         ...
         expect(updatedIntervention.issueDescription).toBe(
            updates.issueDescription
         );
         expect(updatedIntervention.implementationNotes).toBe(
            updates.implementationNotes
         );
         expect(updatedIntervention.updatedBy?.toString()).toBe(
            testUserId.toString()
         );
      });
         ...
         await ClinicalIntervention.create([
            {
               ...
               priority: 'high',
               issueDescription:
                  'Test issue 1 that meets minimum length requirements for validation',
               identifiedBy: testUserId,
               createdBy: testUserId,
               status: 'in_progress',
            ...
            },
            {
               ...
               priority: 'critical',
               issueDescription:
                  'Test issue 2 that meets minimum length requirements for validation',
               identifiedBy: testUserId,
               createdBy: testUserId,
               status: 'completed',
            ...
            },
            {
               ...
               priority: 'medium',
               issueDescription:
                  'Test issue 3 that meets minimum length requirements for validation',
               identifiedBy: testUserId,
               createdBy: testUserId,
               status: 'planning',
   ...
   describe('getInterventionById', () => {
      ...
      beforeEach(async () => {
         testIntervention = await ClinicalIntervention.create({
            ...
            priority: 'high',
            issueDescription:
               'Test issue that meets minimum length requirements for validation',
            identifiedBy: testUserId,
            createdBy: testUserId,
            strategies: [
               {
      ...
      it('should get intervention with populated fields', async () => {
         ...
         expect(intervention).toBeDefined();
         expect(intervention._id.toString()).toBe(
            testIntervention._id.toString()
         );
   ...
   describe('deleteIntervention', () => {
      ...
      it('should soft delete intervention', async () => {
         ...
         // Verify soft delete
         const deletedIntervention = await ClinicalIntervention.findById(
            testIntervention._id
         );
         expect(deletedIntervention?.isDeleted).toBe(true);
         expect(deletedIntervention?.updatedBy?.toString()).toBe(
            testUserId.toString()
         );
      });
   ...
   describe('checkDuplicateInterventions', () => {
      beforeEach(async () => {
         ...
         await ClinicalIntervention.create({
            ...
            priority: 'high',
            issueDescription:
               'Existing intervention that meets minimum length requirements',
            identifiedBy: testUserId,
            createdBy: testUserId,
            status: 'in_progress',
      ...
      it('should only find interventions within 30 days', async () => {
         ...
         await ClinicalIntervention.create({
            ...
            priority: 'high',
            issueDescription:
               'Old intervention that meets minimum length requirements',
            identifiedBy: testUserId,
            createdBy: testUserId,
            status: 'in_progress',
         ...
         await ClinicalIntervention.create([
            {
               ...
               priority: 'high',
               issueDescription:
                  'Test issue 1 that meets minimum length requirements',
               identifiedBy: testUserId,
               createdBy: testUserId,
               status: 'completed',
               outcomes: {
            ...
            },
            {
               ...
               priority: 'critical',
               issueDescription:
                  'Test issue 2 that meets minimum length requirements',
               identifiedBy: testUserId,
               createdBy: testUserId,
               status: 'in_progress',
            },
            {
               ...
               priority: 'medium',
               issueDescription:
                  'Test issue 3 that meets minimum length requirements',
               identifiedBy: testUserId,
               createdBy: testUserId,
               status: 'completed',
               outcomes: {
   ...
   describe('generateInterventionNumber', () => {
      it('should generate intervention number in correct format', async () => {
         const interventionNumber =
            await ClinicalInterventionService.generateInterventionNumber(
               testWorkplaceId
            );
   ...
   describe('Error Handling', () => {
      it('should handle database connection errors gracefully', async () => {
         ...
         await expect(
            ClinicalInterventionService.generateInterventionNumber(
               testWorkplaceId
            )
         ).rejects.toThrow();
