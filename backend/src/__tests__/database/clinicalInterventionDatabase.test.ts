...
describe('Clinical Intervention Database Operations', () => {
   ...
   describe('CRUD Operations', () => {
      it('should create intervention with all fields', async () => {
         ...
         const intervention =
            await ClinicalIntervention.create(interventionData);
      ...
      it('should read intervention with populated fields', async () => {
         const intervention = await ClinicalIntervention.create({
            ...
            priority: 'high',
            issueDescription:
               'Test intervention for reading with populated fields',
            identifiedBy: testUserId,
            createdBy: testUserId,
         });

         const foundIntervention = await ClinicalIntervention.findById(
            intervention._id
         )
      ...
      it('should update intervention fields', async () => {
         const intervention = await ClinicalIntervention.create({
            ...
            priority: 'medium',
            issueDescription:
               'Original issue description that meets minimum length requirements',
            identifiedBy: testUserId,
            createdBy: testUserId,
         });
      ...
      it('should soft delete intervention', async () => {
         ...
         // Soft delete
         await ClinicalIntervention.findByIdAndUpdate(intervention._id, {
            isDeleted: true,
            updatedBy: testUserId,
         });
         ...
         const activeInterventions =
            await ClinicalIntervention.find().setOptions({
               workplaceId: testWorkplaceId,
            });
   ...
   describe('Query Operations', () => {
      ...
      it('should sort interventions', async () => {
         ...
         expect(interventions).toHaveLength(3);
         expect(
            interventions[0]?.identifiedDate.getTime()
         ).toBeGreaterThanOrEqual(
            interventions[1]?.identifiedDate.getTime() || 0
         );
         expect(
            interventions[1]?.identifiedDate.getTime()
         ).toBeGreaterThanOrEqual(
      ...
      it('should count interventions', async () => {
         const totalCount =
            await ClinicalIntervention.countDocuments().setOptions({
               workplaceId: testWorkplaceId,
            });
            ...
            },
            {
               $group: {
                  ...
                  medicationsOptimized: {
                     $sum: {
                        $cond: [
                           '$outcomes.successMetrics.medicationOptimized',
                           1,
                           0,
                        ],
                     },
   ...
   describe('Static Methods', () => {
      ...
      it('should find active interventions', async () => {
         const activeInterventions = await (
            ClinicalIntervention as any
         ).findActive(testWorkplaceId);
         ...
         activeInterventions.forEach((intervention: IClinicalIntervention) => {
            expect([
               ...
               'in_progress',
               'implemented',
            ]).toContain(intervention.status);
         });
      });

      it('should find overdue interventions', async () => {
         const overdueInterventions = await (
            ClinicalIntervention as any
         ).findOverdue(testWorkplaceId);
      ...
      it('should find interventions by patient', async () => {
         const patientInterventions = await (
            ClinicalIntervention as any
         ).findByPatient(testPatientId, testWorkplaceId);
      ...
      it('should generate next intervention number', async () => {
         const nextNumber = await (
            ClinicalIntervention as any
         ).generateNextInterventionNumber(testWorkplaceId);
   ...
   describe('Tenancy and Security', () => {
      ...
      it('should enforce tenancy filtering', async () => {
         ...
         expect(interventions).toHaveLength(1);
         expect(interventions[0]?.workplaceId.toString()).toBe(
   ...
   describe('Data Validation', () => {
      ...
      it('should validate intervention number format', async () => {
         ...
         await expect(invalidIntervention.save()).rejects.toThrow(
            /Invalid intervention number format/
         );
      });
