...
class ClinicalInterventionTestRunner {
   ...
   async runAllTests(): Promise<void> {
      ...
      const testSuites = [
         ...
         },
         {
            ...
            pattern:
               '__tests__/controllers/clinicalInterventionController*.test.ts',
         },
         {
            name: 'Unit Tests - Validators',
            pattern:
               '__tests__/validators/clinicalInterventionValidators*.test.ts',
         },
         {
            name: 'Unit Tests - Middlewares',
            pattern:
               '__tests__/middlewares/clinicalInterventionErrorHandler*.test.ts',
         },
         {
            name: 'Integration Tests',
            pattern:
               '__tests__/integration/clinicalInterventionIntegration*.test.ts',
         },
         {
            name: 'Database Tests',
         ...
         },
         {
            ...
            pattern:
               '__tests__/performance/clinicalInterventionPerformance*.test.ts',
         },
      ];
   ...
   private async runTestSuite(
      ...
      try {
         ...
         const failedTests = this.parseJestOutput(
   ...
   private generateCoverageSummary(): void {
      ...
      try {
         ...
         const summaryLine = coverageLines.find((line) =>
   ...
   private generateQualityMetrics(): void {
      ...
      if (avgTestDuration > 5000) {
         ...
         console.log(
            '🚀 Excellent: Tests are running very fast (<100ms average)'
         );
      } else {
   ...
   async runWithWatch(): Promise<void> {
      ...
      try {
         const command =
            'npx jest --testPathPattern="__tests__.*clinicalIntervention.*" --watch';
         execSync(command, {
            stdio: 'inherit',
            cwd: process.cwd(),
         });
