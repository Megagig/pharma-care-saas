...
jest.mock('../../utils/responseHelpers', () => ({
   ...
   respondWithPaginatedResults: jest.fn(
      (res, data, total, page, limit, message) => {
         res.json({
            success: true,
            data,
...
describe('MTR Controller', () => {
   ...
   describe('GET /api/mtr/:id', () => {
      ...
      it('should return 404 for non-existent MTR session', async () => {
         ...
         await request(app).get(`/api/mtr/${nonExistentId}`).expect(404);
      });
   });
   ...
   describe('POST /api/mtr', () => {
      it('should create new MTR session', async () => {
         ...
         expect(response.body.success).toBe(true);
         expect(response.body.data.session.patientId).toBe(
            patientId.toString()
         );
         expect(response.body.data.session.reviewNumber).toMatch(
            /^MTR-\d{6}-\d{4}$/
         );
      });
      ...
      it('should fail without patient consent', async () => {
         ...
         await request(app).post('/api/mtr').send(mtrData).expect(400);
      });
   ...
   describe('DELETE /api/mtr/:id', () => {
      ...
      it('should soft delete MTR session', async () => {
         ...
         // Verify soft delete
         const deletedSession = await MedicationTherapyReview.findById(
            mtrSession._id
         );
      ...
      it('should not allow deleting completed sessions by non-admin', async () => {
         ...
         await request(app).delete(`/api/mtr/${mtrSession._id}`).expect(403);
      });
   });
   ...
   describe('PUT /api/mtr/:id/step/:stepName', () => {
      ...
      it('should update workflow step', async () => {
         ...
         expect(response.body.success).toBe(true);
         expect(
            response.body.data.session.steps.medicationHistory.completed
         ).toBe(true);
      });
         ...
         it('should create new problem', async () => {
            ...
            const problemData = {
               ...
               clinicalSignificance:
                  'Increased bleeding risk due to additive effects',
               evidenceLevel: 'definite',
   ...
   describe('Interventions', () => {
      ...
      describe('GET /api/mtr/:id/interventions', () => {
         it('should get interventions for MTR session', async () => {
            ...
            app.get(
               '/api/mtr/:id/interventions',
               mtrController.getMTRInterventions
            );
      ...
      describe('POST /api/mtr/:id/interventions', () => {
         it('should create new intervention', async () => {
            app.post(
               '/api/mtr/:id/interventions',
               mtrController.createMTRIntervention
            );
            ...
            const interventionData = {
               ...
               category: 'medication_change',
               description:
                  'Recommend discontinuing aspirin due to bleeding risk',
               rationale:
                  'Patient has high bleeding risk with concurrent warfarin therapy',
               targetAudience: 'prescriber',
               communicationMethod: 'phone',
               documentation:
                  'Called Dr. Smith to discuss discontinuing aspirin',
            };
   ...
   describe('Patient-specific operations', () => {
      describe('GET /api/mtr/patient/:patientId', () => {
         ...
         it('should get patient MTR history', async () => {
            app.get(
               '/api/mtr/patient/:patientId',
               mtrController.getPatientMTRHistory
            );
      ...
      describe('POST /api/mtr/patient/:patientId', () => {
         it('should create MTR session for specific patient', async () => {
            app.post(
               '/api/mtr/patient/:patientId',
               mtrController.createPatientMTRSession
            );
