...
const mockMtrNotificationService = mtrNotificationService as jest.Mocked<
   typeof mtrNotificationService
>;
...
describe('MTR Notification Controller', () => {
   ...
   describe('POST /follow-up/:followUpId/reminder', () => {
      it('should schedule follow-up reminder successfully', async () => {
         ...
         expect(
            mockMtrNotificationService.scheduleFollowUpReminder
   ...
   describe('POST /alert/critical', () => {
      it('should send critical alert successfully', async () => {
         ...
         expect(
            mockMtrNotificationService.sendCriticalAlert
         ).toHaveBeenCalledWith(
   ...
   describe('POST /check-overdue', () => {
      it('should check overdue follow-ups successfully', async () => {
         ...
         const response = await request(app).post(
            '/api/mtr/notifications/check-overdue'
         );
         ...
         expect(
            mockMtrNotificationService.checkOverdueFollowUps
         ).toHaveBeenCalled();
      });
      ...
      it('should handle service errors', async () => {
         ...
         expect(response.body.success).toBe(false);
         expect(response.body.error.message).toBe(
            'Failed to check overdue follow-ups'
         );
      });
   });
   ...
   describe('GET /preferences', () => {
      it('should get notification preferences successfully', async () => {
         ...
         const response = await request(app).get(
            '/api/mtr/notifications/preferences'
         );
   ...
   describe('GET /statistics', () => {
      it('should get notification statistics successfully', async () => {
         ...
         mockMtrNotificationService.getNotificationStatistics.mockResolvedValue(
            mockStats
         );

         const response = await request(app).get(
            '/api/mtr/notifications/statistics'
         );
         ...
         expect(
            mockMtrNotificationService.getNotificationStatistics
         ).toHaveBeenCalledWith(expect.any(mongoose.Types.ObjectId));
      });
   });
   ...
   describe('GET /follow-up/:followUpId/reminders', () => {
      it('should get follow-up reminders successfully', async () => {
         ...
         const response = await request(app).get(
            `/api/mtr/notifications/follow-up/${testFollowUpId}/reminders`
         );
      ...
      it('should return 400 for invalid follow-up ID', async () => {
         const response = await request(app).get(
            '/api/mtr/notifications/follow-up/invalid-id/reminders'
         );
   ...
   describe('POST /process-pending', () => {
      it('should process pending reminders successfully', async () => {
         ...
         const response = await request(app).post(
            '/api/mtr/notifications/process-pending'
         );
         ...
         expect(
            mockMtrNotificationService.processPendingReminders
         ).toHaveBeenCalled();
      });
   });
});
