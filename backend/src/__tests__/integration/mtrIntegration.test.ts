...
describe('MTR Integration Tests', () => {
   ...
   describe('Complete MTR Workflow', () => {
      it('should complete full MTR workflow from creation to completion', async () => {
         ...
         const createResponse = await request(app).post('/api/mtr').send({
         ...
         updatedMtr!.plan = {
            ...
            recommendations: [
               {
                  ...
                  medication: 'Aspirin',
                  rationale:
                     'High bleeding risk with concurrent warfarin therapy',
                  priority: 'high',
                  expectedOutcome:
                     'Reduced bleeding risk while maintaining anticoagulation',
               },
            ],
            monitoringPlan: [
               {
            ...
            counselingPoints: [
               'Discuss bleeding precautions',
               'Review signs of bleeding',
            ],
            goals: [
               {
                  description:
                     'Maintain therapeutic anticoagulation without bleeding complications',
            ...
            timeline: '2 weeks',
            pharmacistNotes:
               'Patient counseled on bleeding risks and aspirin discontinuation',
         };
         ...
         const followUpResponse = await request(app)
            ...
            .send({
               type: 'phone_call',
               description:
                  'Follow-up call to assess bleeding status and INR results after aspirin discontinuation',
      ...
      it('should handle drug interaction checking workflow', async () => {
         ...
         expect(
            interactionResponse.body.data.interactions.length
         ).toBeGreaterThan(0);
      ...
      it('should handle MTR reports and analytics', async () => {
         ...
         expect(
            effectivenessResponse.body.data.interventionAcceptanceRate
         ).toBeGreaterThan(0);
      });

      it('should handle error scenarios gracefully', async () => {
         // Test creating MTR without patient consent
         const response1 = await request(app).post('/api/mtr').send({
            patientId: patient._id.toString(),
         ...
         const response2 = await request(app).get(`/api/mtr/${nonExistentId}`);
         ...
         await request(app).post(`/api/mtr/patient/${patient._id}`).send({
            patientConsent: true,
            confidentialityAgreed: true,
         });
