...
describe('Migration Workflow Integration Tests', () => {
   ...
   beforeEach(async () => {
      ...
      legacyWorkplaces[0].teamMembers = [
         legacyUsers[0]._id,
         legacyUsers[1]._id,
      ];
   ...
   describe('Workspace Subscription Migration', () => {
      it('should migrate all workspaces to have subscriptions', async () => {
         ...
         const postWorkspaces = await Workplace.find({}).populate(
            'subscriptionId'
         );
         ...
         for (const workspace of postWorkspaces) {
            ...
            expect(subscription.workspaceId.toString()).toBe(
      ...
      it('should migrate with intelligent plan assignment based on usage', async () => {
         ...
         legacyWorkplaces[0].teamMembers.push(
         ...
         expect(migrationResult).toContain(
            'Intelligent plan assignment completed'
         );
         ...
         const workspace1 = workspaces.find(
            (w) => w._id.toString() === legacyWorkplaces[0]._id.toString()
         );
         const workspace2 = workspaces.find(
            (w) => w._id.toString() === legacyWorkplaces[1]._id.toString()
         );
         ...
         const trialDays1 = Math.ceil(
            (subscription1.trialEndsAt.getTime() -
               subscription1.startDate.getTime()) /
               (1000 * 60 * 60 * 24)
         );
      ...
      it('should handle migration rollback correctly', async () => {
         ...
         expect(rolledBackWorkspaces.every((w) => !w.subscriptionId)).toBe(
            true
         );
   ...
   describe('Data Integrity Validation', () => {
      it('should validate data integrity after migration', async () => {
         ...
         expect(validationResult).toContain(
            'All workspaces have valid subscriptions'
         );
         expect(validationResult).toContain(
            'All subscriptions have valid plans'
         );
         expect(validationResult).toContain(
            'All users are properly associated'
         );
         ...
         for (const user of users) {
            const workspace = workspaces.find(
               (w) => w._id.toString() === user.workplaceId.toString()
            );
            expect(workspace).toBeTruthy();
            expect(workspace!.teamMembers.map((id) => id.toString())).toContain(
               user._id.toString()
            );
         }
      });
      ...
      it('should detect and report data inconsistencies', async () => {
         ...
         // Run validation script
         try {
            execSync('npm run validate:migration-integrity', {
               cwd: process.cwd() + '/backend',
               encoding: 'utf8',
               env: { ...process.env, NODE_ENV: 'test' },
            });
            ...
            expect(error.stdout).toContain(
               'Invalid subscription references found'
            );
   ...
   describe('Migration Error Handling', () => {
      it('should handle migration errors gracefully and provide recovery options', async () => {
         ...
         const invalidWorkspaceAfter = await Workplace.findById(
            invalidWorkspace._id
         );
      ...
      it('should provide detailed error reporting and recovery suggestions', async () => {
         ...
         try {
            ...
            expect(error.stdout).toContain(
               'Run with --continue-on-error to skip failed records'
            );
            expect(error.stdout).toContain(
