...
describe('Subscription Workflow Integration Tests', () => {
   ...
   describe('Subscription Upgrade Workflow', () => {
      it('should upgrade from basic to premium plan successfully', async () => {
         ...
         const oldSubscription = await SubscriptionModel.findById(
         ...
         const newSubscription = await SubscriptionModel.findById(
            upgradeResponse.body.subscription._id
         );
         expect(newSubscription!.planId.toString()).toBe(
         ...
         expect(updatedWorkspace!.subscriptionId.toString()).toBe(
            newSubscription!._id.toString()
         );

         // Step 7: Verify upgrade notification email sent
         expect(
            mockEmailService.sendSubscriptionStatusChange
         ...
         expect(featureCheckResponse.body.features).toContain(
            'team_management'
         );
         expect(featureCheckResponse.body.features).toContain(
            'inventory_management'
      ...
      it('should handle upgrade with proration calculation', async () => {
         ...
         const expectedProration = Math.round(
            (priceDifference * daysRemaining) / 30
         );
   ...
   describe('Subscription Downgrade Workflow', () => {
      ...
      it('should downgrade from premium to basic plan successfully', async () => {
         ...
         expect(downgradeResponse.body.currentSubscription.status).toBe(
            'active'
         );
         ...
         const currentSub = await SubscriptionModel.findById(
            currentSubscription._id
         );
      ...
      it('should prevent immediate downgrade if usage exceeds new plan limits', async () => {
         ...
         expect(downgradeResponse.body.error).toBe(
   ...
   describe('Trial to Paid Conversion', () => {
      beforeEach(async () => {
         ...
         currentSubscription.trialEndsAt = new Date(
      ...
      it('should convert trial to paid subscription successfully', async () => {
         ...
         const newSubscription = await SubscriptionModel.findById(
            conversionResponse.body.subscription._id
         );
      ...
      it('should handle trial expiration and paywall mode', async () => {
         ...
         expect(featureResponse.body.availableFeatures).toEqual([
            'basic_access',
         ]);
         ...
         expect(patientResponse.body.error).toBe(
   ...
   describe('Subscription Cancellation', () => {
      ...
      it('should handle immediate cancellation', async () => {
         ...
         expect(subscription!.endDate.getTime()).toBeLessThanOrEqual(
            Date.now()
         );
      });
   });
   ...
   describe('Billing and Payment', () => {
      it('should handle failed payment and retry logic', async () => {
         ...
         expect(
            mockEmailService.sendPaymentFailedNotification
         ).toHaveBeenCalledWith(
            ownerUser.email,
            expect.objectContaining({
               workspaceName: workspace.name,
      ...
      it('should suspend subscription after multiple payment failures', async () => {
         ...
         expect(
            mockEmailService.sendSubscriptionSuspended
         ).toHaveBeenCalledWith(
            ownerUser.email,
            expect.objectContaining({
               workspaceName: workspace.name,
