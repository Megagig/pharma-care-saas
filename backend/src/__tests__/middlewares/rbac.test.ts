...
const mockSubscriptionPlan = SubscriptionPlanModel as jest.Mocked<
   typeof SubscriptionPlanModel
>;
...
describe('rbacMiddleware', () => {
   ...
   describe('requireFeature', () => {
      it('should allow access when plan includes required feature', async () => {
         ...
         expect(
            mockPermissionServiceInstance.hasFeatureAccess
         ).toHaveBeenCalledWith(mockPlanData, 'patient_management');
         expect(mockNext).toHaveBeenCalled();
         expect(mockResponse.status).not.toHaveBeenCalled();
      });
      ...
      it('should deny access when plan lacks required feature', async () => {
         ...
         mockPermissionServiceInstance.hasFeatureAccess.mockResolvedValue(
            false
         );
   ...
   describe('attachUserPermissions', () => {
      it('should attach user permissions to request', async () => {
         ...
         mockPermissionServiceInstance.resolveUserPermissions.mockResolvedValue(
            mockPermissions
         );
         ...
         expect(
            mockPermissionServiceInstance.resolveUserPermissions
         ).toHaveBeenCalledWith(mockUser, mockWorkspaceData, mockPlanData);
         expect(mockNext).toHaveBeenCalled();
      });
   ...
   describe('checkMultiplePermissions', () => {
      it('should allow access when user has all required permissions', async () => {
         const middleware = rbacMiddleware.checkMultiplePermissions([
            'patient.create',
            'patient.read',
         ]);
   ...
   describe('role hierarchy', () => {
      ...
      it('should correctly determine role hierarchy', () => {
         expect(
            rbacMiddleware.hasRequiredRole('Owner', 'Technician', roleHierarchy)
         ).toBe(true);
         expect(
            rbacMiddleware.hasRequiredRole(
               'Pharmacist',
               'Technician',
               roleHierarchy
            )
         ...
         expect(
            rbacMiddleware.hasRequiredRole('Technician', 'Owner', roleHierarchy)
         ).toBe(false);
         expect(
            rbacMiddleware.hasRequiredRole('Pharmacist', 'Owner', roleHierarchy)
         ).toBe(false);
      });
