...
describe('Plan Upgrade and Trial Expiration Journey E2E Tests', () => {
   ...
   beforeEach(async () => {
      ...
      // Mock email service
      mockEmailService.sendSubscriptionUpgrade.mockResolvedValue({
         success: true,
         messageId: 'test-id',
      });
      mockEmailService.sendSubscriptionDowngrade.mockResolvedValue({
         success: true,
         messageId: 'test-id',
      });
      mockEmailService.sendTrialExpiryWarning.mockResolvedValue({
         provider: 'test',
         success: true,
         messageId: 'test-id',
      });
      mockEmailService.sendTrialExpired.mockResolvedValue({
         success: true,
         messageId: 'test-id',
      });
   });
   ...
   describe('Trial to Paid Plan Upgrade Journey', () => {
      it('should complete full upgrade journey: trial usage → upgrade decision → payment → immediate access', async () => {
         ...
         expect(usageResponse.body.warnings).toContain(
         ...
         expect(recommendationsResponse.body.reasons).toContain(
            'Patient limit nearly reached'
         );
         ...
         const premiumPlanOption = availablePlans.find(
            (p: any) => p.code === 'premium'
         );
         expect(premiumPlanOption.upgradeHighlights).toContain(
            '500 patient limit'
         );
         expect(premiumPlanOption.upgradeHighlights).toContain(
            'Team management'
         ...
         expect(updatedWorkspace!.currentSubscriptionId!.toString()).toBe(
            newSubscriptionId
         );
         ...
         expect(mockEmailService.sendSubscriptionUpgrade).toHaveBeenCalledWith(
            ...
            expect.objectContaining({
               ...
               newFeatures: expect.arrayContaining([
                  'inventory_management',
                  'advanced_reports',
               ]),
         ...
         expect(featuresResponse.body.features).toContain(
            'inventory_management'
         );
      ...
      it('should handle upgrade with team member invitation capability', async () => {
         ...
         expect(inviteResponse.body.invitation.email).toBe(
   ...
   describe('Trial Expiration and Paywall Mode Journey', () => {
      it('should handle trial expiration with grace period and paywall activation', async () => {
         ...
         trialSubscription.trialEndsAt = new Date(
         ...
         expect(dashboardResponse.body.availableFeatures).toEqual([
            'basic_access',
            'view_data',
         ]);
         expect(dashboardResponse.body.blockedFeatures).toContain(
            'patient_management'
         );
         ...
         expect(blockedPatientResponse.body.error).toBe(
         ...
         expect(upgradeFromExpiredResponse.body.subscription.status).toBe(
            'active'
         );
         expect(upgradeFromExpiredResponse.body.subscription.isTrial).toBe(
            false
         );
      ...
      it('should handle grace period expiration and account suspension', async () => {
         ...
         const expiredBeyondGrace = new Date(
         ...
         expect(suspendedResponse.body.error).toBe(
         ...
         expect(restoredDashboardResponse.body.workspace.name).toBe(
            workspace.name
         );
         expect(restoredDashboardResponse.body.subscription.status).toBe(
            'active'
         );
      });
   });
   ...
   describe('Plan Downgrade Journey', () => {
      ...
      it('should handle downgrade with usage validation and scheduling', async () => {
         ...
         expect(blockedDowngradeResponse.body.error).toBe(
         ...
         expect(scheduledDowngradeResponse.body.scheduledChange.type).toBe(
            'downgrade'
         );
         ...
         expect(
            mockEmailService.sendSubscriptionDowngrade
            ...
            expect.objectContaining({
               ...
               newPlan: 'Basic Plan',
               effectiveDate: expect.any(String),
               removedFeatures: expect.arrayContaining([
                  'inventory_management',
                  'advanced_reports',
               ]),
            })
