...
describe('User Registration Journey E2E Tests', () => {
   ...
   beforeEach(async () => {
      ...
      // Mock email service
      mockEmailService.sendTrialActivation.mockResolvedValue({
         success: true,
   ...
   describe('New User Registration with Workspace Creation', () => {
      it('should complete full registration journey: signup → workspace creation → trial activation', async () => {
         ...
         expect(
            workspace!.teamMembers.map((id: any) => id.toString())
         ).toContain(userId);
         ...
         const trialDays = Math.ceil(
            (subscription!.trialEndDate!.getTime() -
         ...
         expect(mockEmailService.sendTrialActivation).toHaveBeenCalledWith(
            ...
            expect.objectContaining({
               ...
               trialDuration: 14,
               trialEndsAt: expect.any(String),
               planFeatures: expect.arrayContaining([
                  'patient_management',
                  'basic_reports',
               ]),
         ...
         expect(dashboardResponse.body.availableFeatures).toContain(
            'patient_management'
         );
   ...
   describe('Post-Registration User Experience', () => {
      ...
      it('should provide onboarding guidance for new users', async () => {
         ...
         expect(onboardingResponse.body.nextSteps).toContain(
            'complete_profile'
         );
         expect(onboardingResponse.body.nextSteps).toContain(
            'add_first_patient'
      ...
      it('should track onboarding progress', async () => {
         ...
         expect(statusResponse.body.completedSteps).toContain(
            'complete_profile'
         );
         expect(statusResponse.body.nextSteps).not.toContain(
            'complete_profile'
         );
      ...
      it('should provide trial usage guidance and warnings', async () => {
         ...
         expect(usageResponse.body.recommendations).toContain(
      ...
      it('should provide upgrade recommendations based on usage patterns', async () => {
         ...
         expect(recommendationsResponse.body.benefits).toContain(
            '100 patient limit'
         );
         expect(recommendationsResponse.body.benefits).toContain(
