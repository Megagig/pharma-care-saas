...
describe('Team Invitation Journey E2E Tests', () => {
   ...
   beforeEach(async () => {
      ...
      mockEmailService.sendInvitationAcceptedNotification.mockResolvedValue({
         success: true,
         messageId: 'test-id',
         provider: 'test',
      });
   ...
   describe('Complete Team Member Invitation and Onboarding Journey', () => {
      it('should complete full team invitation journey: invite → accept → onboard → collaborate', async () => {
         ...
         expect(
            updatedWorkspace!.teamMembers.map((id) => id.toString())
         ).toContain(newUserId);
         expect(updatedWorkspace!.teamMembers).toHaveLength(2);

         // Verify acceptance notification emails
         expect(
            mockEmailService.sendInvitationAcceptedNotification
         ...
         expect(onboardingResponse.body.steps).toContain(
            'first_patient_interaction'
         );
         ...
         expect(patientResponse.body.patient.workplaceId).toBe(
         ...
         expect(unauthorizedInviteResponse.body.error).toBe(
            'Insufficient permissions to create invitations'
         );
      });
      ...
      it('should handle invitation to technician role with appropriate permissions', async () => {
         ...
         expect(permissionsResponse.body.permissions).toContain(
            'patient.create'
         );
         expect(permissionsResponse.body.permissions).toContain('patient.read');
         expect(permissionsResponse.body.permissions).not.toContain(
            'patient.delete'
         );
         expect(permissionsResponse.body.permissions).not.toContain(
            'invitation.create'
      ...
      it('should prevent invitation when user limit would be exceeded', async () => {
         ...
         expect(response.body.error).toBe(
            'User limit exceeded for current plan'
         );
      ...
      it('should handle invitation expiry and cleanup', async () => {
         ...
         const expiredInvitation = await Invitation.findOne({
            code: invitationCode,
         });
   ...
   describe('Team Management and Collaboration', () => {
      ...
      it('should enable team collaboration on patient care', async () => {
         ...
         expect(noteResponse.body.note.createdBy).toBe(
      ...
      it('should handle team member role changes', async () => {
         ...
         expect(newPermissionsResponse.body.permissions).toContain(
            'patient.delete'
         );
      });
      ...
      it('should handle team member removal', async () => {
         ...
         expect(
            updatedWorkspace!.teamMembers.map((id) => id.toString())
         ).not.toContain(technicianUser._id.toString());
