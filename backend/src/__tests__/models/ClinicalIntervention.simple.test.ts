...
describe('ClinicalIntervention Model - Simple Test', () => {
   ...
   it('should have the correct schema structure', () => {
      ...
      expect(
         schema.paths.outcomes || schema.paths['outcomes.patientResponse']
      ).toBeDefined();
      expect(
         schema.paths.followUp || schema.paths['followUp.required']
      ).toBeDefined();
   ...
   it('should have static methods defined', () => {
      expect(
         typeof (ClinicalIntervention as any).generateNextInterventionNumber
      ).toBe('function');
      ...
      expect(typeof (ClinicalIntervention as any).findByPatient).toBe(
         'function'
      );
      expect(typeof (ClinicalIntervention as any).findAssignedToUser).toBe(
         'function'
      );
   });
});
