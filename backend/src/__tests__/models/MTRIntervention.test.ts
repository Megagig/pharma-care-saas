...
import MTRIntervention, {
   IMTRIntervention,
} from '../../models/MTRIntervention';
...
describe('MTRIntervention Model', () => {
   ...
   describe('Model Creation', () => {
      it('should create a valid MTR intervention', async () => {
         const interventionData = {
            ...
            rationale:
               'Patient has high bleeding risk with concurrent warfarin therapy',
            targetAudience: 'prescriber',
            communicationMethod: 'phone',
            documentation:
               'Called Dr. Smith to discuss discontinuing aspirin. Agreed to stop medication.',
      ...
      it('should validate high priority interventions have detailed documentation', async () => {
         const interventionData = {
            ...
            priority: 'high',
            documentation:
               'This is a detailed documentation for high priority intervention that meets the minimum character requirement for proper validation and compliance with system requirements.',
            createdBy: pharmacistId,
         };
         ...
         await expect(intervention.save()).rejects.toThrow(
            'High priority interventions require detailed documentation'
         );
      });
   });
   ...
   describe('Instance Methods', () => {
      ...
      it('should mark intervention as completed', () => {
         intervention.markCompleted(
            'accepted',
            'Prescriber agreed to discontinue aspirin'
         );

         expect(intervention.outcome).toBe('accepted');
         expect(intervention.outcomeDetails).toBe(
            'Prescriber agreed to discontinue aspirin'
         );
      });
   ...
   describe('Static Methods', () => {
      beforeEach(async () => {
         ...
         const interventionData1 = {
            ...
            communicationMethod: 'phone',
            documentation:
               'Called prescriber to discuss high priority medication intervention and received confirmation of acceptance for the recommended changes to patient therapy plan.',
            outcome: 'accepted',
            priority: 'high',
            createdBy: pharmacistId,
         };
      ...
      it('should find interventions by review', async () => {
         const interventions = await (MTRIntervention as any).findByReview(
            reviewId,
            workplaceId
         );
      ...
      it('should find interventions by patient', async () => {
         const interventions = await (MTRIntervention as any).findByPatient(
            patientId,
            workplaceId
         );
      ...
      it('should find pending interventions', async () => {
         const pendingInterventions = await (
            MTRIntervention as any
         ).findPending(workplaceId);
      ...
      it('should find overdue follow-ups', async () => {
         ...
         const overdueFollowUps = await (
            MTRIntervention as any
         ).findOverdueFollowUps(workplaceId);
      ...
      it('should get intervention statistics', async () => {
         const stats = await (MTRIntervention as any).getStatistics(
            workplaceId
         );
   ...
   describe('Follow-up Date Validation', () => {
      it('should validate follow-up date is after intervention date', async () => {
         ...
         const followUpDate = new Date(
            performedAt.getTime() - 24 * 60 * 60 * 1000
         ); // Before performed date
         ...
         await expect(intervention.save()).rejects.toThrow(
            'Follow-up date must be after intervention date'
         );
      });
   });
});
