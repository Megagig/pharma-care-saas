...
describe('MTRFollowUp Model', () => {
   ...
   describe('Virtual Properties', () => {
      ...
      it('should calculate days since scheduled', () => {
         followUp.scheduledDate = new Date(
      ...
      it('should schedule default reminders', () => {
         ...
         followUp.reminders.forEach((reminder) => {
            expect(reminder.scheduledFor.getTime()).toBeLessThan(
               followUp.scheduledDate.getTime()
            );
         });
      });
   });
   ...
   describe('Pre-save Middleware', () => {
      ...
      it('should require outcome when status is completed', async () => {
         ...
         await expect(followUp.save()).rejects.toThrow(
            'Outcome is required when follow-up is completed'
         );
      });
   ...
   describe('Static Methods', () => {
      beforeEach(async () => {
         ...
         const followUps = await MTRFollowUp.create([
            followUpData1,
            followUpData2,
            followUpData3,
         ]);
      ...
      it('should find follow-ups by review', async () => {
         const followUps = await (MTRFollowUp as any).findByReview(
            reviewId,
            workplaceId
         );
      ...
      it('should find follow-ups by patient', async () => {
         const followUps = await (MTRFollowUp as any).findByPatient(
            patientId,
            workplaceId
         );
      ...
      it('should find scheduled follow-ups', async () => {
         const scheduledFollowUps = await (MTRFollowUp as any).findScheduled(
            workplaceId
         );
      ...
      it('should find overdue follow-ups', async () => {
         const overdueFollowUps = await (MTRFollowUp as any).findOverdue(
            workplaceId
         );

         expect(overdueFollowUps).toHaveLength(1);
         expect(overdueFollowUps[0].scheduledDate.getTime()).toBeLessThan(
            Date.now()
         );
      });

      it('should find follow-ups by assignee', async () => {
         const assignedFollowUps = await (MTRFollowUp as any).findByAssignee(
            assignedTo,
            workplaceId
         );
      ...
      it('should find pending reminders', async () => {
         ...
         const pendingReminders = await (
            MTRFollowUp as any
         ).findPendingReminders(workplaceId);
