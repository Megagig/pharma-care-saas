...
describe('DrugTherapyProblem Model', () => {
   ...
   describe('Model Creation', () => {
      ...
      it('should validate description length for critical severity', async () => {
         ...
         await expect(dtp.save()).rejects.toThrow(
            'critical severity DTPs require detailed description'
         );
      });
      ...
      it('should validate clinical significance for high evidence levels', async () => {
         const dtpData = {
            ...
            severity: 'major',
            description:
               'Drug interaction between warfarin and aspirin causing bleeding risk',
         ...
         await expect(dtp.save()).rejects.toThrow(
            'DTPs with definite evidence level require clinical significance explanation'
         );
      });
   });
   ...
   describe('Static Methods', () => {
      ...
      it('should find problems by patient', async () => {
         const problems = await (DrugTherapyProblem as any).findByPatient(
            patientId,
            undefined,
            workplaceId
         );
      ...
      it('should find problems by type', async () => {
         const problems = await (DrugTherapyProblem as any).findByType(
            'interaction',
            undefined,
            workplaceId
         );
      ...
      it('should find active problems', async () => {
         const activeProblems = await (DrugTherapyProblem as any).findActive(
            workplaceId
         );
      ...
      it('should find problems by review', async () => {
         const problems = await (DrugTherapyProblem as any).findByReview(
            reviewId,
            workplaceId
         );
      ...
      it('should get statistics', async () => {
         const stats = await (DrugTherapyProblem as any).getStatistics(
            workplaceId
         );
