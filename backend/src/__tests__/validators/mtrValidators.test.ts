...
describe('MTR Validators', () => {
   describe('createMTRSessionSchema', () => {
      ...
      it('should require patientId', async () => {
         ...
         expect(errors.some((error: any) => error.path === 'patientId')).toBe(
            true
         );
      });
      ...
      it('should validate ObjectId format for patientId', async () => {
         ...
         expect(
            errors.some(
               (error: any) =>
                  error.path === 'patientId' &&
                  error.msg === 'Invalid patient ID format'
            )
         ).toBe(true);
      });
      ...
      it('should validate priority enum values', async () => {
         ...
         const errors = result.array();
         expect(errors.some((error: any) => error.path === 'priority')).toBe(
            true
         );
      });
      ...
      it('should validate reviewType enum values', async () => {
         ...
         const errors = result.array();
         expect(errors.some((error: any) => error.path === 'reviewType')).toBe(
            true
         );
      });
      ...
      it('should require patient consent', async () => {
         ...
         expect(
            errors.some(
               (error: any) =>
                  error.path === 'patientConsent' &&
                  error.msg === 'Patient consent is required'
            )
         ).toBe(true);
      });
      ...
      it('should require confidentiality agreement', async () => {
         ...
         expect(
            errors.some(
               (error: any) =>
                  error.path === 'confidentialityAgreed' &&
                  error.msg === 'Confidentiality agreement is required'
            )
         ).toBe(true);
      });
      ...
      it('should validate string length limits', async () => {
         ...
         const errors = result.array();
         expect(
            errors.some((error: any) => error.path === 'referralSource')
         ).toBe(true);
         expect(
            errors.some((error: any) => error.path === 'reviewReason')
         ).toBe(true);
      });
   });
   ...
   describe('updateMTRSessionSchema', () => {
      ...
      it('should validate status enum values', async () => {
         ...
         const errors = result.array();
         expect(errors.some((error: any) => error.path === 'status')).toBe(
            true
         );
      });
      ...
      it('should validate date format', async () => {
         ...
         const errors = result.array();
         expect(
            errors.some((error: any) => error.path === 'nextReviewDate')
         ).toBe(true);
      });
      ...
      it('should validate estimated duration is positive', async () => {
         ...
         const errors = result.array();
         expect(
            errors.some((error: any) => error.path === 'estimatedDuration')
         ).toBe(true);
      });
   });
   ...
   describe('updateStepSchema', () => {
      ...
      it('should validate step name enum', async () => {
         ...
         const errors = result.array();
         expect(errors.some((error: any) => error.path === 'stepName')).toBe(
            true
         );
      });
      ...
      it('should require completed field to be boolean', async () => {
         ...
         const errors = result.array();
         expect(errors.some((error: any) => error.path === 'completed')).toBe(
            true
         );
      });
   });
   ...
   describe('medicationSchema', () => {
      ...
      it('should require medications to be an array', async () => {
         ...
         const errors = result.array();
         expect(errors.some((error: any) => error.path === 'medications')).toBe(
            true
         );
      });
      ...
      it('should validate required medication fields', async () => {
         ...
         expect(
            errors.some(
               (error: any) => error.path === 'medications[0].drugName'
            )
         ).toBe(true);
         expect(
            errors.some(
               (error: any) => error.path === 'medications[0].dosageForm'
            )
         ).toBe(true);
         expect(
            errors.some(
               (error: any) => error.path === 'medications[0].instructions.dose'
            )
         ).toBe(true);
      });
      ...
      it('should validate strength value is positive', async () => {
         ...
         expect(
            errors.some(
               (error: any) => error.path === 'medications[0].strength.value'
            )
         ).toBe(true);
      });
      ...
      it('should validate adherence score range', async () => {
         ...
         expect(
            errors.some(
               (error: any) => error.path === 'medications[0].adherenceScore'
            )
         ).toBe(true);
      });
   });
   ...
   describe('createProblemSchema', () => {
      it('should validate valid problem data', async () => {
         const req = createMockRequest({
            ...
            severity: 'major',
            description:
               'Drug interaction between warfarin and aspirin causing increased bleeding risk',
            clinicalSignificance:
               'Additive anticoagulant effects increase the risk of bleeding complications',
      ...
      it('should validate enum values', async () => {
         const req = createMockRequest({
            ...
            clinicalSignificance:
               'Valid clinical significance with sufficient length',
         ...
         const errors = result.array();
         expect(errors.some((error: any) => error.path === 'category')).toBe(
            true
         );
         expect(errors.some((error: any) => error.path === 'type')).toBe(true);
         expect(errors.some((error: any) => error.path === 'severity')).toBe(
            true
         );
         expect(
            errors.some((error: any) => error.path === 'evidenceLevel')
         ).toBe(true);
      });
      ...
      it('should validate description length', async () => {
         ...
         const errors = result.array();
         expect(errors.some((error: any) => error.path === 'description')).toBe(
            true
         );
      });
      ...
      it('should validate clinical significance length', async () => {
         ...
         const errors = result.array();
         expect(
            errors.some((error: any) => error.path === 'clinicalSignificance')
         ).toBe(true);
      });
   });
   ...
   describe('createInterventionSchema', () => {
      it('should validate valid intervention data', async () => {
         const req = createMockRequest({
            ...
            description:
               'Recommend discontinuing aspirin due to bleeding risk with concurrent warfarin therapy',
            rationale:
               'Patient has high bleeding risk with concurrent warfarin therapy and aspirin provides minimal additional benefit',
            targetAudience: 'prescriber',
            communicationMethod: 'phone',
            documentation:
               'Called Dr. Smith at 2:00 PM to discuss discontinuing aspirin. Dr. Smith agreed to stop aspirin and will reassess in 2 weeks.',
            priority: 'high',
            urgency: 'within_24h',
      ...
      it('should validate enum values', async () => {
         ...
         expect(
            errors.some((error: any) => error.path === 'targetAudience')
         ).toBe(true);
         expect(
            errors.some((error: any) => error.path === 'communicationMethod')
         ).toBe(true);
         expect(errors.some((error: any) => error.path === 'priority')).toBe(
            true
         );
         expect(errors.some((error: any) => error.path === 'urgency')).toBe(
            true
         );
      });
      ...
      it('should require description, rationale, and documentation', async () => {
         ...
         expect(errors.some((error: any) => error.path === 'rationale')).toBe(
            true
         );
         expect(
            errors.some((error: any) => error.path === 'documentation')
         ).toBe(true);
      });
   ...
   describe('createFollowUpSchema', () => {
      it('should validate valid follow-up data', async () => {
         ...
         const req = createMockRequest({
            type: 'phone_call',
            description:
               'Follow-up call to assess medication adherence and side effects',
      ...
      it('should validate scheduled date is in future', async () => {
         ...
         const errors = result.array();
         expect(
            errors.some((error: any) => error.path === 'scheduledDate')
         ).toBe(true);
      });
