    ...
    async collectValidationStats(): Promise<ValidationStats> {
        try {
            ...
            const consistencyFactors = [
                ...
                1 - (orphanedRecords / Math.max(totalUsers + totalWorkspaces + totalSubscriptions, 1)), // Low orphaned records
            ];

            const dataConsistencyScore = Math.round(
                consistencyFactors.reduce((sum, factor) => sum + factor, 0) / consistencyFactors.length * 100
            );
