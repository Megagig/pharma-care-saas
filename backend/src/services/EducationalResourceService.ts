  ...
  static async getResources(options: ResourceSearchOptions = {}): Promise<{
    ...
    try {
      ...
      let query: any = {
        ...
        isDeleted: false,
        language,
        localizedFor: { $in: [localizedFor, 'general'] },
      };
      ...
      const [resources, total] = await Promise.all([
        EducationalResource.find(query)
          .select('title slug description thumbnail category mediaType difficulty language readingTime duration viewCount downloadCount ratings publishedAt targetAudience')
          .sort(sortCriteria)
          .skip(skip)
          .limit(limit)
          .lean(),
