...
export class LocationFilterService {
   ...
   async getLocationAnalytics(
      ...
      try {
         ...
         const totalClinicalNotes =
            await ClinicalNote.countDocuments(clinicalNotesFilter);
         ...
         const lastClinicalNote = await ClinicalNote.findOne(
            clinicalNotesFilter
         )
            .sort({ createdAt: -1 })
            .select('createdAt')
         ...
         const lastActivity =
            activities.length > 0
               ? new Date(
                    Math.max(...activities.map((date) => date!.getTime()))
                 )
               : null;
   ...
   async getLocationDistributionSummary(
      ...
      try {
         ...
         const sharedPatients =
            distribution.find((item) => !item._id)?.count || 0;
