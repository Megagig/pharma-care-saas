...
export class LocationFilterService {
     ...
     */
    async getClinicalNotesForLocation(options: LocationFilterOptions): Promise<any[]> {
        try {
            const filter = this.buildLocationFilter(options);
    ...
    async getLocationAnalytics(
        ...
        try {
            ...
            const totalClinicalNotes = await ClinicalNote.countDocuments(clinicalNotesFilter);
    ...
    async bulkAssignPatientsToLocation(
        ...
        try {
            ...
            logger.info(`Bulk assigned ${result.modifiedCount} patients to location ${locationId}`);
    ...
    async removeLocationAssignment(
        ...
        try {
            ...
            logger.info(`Removed location assignment from ${result.modifiedCount} patients`);
    ...
    async transferPatientsBetweenLocations(
        ...
        try {
            ...
            logger.info(`Transferred ${result.modifiedCount} patients from ${fromLocationId} to ${toLocationId}`);
    ...
    async assignClinicalNoteToLocation(
        ...
        try {
            ...
            logger.info(`Clinical note ${clinicalNoteId} assigned to location ${locationId}`);
            return true;
        } catch (error) {
     ...
     */
    async getSharedPatients(workspaceId: mongoose.Types.ObjectId): Promise<any[]> {
        try {
            const filter = {
                workspaceId,
                isDeleted: { $ne: true },
    ...
    async getLocationDistributionSummary(
        ...
        totalPatients: number;
        locationDistribution: { locationId: string; count: number; percentage: number }[];
