...
class ConfidentialNoteService {
     ...
     */
    public applyConfidentialFilters(query: any, user: IUser, includeConfidential?: boolean): any {
     ...
     */
    public async getConfidentialNotesStats(workplaceId: string, user: IUser): Promise<any> {
        ...
        const priorityCount = result.notesByPriority.reduce((acc: any, priority: string) => {
     ...
     */
    public validateConfidentialNoteData(noteData: any, user: IUser): { valid: boolean; errors: string[] } {
        const errors: string[] = [];
     ...
     */
    public requiresAccessJustification(note: any, user: IUser, action: string): boolean {
        ...
        if (action === 'VIEW' && note.pharmacist.toString() === user._id.toString()) {
            return false;
        }
