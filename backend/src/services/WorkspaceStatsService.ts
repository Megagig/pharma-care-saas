...
export class WorkspaceStatsService {
      ...
      try {
         ...
         switch (operation) {
            case 'increment':
               ...
               break;
            case 'decrement':
               updateQuery[`stats.${resource}Count`] = {
                  $inc: -Math.abs(delta),
               };
               break;
            case 'set':
         ...
         if (needsCorrection) {
            await Workplace.findByIdAndUpdate(workspaceId, corrections, {
               session,
            });

            // Refetch the corrected stats
            const correctedWorkplace = await Workplace.findById(
               workspaceId,
               null,
               { session }
            );
   ...
   async recalculateUsageStats(
      ...
      try {
         ...
         const workplace = await Workplace.findById(workspaceId, null, {
            session,
         });
         if (!workplace) {
            throw new Error(`Workspace not found: ${workspaceId}`);
         ...
         if (previousStats.patientsCount !== newStats.patientsCount) {
            differences.patientsCount =
               newStats.patientsCount - previousStats.patientsCount;
         }
         if (previousStats.usersCount !== newStats.usersCount) {
            differences.usersCount =
               newStats.usersCount - previousStats.usersCount;
         }
   ...
   async batchRecalculateStats(
      ...
      for (const workspaceId of targetWorkspaces) {
         try {
            ...
            logger.error(
               `Failed to recalculate stats for workspace ${workspaceId}:`,
               error
            );
         }
      }
      ...
      if (errors.length > 0) {
         logger.warn(
            `Batch recalculation completed with ${errors.length} errors:`,
            errors
         );
      }

      logger.info(
         `Batch recalculation completed: ${results.length} successful, ${errors.length} failed`
      );
      return results;
   }
   ...
   async batchResetMonthlyApiCalls(): Promise<void> {
      ...
      logger.info(
         `Batch reset monthly API calls: ${result.modifiedCount} workspaces updated`
      );
   }
