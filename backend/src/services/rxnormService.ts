...
export class RxNormService {
   ...
   async searchDrugs(
      ...
      try {
         const response = await this.client.get<RxNormSearchResult>(
            '/drugs.json',
            {
               params: {
                  name: drugName,
         ...
         logger.info(
            `RxNorm search found ${drugs.length} drugs for "${drugName}"`
         );
         return drugs;
      } catch (error) {
   ...
   async getTherapeuticEquivalents(rxcui: string): Promise<RxNormDrug[]> {
      try {
         const response = await this.client.get<RxNormRelatedResult>(
            `/related.json`,
            {
               params: {
                  rxcui,
         ...
         logger.info(
            `Found ${equivalents.length} therapeutic equivalents for RxCUI ${rxcui}`
         );
         return equivalents;
      } catch (error) {
   ...
   async getRelatedDrugs(rxcui: string): Promise<RxNormDrug[]> {
      try {
         ...
         logger.info(
            `Found ${related.length} related drugs for RxCUI ${rxcui}`
         );
         return related;
      } catch (error) {
   ...
   async getDrugDetails(rxcui: string): Promise<any> {
      try {
         ...
         const propertiesResponse = await this.client.get(
   ...
   async getSpellingSuggestions(drugName: string): Promise<string[]> {
      try {
         ...
         logger.info(
            `Found ${suggestions.length} spelling suggestions for "${drugName}"`
         );
         return suggestions;
      } catch (error) {
