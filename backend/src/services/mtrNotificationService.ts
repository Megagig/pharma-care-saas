...
class MTRNotificationService {
   ...
   async scheduleFollowUpReminder(
      ...
      try {
         ...
         const reminderTime =
            scheduledFor ||
            new Date(followUp.scheduledDate.getTime() - 2 * 60 * 60 * 1000);

         // Check user preferences
         const preferences = assignedUser.notificationPreferences || {};
         if (!preferences.followUpReminders) {
            logger.info(
               `Follow-up reminders disabled for user ${assignedUser._id}`
            );
            return;
         }
         ...
         if (
            (reminderType === 'sms' || followUp.priority === 'high') &&
         ...
         logger.info(
            `Follow-up reminder scheduled for ${assignedUser.email} at ${reminderTime}`
         );
      } catch (error) {
   ...
   async sendCriticalAlert(alert: CriticalAlert): Promise<void> {
      try {
         ...
         const patient = await Patient.findById(alert.patientId).populate(
            'workplaceId'
         );
         if (!patient) {
         ...
         for (const pharmacist of pharmacists) {
            ...
            if (
               (alert.severity === 'critical' || alert.requiresImmediate) &&
               pharmacist.phone &&
         ...
         logger.info(
            `Critical alert sent for patient ${patient.mrn}: ${alert.message}`
         );
      } catch (error) {
   ...
   async checkOverdueFollowUps(): Promise<void> {
      try {
         ...
         for (const followUp of overdueFollowUps) {
            ...
            const recentAlert = followUp.reminders.find(
               (r) =>
                  r.type === 'system' &&
            ...
            const daysOverdue = Math.floor(
               (Date.now() - followUp.scheduledDate.getTime()) /
                  (1000 * 60 * 60 * 24)
            );
   ...
   private async sendNotification(notificationId: string): Promise<void> {
      ...
      try {
         ...
         for (const channel of notification.channels) {
            switch (channel) {
               ...
               case 'push':
                  ...
                  logger.info(
                     `Push notification would be sent to ${recipient.email}`
                  );
                  break;
            }
         }
         ...
         logger.info(
            `Notification ${notificationId} sent successfully to ${recipient.email}`
         );
      } catch (error) {
      ...
      return `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                ...
                <p style="font-size: 12px; color: #6b7280;">
                    This is an automated reminder from the PharmaCare MTR system. 
   ...
   private generateCriticalAlertEmail(data: any): string {
      ...
      return `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                ...
                <hr style="margin: 30px 0; border: none; border-top: 1px solid #e5e7eb;">
                <p style="font-size: 12px; color: #6b7280;">
                    This is an automated alert from the PharmaCare MTR system. 
   ...
   async getNotificationStatistics(
      ...
      try {
         const stats = {
            ...
            sent: Array.from(this.scheduledNotifications.values()).filter(
               (n) => n.sent
            ).length,
            pending: Array.from(this.scheduledNotifications.values()).filter(
               (n) => !n.sent
            ).length,
            failed: Array.from(this.scheduledNotifications.values()).filter(
               (n) => n.error
            ).length,
         ...
         Array.from(this.scheduledNotifications.values()).forEach(
            (notification) => {
               stats.byType[notification.type] =
                  (stats.byType[notification.type] || 0) + 1;
               notification.channels.forEach((channel) => {
                  stats.byChannel[channel] =
                     (stats.byChannel[channel] || 0) + 1;
               });
            }
         );
   ...
   async processPendingReminders(): Promise<void> {
      try {
            ...
            for (const reminder of followUp.reminders) {
               if (!reminder.sent && reminder.scheduledFor <= new Date()) {
                  try {
                     ...
                     logger.error(
                        `Error processing reminder for follow-up ${followUp._id}:`,
                        error
                     );
                  }
               }
         ...
         logger.info(
            `Processed ${pendingFollowUps.length} pending follow-up reminders`
         );
      } catch (error) {
