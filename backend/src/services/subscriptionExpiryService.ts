...
export class SubscriptionExpiryService {
      ...
      try {
         ...
         for (const subscription of expiredTrials) {
            try {
               ...
               logger.info(
                  `Expired trial subscription ${subscription._id} for workspace ${subscription.workspaceId}`
               );
            } catch (error) {
               logger.error(
                  `Error expiring trial subscription ${subscription._id}:`,
                  error
               );
            }
         }
      ...
      try {
         ...
         for (const subscription of expiredSubscriptions) {
            try {
               ...
               logger.info(
                  `Handled expired subscription ${subscription._id} for workspace ${subscription.workspaceId}`
               );
            } catch (error) {
               logger.error(
                  `Error handling expired subscription ${subscription._id}:`,
                  error
               );
            }
         }
   ...
   async checkGracePeriodExpiry(): Promise<void> {
      try {
         ...
         logger.info(
            `Found ${expiredGracePeriods.length} expired grace periods to process`
         );
         ...
         for (const subscription of expiredGracePeriods) {
            try {
               ...
               logger.info(
                  `Expired subscription ${subscription._id} after grace period for workspace ${subscription.workspaceId}`
               );
            } catch (error) {
               logger.error(
                  `Error expiring subscription after grace period ${subscription._id}:`,
                  error
               );
            }
         }
   ...
   async processScheduledDowngrades(): Promise<void> {
      try {
         ...
         logger.info(
            `Found ${subscriptionsToDowngrade.length} scheduled downgrades to process`
         );
         ...
         for (const subscription of subscriptionsToDowngrade) {
            try {
               ...
               logger.info(
                  `Applied scheduled downgrade for subscription ${subscription._id}`
               );
            } catch (error) {
               logger.error(
                  `Error applying scheduled downgrade for subscription ${subscription._id}:`,
                  error
               );
            }
         }
         ...
         for (const subscription of trialsSoonToExpire) {
            try {
               ...
               if (owner) {
                  ...
                  logger.info(
                     `Sent trial expiry warning to ${owner.email} for workspace ${workspace.name}`
                  );
               }
            } catch (error) {
               logger.error(
                  `Error sending trial expiry warning for subscription ${subscription._id}:`,
                  error
               );
            }
         }
         ...
         for (const subscription of subscriptionsSoonToExpire) {
            try {
               ...
               if (owner) {
                  ...
                  logger.info(
                     `Sent subscription expiry warning to ${owner.email} for workspace ${workspace.name}`
                  );
               }
            } catch (error) {
               logger.error(
                  `Error sending subscription expiry warning for subscription ${subscription._id}:`,
                  error
               );
            }
         }
    ...
    */
   private async expireSubscriptionAfterGracePeriod(
      subscription: any
   ): Promise<void> {
      const workspace = subscription.workspaceId;
