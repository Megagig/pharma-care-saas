...
import Notification, { INotification, INotificationData, INotificationDeliveryChannels } from '../models/Notification';
...
import { notificationTemplateService, NotificationTemplateService } from '../templates/notifications/notificationTemplates';
...
class NotificationService {
    ...
    async createNotification(data: CreateNotificationData): Promise<INotification> {
        try {
            ...
            const user = await User.findById(data.userId).select('notificationPreferences');
     ...
     */
    async sendRealTimeNotification(userId: string, notification: INotification): Promise<void> {
        if (!this.io) {
        ...
        try {
            ...
            logger.debug(`Real-time notification sent to ${userSockets.length} sockets for user ${userId}`);
     ...
     */
    async sendEmailNotification(userId: string, notification: INotification): Promise<void> {
        try {
     ...
     */
    async sendSMSNotification(userId: string, notification: INotification): Promise<void> {
        try {
            const user = await User.findById(userId).select('phone firstName lastName');
            if (!user || !user.phone) {
    ...
    async deliverNotification(notification: INotification): Promise<void> {
        ...
        if (notification.deliveryChannels.inApp) {
            deliveryPromises.push(
                this.sendRealTimeNotification(notification.userId.toString(), notification)
                    .catch(error => logger.error('In-app delivery failed:', error))
            );
        }
        ...
        if (notification.deliveryChannels.email) {
            deliveryPromises.push(
                this.sendEmailNotification(notification.userId.toString(), notification)
                    .catch(error => logger.error('Email delivery failed:', error))
            );
        }
        ...
        if (notification.deliveryChannels.sms) {
            deliveryPromises.push(
                this.sendSMSNotification(notification.userId.toString(), notification)
                    .catch(error => logger.error('SMS delivery failed:', error))
            );
        }
    ...
    async processScheduledNotifications(): Promise<void> {
        try {
            const scheduledNotifications = await Notification.findScheduledForDelivery();

            for (const notification of scheduledNotifications) {
                try {
                    await this.deliverNotification(notification);
                } catch (error) {
                    logger.error(`Failed to deliver scheduled notification ${notification._id}:`, error);
                }
            }
    ...
    async createConversationNotification(
        ...
        try {
            const [conversation, sender, message] = await Promise.all([
                Conversation.findById(conversationId).populate('patientId', 'firstName lastName'),
                ...
                for (const deliveryStatus of notification.deliveryStatus) {
                    if (deliveryStatus.status === 'failed' && notification.canRetryDelivery(deliveryStatus.channel)) {
                        try {
                            ...
                            logger.error(`Retry failed for notification ${notification._id}, channel ${deliveryStatus.channel}:`, error);
                        }
                    }
                }
            }

            logger.info(`Processed retry for ${failedNotifications.length} failed notifications`);
    ...
    private getNextAvailableTime(quietHours: NotificationPreferences['quietHours']): Date {
        const now = new Date();
    ...
    private getNotificationTemplate(type: string, data: INotificationData): NotificationTemplate {
        const variables = NotificationTemplateService.getTemplateVariables(type, data);
        return notificationTemplateService.getTemplate(type, variables);
    }
