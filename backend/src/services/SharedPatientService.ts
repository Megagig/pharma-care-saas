...
export class SharedPatientService {
    ...
    */
   async sharePatientWithLocations(
      options: PatientSharingOptions
   ): Promise<boolean> {
      try {
         const {
            patientId,
         ...
         logger.info(
            `Patient ${patientId} shared from location ${fromLocationId} to locations: ${toLocationIds.join(', ')}`
         );
         return true;
      } catch (error) {
   ...
   async revokeSharedAccess(
      ...
      try {
         ...
         if (locationIds && locationIds.length > 0) {
            ...
            const currentSharedAccess = patient.metadata
               ?.sharedAccess as SharedPatientAccess;
            if (currentSharedAccess) {
               const updatedSharedLocations =
                  currentSharedAccess.sharedWithLocations.filter(
               ...
               if (updatedSharedLocations.length === 0) {
                  ...
                  await Patient.findByIdAndUpdate(patientId, {
                     $set: {
                        'metadata.sharedAccess.sharedWithLocations':
                           updatedSharedLocations,
                     },
                  });
               }
            }
   ...
   async getPatientsAccessibleFromLocation(
      ...
      try {
         ...
         const patients = await Patient.find(query)
            .select(
               '_id firstName lastName mrn locationId metadata.sharedAccess createdAt'
            )
            .sort({ createdAt: -1 })
            .lean();
         ...
         const enrichedPatients = patients.map((patient) => {
            const sharedAccess = patient.metadata
               ?.sharedAccess as SharedPatientAccess;
            ...
            if (patient.locationId === locationId) {
               ...
               sharedAccess?.sharedWithLocations?.includes(locationId)
            ) {
               accessType = 'shared';
         ...
         logger.error(
            'Error getting patients accessible from location:',
            error
         );
         throw error;
      }
   ...
   async createTransferWorkflow(
      ...
      try {
         ...
         logger.info(
            `Transfer workflow created: ${transferId} for patient ${patientId}`
         );
   ...
   async completePatientTransfer(
      ...
      try {
         ...
         logger.info(
            `Patient transfer completed: ${transferId} for patient ${patientId}`
         );
         return true;
      } catch (error) {
   ...
   async cleanupExpiredSharedAccess(): Promise<number> {
      try {
         ...
         logger.info(
            `Cleaned up ${result.modifiedCount} expired shared access records`
         );
