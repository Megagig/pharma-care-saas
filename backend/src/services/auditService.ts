...
class AuditService {
    ...
    static async getAuditLogs(
        ...
        if (filters.complianceCategory) query.complianceCategory = filters.complianceCategory;
     ...
     * Helper methods
     */
    private static determineComplianceCategory(action: string): 'clinical_documentation' | 'patient_safety' | 'data_access' | 'system_security' | 'workflow_compliance' {
    ...
    private static determineRiskLevel(action: string, resourceType: string): 'low' | 'medium' | 'high' | 'critical' {
    ...
    private static calculateComplianceScore(stats: any): number {
        ...
        const riskPenalty = stats.riskDistribution?.filter((r: string) => r === 'critical').length * 5;

        return Math.max(0, baseScore - errorPenalty - riskPenalty);
    }

    private static generateComplianceRecommendations(summary: any, metrics: any[]): string[] {
    ...
    private static async triggerSecurityAlert(auditLog: IMTRAuditLog): Promise<void> {
        // Log security alert
