...
class PermissionService {
   ...
   private async loadPermissionMatrix(): Promise<PermissionMatrix> {
      ...
      logger.info(
         `Permission matrix loaded with ${Object.keys(this.cachedMatrix).length} actions`
      );
   ...
   private checkSubscriptionRequirements(
      ...
      if (
         ...
         context.workspace?.subscriptionStatus === 'trial'
      ) {
   ...
   private checkWorkplaceRoles(
      ...
      if (
         !permission.workplaceRoles ||
         permission.workplaceRoles.length === 0
      ) {
         return { allowed: true };
      }
      ...
      const allowedRoles = WORKPLACE_ROLE_HIERARCHY[userWorkplaceRole] || [
         userWorkplaceRole,
      ];
