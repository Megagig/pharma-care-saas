...
export class PatientMTRIntegrationService {
   ...
   async getPatientDataForMTR(
      ...
      try {
         ...
         throw error instanceof AppError
            ? error
            : new AppError('Failed to get patient data for MTR', 500);
      }
   }
   ...
   async syncMedicationsWithMTR(
      ...
      try {
         ...
         throw error instanceof AppError
            ? error
            : new AppError('Failed to sync medications with MTR', 500);
      }
   }
   ...
   async getPatientDashboardMTRData(
      ...
      try {
         const mtrSummary = await this.getPatientMTRSummary(
            patientId,
            workplaceId
         );
