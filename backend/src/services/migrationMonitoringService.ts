...
import { MigrationValidationService, ValidationResult } from './migrationValidationService';
...
export class MigrationMonitoringService {
    ...
    async checkForAlerts(): Promise<MigrationAlert[]> {
        try {
            ...
            this.alerts = this.alerts.filter(alert =>
                !alert.resolved ||
                (new Date().getTime() - alert.timestamp.getTime()) < 7 * 24 * 60 * 60 * 1000 // Keep for 7 days
            );
     ...
     */
    async generateReport(type: 'daily' | 'weekly' | 'on_demand' = 'on_demand'): Promise<MigrationReport> {
        try {
    ...
    getTrendAnalysis(): {
        ...
        const progressTrend =
            secondHalfProgress > firstHalfProgress + 2 ? 'improving' :
                secondHalfProgress < firstHalfProgress - 2 ? 'declining' : 'stable';

        const validationTrend =
            secondHalfValidation > firstHalfValidation + 2 ? 'improving' :
    ...
    async getStatusSummary(): Promise<{
        ...
        try {
            ...
            if (status === 'in_progress' && trend.progressTrend === 'improving') {
                ...
                estimatedCompletion = new Date(Date.now() + estimatedMeasurements * 60 * 60 * 1000); // Assuming hourly measurements
            }
     ...
     * Generate next actions based on current state
     */
    private generateNextActions(metrics: MigrationMetrics, validation: ValidationResult): string[] {
