...
export class NotificationSchedulerService {
    ...
    addJob(id: string, cronExpression: string, taskFunction: () => Promise<void>, description: string): void {
        ...
        logger.info(`Added scheduled job: ${id} (${cronExpression}) - ${description}`);
    }
    ...
    private async processScheduledNotifications(): Promise<void> {
        try {
            ...
            logger.debug(`Processing ${scheduledNotifications.length} scheduled notifications`);
     ...
     */
    private async sendDigestToUser(user: any, frequency: 'daily' | 'weekly'): Promise<void> {
        const now = new Date();
     ...
     */
    private createDigestContent(groupedNotifications: Record<string, any[]>, frequency: string): string {
    ...
    private async cleanupExpiredNotifications(): Promise<void> {
        try {
            ...
            const result = await Notification.markExpiredAsArchived();

            if (result.modifiedCount > 0) {
                logger.info(`Marked ${result.modifiedCount} expired notifications as archived`);
            }
        } catch (error) {
    ...
    async scheduleOneTimeNotification(
        ...
        setTimeout(async () => {
            try {
                await notificationService.createNotification(notificationData);
                logger.debug(`Delivered scheduled notification for user ${notificationData.userId}`);
            } catch (error) {
                logger.error('Error delivering scheduled notification:', error);
            }
        }, delay);

        logger.debug(`Scheduled one-time notification for ${scheduledFor.toISOString()}`);
    }
