...
export class SocketNotificationService {
    ...
    private setupSocketHandlers(): void {
        ...
        this.io.use(async (socket: AuthenticatedSocket, next) => {
            try {
                ...
                const user = await User.findById(decoded.id).select('_id workplaceId role email firstName lastName');
    ...
    private handleConnection(socket: AuthenticatedSocket): void {
        ...
        logger.info(`User ${userData.firstName} ${userData.lastName} connected (${socket.id})`);
    ...
    private handleDisconnection(socket: AuthenticatedSocket): void {
        ...
        if (userData) {
            logger.info(`User ${userData.firstName} ${userData.lastName} disconnected (${socket.id})`);
        }
    ...
    private setupNotificationHandlers(socket: AuthenticatedSocket): void {
        ...
        socket.on('notification:mark_read', async (data: { notificationId: string }) => {
            try {
                ...
                logger.debug(`Notification ${data.notificationId} marked as read by user ${socket.userId}`);
            } catch (error) {
        ...
        socket.on('notification:get_recent', async (data: { limit?: number } = {}) => {
            try {
        ...
        socket.on('notification:update_preferences', async (preferences: any) => {
            try {
                await notificationService.updateNotificationPreferences(socket.userId!, preferences);
                ...
                logger.debug(`Notification preferences updated for user ${socket.userId}`);
            } catch (error) {
    ...
    private setupConversationHandlers(socket: AuthenticatedSocket): void {
        ...
        // Typing indicators
        socket.on('conversation:typing_start', (data: { conversationId: string }) => {
        ...
        socket.on('conversation:typing_stop', (data: { conversationId: string }) => {
        ...
        socket.on('message:mark_read', (data: { messageId: string, conversationId: string }) => {
     ...
     */
    private async sendInitialNotifications(socket: AuthenticatedSocket): void {
        try {
            // Send unread count
            const unreadCount = await Notification.getUnreadCountByUser(
                socket.userId! as any,
