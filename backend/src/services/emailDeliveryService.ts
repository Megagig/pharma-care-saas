...
export class EmailDeliveryService {
         ...
         for (const delivery of pendingRetries) {
            try {
               ...
               if (!delivery.metadata?.emailContent) {
                  console.warn(
                     `No email content found for delivery ${delivery._id}, skipping retry`
                  );
                  continue;
               }
               ...
               if (result.success) {
                  ...
                  await delivery.save();
                  console.log(
                     `Successfully retried email delivery ${delivery._id}`
                  );
               } else {
                  await delivery.markAsFailed(result.error || 'Retry failed');
                  console.error(
                     `Failed to retry email delivery ${delivery._id}:`,
                     result.error
                  );
               }
            } catch (error) {
               ...
               console.error(
                  `Error retrying email delivery ${delivery._id}:`,
                  error
               );
            }
         }
   ...
   async updateDeliveryStatus(
      ...
      try {
         ...
         if (!delivery) {
            console.warn(
               `No delivery record found for messageId: ${messageId}`
            );
            return;
         }
         ...
         console.error(
            `Error updating delivery status for ${messageId}:`,
            error
         );
      }
   }
   ...
   async cleanupOldRecords(daysOld: number = 90): Promise<void> {
      try {
         ...
         console.log(
            `Cleaned up ${result.deletedCount} old email delivery records`
         );
      } catch (error) {
