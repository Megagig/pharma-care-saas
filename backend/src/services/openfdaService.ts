...
export class OpenFDAService {
   ...
   async getAdverseEvents(
      ...
      limit: number = 100,
      skip: number = 0
   ): Promise<OpenFDAAdverseEventResult> {
      try {
         const searchQuery = `patient.drug.medicinalproduct:"${drugName}"`;

         const response = await this.client.get<OpenFDAAdverseEventResult>(
            '/event.json',
            {
               params: {
                  search: searchQuery,
         ...
         logger.info(
            `OpenFDA adverse events found ${response.data.meta.results.total} results for "${drugName}"`
         );
    ...
    */
   async getDrugLabeling(
      drugName: string,
      limit: number = 10,
      skip: number = 0
   ): Promise<OpenFDADrugLabelResult> {
      try {
         // Try searching by brand name first, then generic name
         let searchQuery = `openfda.brand_name:"${drugName}"`;

         const response = await this.client.get<OpenFDADrugLabelResult>(
            '/label.json',
            {
               params: {
                  search: searchQuery,
         ...
         if (response.data.meta.results.total === 0) {
            ...
            const genericResponse =
               await this.client.get<OpenFDADrugLabelResult>('/label.json', {
                  params: {
                     search: searchQuery,
                     limit,
                     skip,
            ...
            logger.info(
               `OpenFDA drug labeling found ${genericResponse.data.meta.results.total} results for generic "${drugName}"`
            );
            return genericResponse.data;
         }

         logger.info(
            `OpenFDA drug labeling found ${response.data.meta.results.total} results for brand "${drugName}"`
         );
         return response.data;
      } catch (error) {
    ...
    */
   async getAdverseEventsBySeverity(
      drugName: string,
      ...
      try {
         ...
         logger.info(
            `OpenFDA serious adverse events found ${response.data.meta.results.total} results for "${drugName}"`
         );
         return response.data;
      } catch (error) {
      ...
      events.forEach((event) => {
            ...
            event.patient.reaction.forEach((reaction) => {
               if (reaction.reactionmeddrapt) {
                  analysis.reactionCounts[reaction.reactionmeddrapt] =
                     (analysis.reactionCounts[reaction.reactionmeddrapt] || 0) +
                     1;
               }
         ...
         if (event.patient?.patientsex) {
            analysis.demographics.genders[event.patient.patientsex] =
               (analysis.demographics.genders[event.patient.patientsex] || 0) +
               1;
         }
