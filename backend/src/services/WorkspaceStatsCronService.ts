...
export class WorkspaceStatsCronService {
   ...
   private startDailyRecalculation(): void {
      this.dailyRecalculationJob = cron.schedule(
         '0 2 * * *',
         async () => {
            ...
            try {
               const results =
                  await WorkspaceStatsService.batchRecalculateStats();
               logger.info(
                  `Daily recalculation completed: ${results.length} workspaces processed`
               );
            } catch (error) {
               logger.error(
                  'Daily workspace stats recalculation failed:',
                  error
               );
            }
         },
   ...
   private startMonthlyApiReset(): void {
      this.monthlyApiResetJob = cron.schedule(
         '1 0 1 * *',
         async () => {
   ...
   private startStaleStatsCheck(): void {
      this.staleStatsCheckJob = cron.schedule(
         '0 */6 * * *',
         async () => {
            ...
            try {
               const staleWorkspaces =
                  await WorkspaceStatsService.getWorkspacesWithStaleStats();

               if (staleWorkspaces.length > 0) {
                  logger.info(
                     `Found ${staleWorkspaces.length} workspaces with stale stats`
                  );
                  ...
                  logger.info(
                     `Stale stats recalculation completed: ${results.length} workspaces updated`
                  );
               } else {
   ...
   async triggerDailyRecalculation(): Promise<void> {
      ...
      try {
         ...
         logger.info(
            `Manual recalculation completed: ${results.length} workspaces processed`
         );
      } catch (error) {
