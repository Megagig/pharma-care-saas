...
class InvitationCronService {
      ...
      try {
         ...
         for (const invitation of expiredInvitations) {
            try {
               ...
               errors.push(
                  `Failed to send expiry notification for invitation ${invitation._id}: ${(error as Error).message}`
               );
            }
         }
         ...
         return {
            ...
            notificationsSent,
            errors: [
               `Failed to mark expired invitations: ${(error as Error).message}`,
            ],
         };
      }
   }
    ...
    */
   async validateInvitationLimits(
      workspaceId: string
   ): Promise<InvitationLimits> {
      try {
         ...
         const remainingInvites = Math.max(
            0,
            maxPendingInvites - currentPendingInvites
         );
         ...
         return {
            ...
            canSendMore,
            upgradeRequired,
            nextUpgradeLevel: upgradeRequired
               ? 'Professional Plan (50 invitations)'
               : undefined,
         };
      } catch (error) {
   ...
   async getInvitationAnalytics(
      ...
      try {
         ...
         // Get basic counts
         const [totalInvitations, statusCounts, roleStats, monthlyStats] =
            await Promise.all([
         ...
         const acceptanceRate =
            totalSentInvitations > 0
               ? (usedInvitations / totalSentInvitations) * 100
               : 0;
         ...
         if (acceptedInvitations.length > 0) {
            ...
            averageAcceptanceTime =
               totalTime / acceptedInvitations.length / (1000 * 60 * 60); // Convert to hours
         }

         // Process role stats
         const invitationsByRole = roleStats.reduce(
            (acc, item) => {
               acc[item._id] = item.count;
               return acc;
            },
         ...
         return {
            ...
            averageAcceptanceTime:
               Math.round(averageAcceptanceTime * 100) / 100,
            invitationsByRole,
            invitationsByMonth,
         };
      } catch (error) {
   ...
   async getInvitationStats(workspaceId?: string): Promise<InvitationStats> {
      try {
         if (workspaceId) {
            ...
            // Global stats
            const [totalWorkspaces, totalInvitations, workspaceStats] =
               await Promise.all([
            ...
            const averageInvitationsPerWorkspace =
               totalWorkspaces > 0 ? totalInvitations / totalWorkspaces : 0;

            const totalAccepted = workspaceStats.reduce(
               (sum, ws) => sum + ws.acceptedCount,
               0
            );
            const globalAcceptanceRate =
               totalInvitations > 0
                  ? (totalAccepted / totalInvitations) * 100
                  : 0;
            ...
            return {
               global: {
                  totalWorkspaces,
                  totalInvitations,
                  averageInvitationsPerWorkspace:
                     Math.round(averageInvitationsPerWorkspace * 100) / 100,
                  globalAcceptanceRate:
                     Math.round(globalAcceptanceRate * 100) / 100,
               },
            };
         }
      } catch (error) {
      ...
      try {
         ...
         for (const invitation of expiringInvitations) {
            try {
               ...
               errors.push(
                  `Failed to send reminder for invitation ${invitation._id}: ${(error as Error).message}`
               );
            }
         }
         ...
         return {
            ...
            remindersSent,
            errors: [
               `Failed to send expiry reminders: ${(error as Error).message}`,
            ],
         };
      }
   }
