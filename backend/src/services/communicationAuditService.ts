...
export class CommunicationAuditService {
    ...
    static async getAuditLogs(
        ...
        try {
            ...
            if (conversationId) query['details.conversationId'] = new mongoose.Types.ObjectId(conversationId);
            if (patientId) query['details.patientId'] = new mongoose.Types.ObjectId(patientId);
     ...
     */
    static async cleanupOldLogs(daysToKeep: number = 2555): Promise<number> { // 7 years default
        try {
            ...
            logger.info(`Cleaned up ${result.deletedCount} old communication audit logs`, {
                cutoffDate,
