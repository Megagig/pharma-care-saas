...
const suspiciousUsers = new Map<
   string,
   { score: number; lastActivity: Date }
>();
...
class SecurityMonitoringService {
   ...
   async analyzeSecurityEvent(
      ...
      try {
         ...
         switch (eventType) {
            case 'login_failed':
               threats.push(
                  ...(await this.detectBruteForceAttack(req, eventData))
               );
               break;
            case 'permission_denied':
               threats.push(
                  ...(await this.detectPermissionEscalation(req, eventData))
               );
               break;
            case 'invitation_created':
               threats.push(
                  ...(await this.detectInvitationSpam(req, eventData))
               );
               break;
            case 'data_access':
               threats.push(
                  ...(await this.detectDataExfiltration(req, eventData))
               );
               break;
            case 'session_created':
               threats.push(
                  ...(await this.detectSessionAnomalies(req, eventData))
               );
               break;
            default:
               // General suspicious activity detection
               threats.push(
                  ...(await this.detectSuspiciousActivity(req, eventData))
               );
         }
   ...
   private async detectBruteForceAttack(
      ...
      const recentFailures = getAuditLogs({
         ...
         (log) =>
            log.ipAddress === ipAddress && log.action === 'USER_LOGIN_FAILED'
      );
      ...
      if (recentFailures.length >= SECURITY_CONFIG.MAX_FAILED_LOGINS) {
         threats.push({
            ...
            evidence: {
               ...
               targetEmails: [
                  ...new Set(
                     recentFailures.map((f) => f.details?.email).filter(Boolean)
                  ),
               ],
            },
            timestamp: new Date(),
    ...
    */
   private async detectPermissionEscalation(
      req: AuthRequest,
      eventData: any
   ): Promise<SecurityThreat[]> {
      const threats: SecurityThreat[] = [];
         ...
         threats.push({
            ...
            evidence: {
               ...
               attemptedPermissions: [
                  ...new Set(
                     recentDenials
                        .map((d) => d.details?.requiredPermission)
                        .filter(Boolean)
                  ),
               ],
    ...
    */
   private async detectInvitationSpam(
      req: AuthRequest,
      eventData: any
   ): Promise<SecurityThreat[]> {
      const threats: SecurityThreat[] = [];
      ...
      if (recentInvitations.length > SECURITY_CONFIG.MAX_INVITATIONS_PER_HOUR) {
         threats.push({
            ...
            evidence: {
               ...
               invitedEmails: recentInvitations
                  .map((i) => i.details?.inviteeEmail)
                  .filter(Boolean),
            },
            timestamp: new Date(),
            resolved: false,
   ...
   private async detectDataExfiltration(
      ...
      const exportEvents = recentAccess.filter((log) =>
         log.action.includes('EXPORT')
      );
      const bulkAccess = recentAccess.filter((log) =>
      ...
      if (exportEvents.length > 5 || bulkAccess.length > 10) {
         threats.push({
            ...
            evidence: {
               ...
               accessedResources: [
                  ...new Set(
                     recentAccess.map((a) => a.resourceType).filter(Boolean)
                  ),
               ],
            },
            timestamp: new Date(),
   ...
   private async detectSuspiciousActivity(
      ...
      const uniqueUsers = new Set(
         usersFromIP.map((log) => log.userId?.toString()).filter(Boolean)
      );
         ...
         try {
            switch (action) {
               case 'block_ip':
                  ...
                  logger.info(`IP blocked: ${threat.ipAddress}`, {
                     threatId: threat.id,
                  });
                  break;
               ...
               case 'monitor_ip':
                  ...
                  logger.info(`IP added to monitoring: ${threat.ipAddress}`, {
                     threatId: threat.id,
                  });
                  break;

               default:
                  logger.warn(`Unknown threat response action: ${action}`, {
                     threatId: threat.id,
                  });
            }
         } catch (error: any) {
   ...
   private async sendSecurityAlerts(threat: SecurityThreat): Promise<void> {
      ...
      if (
         lastAlert &&
         Date.now() - lastAlert.getTime() < SECURITY_CONFIG.ALERT_COOLDOWN
      ) {
         return;
      }
      ...
      try {
         switch (alert.type) {
            case 'email':
               ...
               logger.info(
                  `Security alert email would be sent to: ${alert.recipient}`,
                  {
                     alertId: alert.id,
   ...
   getUserSuspiciousScore(userId: string): number {
      ...
      const hoursSinceLastActivity =
         (Date.now() - suspiciousData.lastActivity.getTime()) /
         (60 * 60 * 1000);
      const decayedScore = Math.max(
         0,
         suspiciousData.score - hoursSinceLastActivity * 0.5
      );
   ...
   getSecurityThreats(
      ...
      if (filters.userId) {
         threats = threats.filter(
            (t) => t.userId?.toString() === filters.userId
         );
      }

      if (filters.workspaceId) {
         threats = threats.filter(
            (t) => t.workspaceId?.toString() === filters.workspaceId
         );
      }
