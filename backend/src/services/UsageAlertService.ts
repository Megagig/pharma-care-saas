...
export class UsageAlertService {
   ...
   async checkAndSendUsageAlerts(): Promise<void> {
      try {
            ...
            try {
               ...
               if (alerts.length > 0) {
                  ...
                  const notificationSent =
                     await this.sendUsageAlertNotification(workspace, alerts);
                  if (notificationSent) {
                     notificationsSent++;
                  }
               }
               ...
               logger.error(
                  `Error checking alerts for workspace ${workspace._id}:`,
                  error
               );
            }
         }
         ...
         logger.info(
            `Usage alert check completed: ${totalAlerts} alerts found, ${notificationsSent} notifications sent`
         );
      } catch (error) {
    ...
    */
   async checkWorkspaceUsageAlerts(
      workspace: IWorkplace
   ): Promise<UsageAlert[]> {
      try {
         ...
         if (!subscription || !plan) {
            logger.warn(
               `Missing subscription or plan for workspace ${workspace._id}`
            );
            return [];
         }
         ...
         Object.entries(usageStats).forEach(
            ([resource, stats]: [string, any]) => {
               if (stats.isAtLimit) {
                  alerts.push({
                     ...
                     percentage: Math.round(
                        (stats.currentUsage / stats.limit) * 100
                     ),
                     severity: 'critical',
         ...
         logger.error(
            `Error checking usage alerts for workspace ${workspace._id}:`,
            error
         );
         return [];
      }
    ...
    */
   async sendUsageAlertNotification(
      workspace: IWorkplace,
      alerts: UsageAlert[]
   ): Promise<boolean> {
      try {
         ...
         const templateType = hasCriticalAlerts
         ...
         logger.info(
            `Usage alert notification sent to ${owner.email} for workspace ${workspace.name}`
         );
         return true;
      } catch (error) {
         logger.error(
            `Error sending usage alert notification for workspace ${workspace._id}:`,
            error
         );
         return false;
      }
   ...
   private async sendUsageAlertEmail(
      ...
      const subject =
         templateType === 'usage_limit_exceeded'
   ...
   private generateAlertRecommendations(alerts: UsageAlert[]): string[] {
      ...
      if (criticalAlerts.length > 0) {
         ...
         recommendations.push(
            'Consider archiving or removing unused data to free up space'
         );
      }
      ...
      if (warningAlerts.length > 0) {
         ...
         recommendations.push(
            'Review your usage patterns to optimize resource consumption'
         );
      }

      // Resource-specific recommendations
      const patientAlerts = alerts.filter(
         (alert) => alert.resource === 'patients'
      );
      const userAlerts = alerts.filter((alert) => alert.resource === 'users');
      const storageAlerts = alerts.filter(
         (alert) => alert.resource === 'storage'
      );
      ...
      if (storageAlerts.length > 0) {
         recommendations.push(
            'Clean up old files and documents to free up storage space'
         );
      }
   ...
   async getWorkspaceAlertSummary(workspaceId: string): Promise<{
      ...
      try {
         ...
         logger.error(
            `Error getting alert summary for workspace ${workspaceId}:`,
            error
         );
         throw error;
      }
