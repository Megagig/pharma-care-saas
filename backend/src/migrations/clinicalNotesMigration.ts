...
class ClinicalNotesMigration {
    ...
    async runMigrations(): Promise<MigrationResult> {
        try {
            ...
            logger.info(`Clinical Notes migrations completed successfully. Total records migrated: ${totalMigrated}`);
            return {
                success: true,
        ...
        try {
            ...
            for (const index of indexes) {
                try {
                    ...
                    logger.warn(`Index creation warning: ${error instanceof Error ? error.message : 'Unknown error'}`);
                }
            }
    ...
    private async validateDataIntegrity(): Promise<MigrationResult> {
        try {
            ...
            const notesWithoutPatient = await ClinicalNote.countDocuments({ patient: { $exists: false } });
            if (notesWithoutPatient > 0) {
                issues.push(`${notesWithoutPatient} notes missing patient reference`);
            }

            const notesWithoutPharmacist = await ClinicalNote.countDocuments({ pharmacist: { $exists: false } });
            if (notesWithoutPharmacist > 0) {
                issues.push(`${notesWithoutPharmacist} notes missing pharmacist reference`);
            }

            const notesWithoutWorkplace = await ClinicalNote.countDocuments({ workplaceId: { $exists: false } });
            ...
            const notesWithInvalidType = await ClinicalNote.countDocuments({
                type: { $nin: ['consultation', 'medication_review', 'follow_up', 'adverse_event', 'other'] }
            });
            ...
            const activeNotes = await ClinicalNote.countDocuments({ deletedAt: { $exists: false } });
            const deletedNotes = await ClinicalNote.countDocuments({ deletedAt: { $exists: true } });
    ...
    async getMigrationStatus(): Promise<{
        ...
        try {
            ...
            logger.error(`Failed to get migration status: ${error instanceof Error ? error.message : 'Unknown error'}`);
            throw error;
        }
    }
}
...
export const runClinicalNotesMigrations = async (): Promise<MigrationResult> => {
    const migration = ClinicalNotesMigration.getInstance();
    return await migration.runMigrations();
};

export const rollbackClinicalNotesMigrations = async (): Promise<MigrationResult> => {
    const migration = ClinicalNotesMigration.getInstance();
