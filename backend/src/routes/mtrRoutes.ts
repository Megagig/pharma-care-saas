...
import { mtrValidationMiddleware } from '../middlewares/mtrValidation';
...
router.get(
   ...
   auditMTRActivity('VIEW_MTR_SESSIONS'),
   getMTRSessions
);
...
router.post(
   ...
   createMTRSessionSchema,
   mtrValidationMiddleware.handleValidationErrors,
   auditMTRActivity('CREATE_MTR_SESSION'),
   createMTRSession
);
...
router.get(
   ...
   mtrParamsSchema,
   mtrValidationMiddleware.handleValidationErrors,
   auditMTRActivity('VIEW_MTR_SESSION'),
   getMTRSession
);
...
router.put(
   '/:id',
   [...mtrParamsSchema, ...updateMTRSessionSchema],
   mtrValidationMiddleware.handleValidationErrors,
   auditMTRActivity('UPDATE_MTR_SESSION'),
   updateMTRSession
);
...
router.delete(
   ...
   mtrParamsSchema,
   mtrValidationMiddleware.handleValidationErrors,
   auditMTRActivity('DELETE_MTR_SESSION'),
   deleteMTRSession
);
...
router.put(
   ...
   (req: Request, res: Response, next: NextFunction) => {
      if (
         req.params.stepName === 'medicationHistory' &&
         req.body.data?.medications
      ) {
         return mtrValidationMiddleware.validateMedicationHistory(
            req,
            res,
            next
         );
...
router.get(
   '/patient/:patientId',
   [...patientParamsSchema, ...mtrQuerySchema],
   mtrValidationMiddleware.handleValidationErrors,
   getPatientMTRHistory
);
...
router.post(
   '/patient/:patientId',
   [...patientParamsSchema, ...createMTRSessionSchema],
   mtrValidationMiddleware.handleValidationErrors,
   createPatientMTRSession
);
...
router.post(
   '/:id/problems',
   [...mtrParamsSchema, ...createProblemSchema],
   mtrValidationMiddleware.handleValidationErrors,
   createMTRProblem
);
...
router.put(
   ...
   [...problemParamsSchema, ...updateProblemSchema],
   mtrValidationMiddleware.handleValidationErrors,
   updateMTRProblem
);
...
router.post(
   '/:id/interventions',
   [...mtrParamsSchema, ...createInterventionSchema],
   mtrValidationMiddleware.handleValidationErrors,
   createMTRIntervention
);
...
router.put(
   ...
   [...interventionParamsSchema, ...updateInterventionSchema],
   mtrValidationMiddleware.handleValidationErrors,
   updateMTRIntervention
);
...
router.post(
   '/:id/followups',
   [...mtrParamsSchema, ...createFollowUpSchema],
   mtrValidationMiddleware.handleValidationErrors,
   createMTRFollowUp
);
...
router.put(
   ...
   [...followUpParamsSchema, ...updateFollowUpSchema],
   mtrValidationMiddleware.handleValidationErrors,
   updateMTRFollowUp
);
