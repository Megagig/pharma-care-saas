...
} from '../middlewares/compatibilityLayer';
...
router.use('/pharmacy/:pharmacyId/*', (req, res, next) => {
   ...
   const newUrl = originalUrl
      .replace('/pharmacy/', '/workspace/')
      .replace('/pharmacyId/', '/workspaceId/');
...
router.get(
   '/user/subscription',
   compatibilityAuth,
   legacyEndpointWrapper(async (req: CompatibilityRequest, res) => {
      try {
         const user = req.user!;
   ...
   legacyEndpointWrapper(async (req: CompatibilityRequest, res) => {
      try {
         ...
         const workplace = await Workplace.findById(workspaceId)
            ...
            .populate(
               'teamMembers',
               'firstName lastName email role workplaceRole'
            )
   ...
   legacyEndpointWrapper(async (req: CompatibilityRequest, res) => {
      try {
         ...
         if (req.workplace) {
            ...
            return await subscriptionController.upgradeWorkspaceSubscription(
               req,
               res
            );
         } else {
            ...
            console.warn(
               'Legacy user subscription upgrade attempted:',
               req.user!._id,
               planId
            );
