...
const executionRateLimit = rateLimit({
   ...
   message: {
      success: false,
      message:
         'Migration execution rate limit exceeded. Please wait before trying again.',
   },
   standardHeaders: true,
   legacyHeaders: false,
});
...
router.post(
   '/validate',
   migrationRateLimit,
   MigrationDashboardController.runValidation
);
...
router.post(
   ...
   MigrationDashboardController.resolveAlert
);
...
router.post(
   '/report',
   migrationRateLimit,
   MigrationDashboardController.generateReport
);
...
router.post(
   '/dry-run',
   migrationRateLimit,
   MigrationDashboardController.runDryRun
);
...
router.post(
   '/execute',
   executionRateLimit,
   MigrationDashboardController.executeMigration
);
...
router.post(
   '/rollback',
   executionRateLimit,
   MigrationDashboardController.executeRollback
);
