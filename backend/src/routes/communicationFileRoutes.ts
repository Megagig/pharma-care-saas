...
router.use(auth);
router.use(rbac(['pharmacist', 'doctor', 'patient']));

// Apply rate limiting for file operations
const fileUploadLimiter = rateLimiting({
    windowMs: 15 * 60 * 1000, // 15 minutes
    max: 20, // 20 uploads per window
    message: 'Too many file uploads, please try again later',
});

const fileDownloadLimiter = rateLimiting({
    windowMs: 15 * 60 * 1000, // 15 minutes
...
router.delete(
    '/files/:fileId',
    CommunicationFileController.deleteFile
);
