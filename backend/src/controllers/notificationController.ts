...
import { notificationService, CreateNotificationData, NotificationFilters } from '../services/notificationService';
...
export const createNotification = async (req: AuthenticatedRequest, res: Response) => {
    try {
        ...
        if (!type || !title || !content) {
            return sendError(res, 'BAD_REQUEST', 'Missing required fields: type, title, content', 400);
        }
...
export const getUserNotifications = async (req: AuthenticatedRequest, res: Response) => {
    try {
        ...
        const result = await notificationService.getUserNotifications(userId, workplaceId, filters);
 ...
 */
export const markMultipleAsRead = async (req: AuthenticatedRequest, res: Response) => {
    try {
        const { notificationIds } = req.body;
        const userId = req.user!._id;

        if (!Array.isArray(notificationIds) || notificationIds.length === 0) {
            return sendError(res, 'BAD_REQUEST', 'Invalid notification IDs array', 400);
        }

        const validIds = notificationIds.filter(id => mongoose.Types.ObjectId.isValid(id));
        if (validIds.length === 0) {
            return sendError(res, 'BAD_REQUEST', 'No valid notification IDs provided', 400);
        }
        ...
        return sendSuccess(res, { markedCount: validIds.length }, 'Notifications marked as read');
    } catch (error) {
 ...
 */
export const dismissNotification = async (req: AuthenticatedRequest, res: Response) => {
    try {
        const { notificationId } = req.params;
        const userId = req.user!._id;
...
export const getUnreadCount = async (req: AuthenticatedRequest, res: Response) => {
    try {
        ...
        const unreadCount = await Notification.getUnreadCountByUser(
 ...
 */
export const createConversationNotification = async (req: AuthenticatedRequest, res: Response) => {
    try {
        const {
            type,
            conversationId,
        ...
        if (!type || !conversationId || !Array.isArray(recipientIds)) {
            return sendError(res, 'BAD_REQUEST', 'Missing required fields: type, conversationId, recipientIds', 400);
        }
        ...
        const validRecipientIds = recipientIds.filter(id => mongoose.Types.ObjectId.isValid(id));
        if (validRecipientIds.length === 0) {
            return sendError(res, 'BAD_REQUEST', 'No valid recipient IDs provided', 400);
        }
        ...
        return sendSuccess(res, notifications, 'Conversation notifications created successfully');
    } catch (error) {
        logger.error('Error creating conversation notification:', error);
        return sendError(res, 'SERVER_ERROR', 'Failed to create conversation notification', 500);
    }
};
 ...
 */
export const createPatientQueryNotification = async (req: AuthenticatedRequest, res: Response) => {
    try {
        const {
            patientId,
            conversationId,
        ...
        if (!patientId || !conversationId || !messageContent || !Array.isArray(recipientIds)) {
            return sendError(res, 'BAD_REQUEST', 'Missing required fields: patientId, conversationId, messageContent, recipientIds', 400);
        }

        if (!mongoose.Types.ObjectId.isValid(patientId) || !mongoose.Types.ObjectId.isValid(conversationId)) {
            return sendError(res, 'BAD_REQUEST', 'Invalid patient ID or conversation ID', 400);
        }
        ...
        return sendSuccess(res, notifications, 'Patient query notifications created successfully');
    } catch (error) {
        logger.error('Error creating patient query notification:', error);
        return sendError(res, 'SERVER_ERROR', 'Failed to create patient query notification', 500);
    }
};
 ...
 */
export const processScheduledNotifications = async (req: AuthenticatedRequest, res: Response) => {
    try {
        ...
        return sendSuccess(res, null, 'Scheduled notifications processed successfully');
    } catch (error) {
        logger.error('Error processing scheduled notifications:', error);
        return sendError(res, 'SERVER_ERROR', 'Failed to process scheduled notifications', 500);
    }
};
...
export const retryFailedNotifications = async (req: AuthenticatedRequest, res: Response) => {
    try {
        ...
        return sendSuccess(res, null, 'Failed notifications retry processed successfully');
    } catch (error) {
...
export const sendTestNotification = async (req: AuthenticatedRequest, res: Response) => {
    try {
        ...
        const notification = await notificationService.createNotification(testNotificationData);

        return sendSuccess(res, notification, 'Test notification sent successfully');
    } catch (error) {
 ...
 */
export const archiveOldNotifications = async (req: AuthenticatedRequest, res: Response) => {
    try {
        ...
        return sendSuccess(res, { archivedCount: result.modifiedCount }, `Archived ${result.modifiedCount} old notifications`);
    } catch (error) {
        logger.error('Error archiving old notifications:', error);
        return sendError(res, 'SERVER_ERROR', 'Failed to archive old notifications', 500);
    }
};
 ...
 */
export const deleteExpiredNotifications = async (req: AuthenticatedRequest, res: Response) => {
    try {
        // Check if user has admin role
        if (req.user!.role !== 'admin' && req.user!.role !== 'super_admin') {
            return sendError(res, 'FORBIDDEN', 'Insufficient permissions', 403);
        ...
        return sendSuccess(res, { deletedCount: result.deletedCount }, `Deleted ${result.deletedCount} expired notifications`);
    } catch (error) {
        logger.error('Error deleting expired notifications:', error);
        return sendError(res, 'SERVER_ERROR', 'Failed to delete expired notifications', 500);
    }
};