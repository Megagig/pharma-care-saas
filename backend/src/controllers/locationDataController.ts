...
export const getSharedPatients = async (
   ...
   try {
      ...
      const patients = await LocationFilterService.getSharedPatients(
         workspace._id
      );
...
export const getLocationAnalytics = async (
   ...
   try {
      ...
      if (locationId) {
         ...
         const analytics =
            await LocationFilterService.getWorkspaceLocationAnalytics(
               workspace
            );
...
export const transferPatientsBetweenLocations = async (
   ...
   try {
      ...
      const fromLocation = workspace.locations?.find(
         (loc) => loc.id === fromLocationId
      );
      const toLocation = workspace.locations?.find(
      ...
      const objectIds = patientIds.map((id) => new mongoose.Types.ObjectId(id));
      const result =
         await LocationFilterService.transferPatientsBetweenLocations(
            objectIds,
            fromLocationId,
            toLocationId,
            workspace._id
...
export const getLocationDistribution = async (
   ...
   try {
      ...
      const distribution =
         await LocationFilterService.getLocationDistributionSummary(
            workspace._id
         );

      // Enrich with location names
      const enrichedDistribution = distribution.locationDistribution.map(
         (item) => {
            const location = workspace.locations?.find(
      ...
      res.json({
         ...
         data: {
            ...
            summary: {
               ...
               sharedPatientsPercentage:
                  distribution.totalPatients > 0
                     ? Math.round(
                          (distribution.sharedPatients /
                             distribution.totalPatients) *
                             100
                       )
                     : 0,
            },
...
export const getLocationClinicalNotes = async (
   ...
   try {
      ...
      const clinicalNotes =
         await LocationFilterService.getClinicalNotesForLocation({
            workspaceId: workspace._id,
            locationId,
            includeShared: includeShared === 'true',
         });
...
export const sharePatientWithLocations = async (
   ...
   try {
      ...
      if (
         !patientId ||
         !fromLocationId ||
         !Array.isArray(toLocationIds) ||
         toLocationIds.length === 0
      ) {
         res.status(400).json({
            success: false,
            message:
               'Patient ID, from location ID, and to location IDs are required',
         });
         return;
      }
      ...
      const invalidLocationIds = toLocationIds.filter(
         (locId) => !workspace.locations?.find((loc) => loc.id === locId)
      );
...
export const getPatientsAccessibleFromLocation = async (
   ...
   try {
      ...
      const patients =
         await SharedPatientService.getPatientsAccessibleFromLocation(
            workspace._id,
            locationId,
            includeShared === 'true'
         );
...
export const createPatientTransferWorkflow = async (
   ...
   try {
      ...
      const { patientId, fromLocationId, toLocationId, transferReason } =
         req.body;
      const workspace = req.workspaceContext.workspace;
      ...
      if (!patientId || !fromLocationId || !toLocationId) {
         res.status(400).json({
            success: false,
            message:
               'Patient ID, from location ID, and to location ID are required',
         });
         return;
      }
