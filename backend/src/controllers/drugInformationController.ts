...
import { DrugSearchHistory, TherapyPlan } from '../modules/drug-info/models/drugCacheModel';
...
class DrugInformationController {
   ...
   */
  async searchDrugs(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
    try {
   ...
   */
  async getMonograph(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const { id } = req.params;
      if (!id) {
   ...
   */
  async checkInteractions(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      if (rxcui) {
        ...
        results = await interactionService.getInteractionsForMultipleDrugs(rxcuis);
      } else {
   ...
   */
  async getAdverseEffects(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const { id } = req.params;
      ...
      const adverseEffects = await openfdaService.getAdverseEffects(drugName, limitNum);
   ...
   */
  async getFormulary(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const { id } = req.params;
   ...
   */
  async createTherapyPlan(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
    try {
   ...
   */
  async getTherapyPlans(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      const therapyPlans = await TherapyPlan.find({ userId: req.user?._id })
        .sort({ createdAt: -1 });
   ...
   */
  async getTherapyPlanById(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      const { id } = req.params;
   ...
   */
  async updateTherapyPlan(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      const { id } = req.params;
      const { planName, drugs, guidelines } = req.body;
   ...
   */
  async deleteTherapyPlan(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      const { id } = req.params;
