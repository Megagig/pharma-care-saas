...
export class SecurityController {
   ...
   async getSecurityThreats(req: AuthRequest, res: Response): Promise<void> {
      try {
         ...
         // Get threats
         const allThreats =
            securityMonitoringService.getSecurityThreats(filters);
      ...
      try {
         ...
         const dashboard = {
            summary: {
               ...
               criticalThreats: recentThreats.filter(
                  (t) => t.severity === 'critical'
               ).length,
               highThreats: recentThreats.filter((t) => t.severity === 'high')
                  .length,
               timeRange: {
                  days: daysNum,
                  startDate: cutoff,
         ...
         recentThreats.forEach((threat) => {
            dashboard.threatsByType[threat.type] =
               (dashboard.threatsByType[threat.type] || 0) + 1;
            dashboard.threatsBySeverity[threat.severity] =
               (dashboard.threatsBySeverity[threat.severity] || 0) + 1;
         });
         ...
         recentThreats.forEach((threat) => {
            if (threat.userId) {
               ...
               userThreatCounts.set(
                  userId,
                  (userThreatCounts.get(userId) || 0) + 1
               );
            }
         });
         ...
         recentThreats.forEach((threat) => {
            ipThreatCounts.set(
               threat.ipAddress,
               (ipThreatCounts.get(threat.ipAddress) || 0) + 1
            );
         });
      ...
      try {
         ...
         if (req.user?.role !== 'super_admin') {
            res.status(403).json({
               ...
               code: 'INSUFFICIENT_PERMISSIONS',
               message:
                  'Only super administrators can resolve security threats',
            });
            return;
         }
   ...
   async getUserSecurityStatus(req: AuthRequest, res: Response): Promise<void> {
      try {
         ...
         const suspiciousScore =
            securityMonitoringService.getUserSuspiciousScore(userId || '');
         ...
         const status = {
            ...
            lastThreatAt:
               userThreats.length > 0 ? userThreats[0]?.timestamp : null,
   ...
   async getBlockedIPs(req: AuthRequest, res: Response): Promise<void> {
      try {
         ...
         const blockingThreats = securityMonitoringService
            .getSecurityThreats({
               type: 'brute_force',
            })
   ...
   private generateSecurityRecommendations(
      ...
      if (suspiciousScore > 8) {
         recommendations.push(
            'Your account has been flagged for high-risk activity. Please contact support immediately.'
         );
      } else if (suspiciousScore > 5) {
         recommendations.push(
            'Your account is under monitoring due to suspicious activity. Please review your recent actions.'
         );
      }

      if (threats.some((t) => t.type === 'brute_force')) {
         recommendations.push(
            'Multiple failed login attempts detected. Consider enabling two-factor authentication.'
         );
      }

      if (threats.some((t) => t.type === 'permission_escalation')) {
         recommendations.push(
            'Unauthorized access attempts detected. Review your account permissions.'
         );
      }

      if (threats.some((t) => t.type === 'data_exfiltration')) {
         recommendations.push(
            'Unusual data access patterns detected. Review your data export activities.'
         );
      }

      if (threats.some((t) => t.type === 'session_hijacking')) {
         recommendations.push(
            'Multiple concurrent sessions detected. Log out from unused devices.'
         );
      }

      if (recommendations.length === 0) {
         recommendations.push(
            'Your account security status is good. Continue following security best practices.'
         );
      }
