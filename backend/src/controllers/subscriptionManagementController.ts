...
export class SubscriptionManagementController {
            ...
            return res.status(200).json({
               ...
               data: {
                  ...
                  workspace: {
                     ...
                     isTrialExpired: workspace.trialEndDate
                        ? new Date() > workspace.trialEndDate
                        : false,
                  },
   ...
   async createTrialSubscription(
      ...
      try {
         const { workspaceId, trialDurationDays = 14 }: TrialCreationData =
            req.body;
      ...
      try {
         ...
         if (req.user!.role !== 'super_admin') {
            return res.status(403).json({
               success: false,
               message:
                  'Insufficient permissions to update subscription status',
            });
         }
   ...
   async createWorkspaceCheckout(
      ...
      try {
         const {
            ...
            planId,
            billingInterval = 'monthly',
         }: WorkspaceSubscriptionData = req.body;
         ...
         if (existingSubscription && existingSubscription.status === 'active') {
            return res.status(400).json({
               success: false,
               message:
                  'Workspace already has an active subscription. Use upgrade/downgrade endpoints.',
            });
         }
         ...
         const paymentData = {
            ...
            callback_url:
               ...
               `${process.env.FRONTEND_URL}/workspace/subscription/success`,
            metadata: {
    ...
    */
   async handleWorkspacePaymentSuccess(
      req: AuthRequest,
      res: Response
   ): Promise<any> {
      try {
         ...
         if (!paymentReference.startsWith('mock_')) {
            const verificationResult =
               await paystackService.verifyTransaction(paymentReference);
            if (
   ...
   private async activateWorkspaceSubscription(paymentRecord: any) {
      ...
      const subscription = new Subscription({
         ...
         features: Object.keys(plan.features).filter(
            (key) => (plan.features as any)[key] === true
         ),
         limits: {
    ...
    */
   async upgradeWorkspaceSubscription(
      req: AuthRequest,
      res: Response
   ): Promise<any> {
      try {
         ...
         if (newTierIndex <= currentTierIndex) {
            return res.status(400).json({
               success: false,
               message:
                  'This is not an upgrade. Use downgrade endpoint for downgrades or same-tier changes.',
            });
         }
         ...
         const paymentData = {
            ...
            callback_url:
               req.body.callbackUrl ||
               `${process.env.FRONTEND_URL}/workspace/subscription/upgrade-success`,
            metadata: {
               userId: req.user!._id.toString(),
               workspaceId: workspaceId.toString(),
    ...
    */
   async downgradeWorkspaceSubscription(
      req: AuthRequest,
      res: Response
   ): Promise<any> {
      try {
         ...
         if (newTierIndex >= currentTierIndex) {
            return res.status(400).json({
               success: false,
               message:
                  'This is not a downgrade. Use upgrade endpoint for upgrades or same-tier changes.',
            });
         }
    ...
    */
   async handleUpgradePaymentSuccess(
      req: AuthRequest,
      res: Response
   ): Promise<any> {
      try {
      ...
      try {
         ...
         if (req.user!.role !== 'super_admin') {
            return res.status(403).json({
               success: false,
               message:
                  'Only super admins can manually handle subscription expiry',
            });
         }
      ...
      try {
         ...
         if (!isExpired) {
            return res.status(400).json({
               success: false,
               message:
                  'Subscription is not expired, paywall mode not applicable',
            });
         }
         ...
         res.json({
            ...
            data: {
               ...
               paywallMode: true,
               message:
                  'Subscription has expired. Please upgrade to restore full access.',
            },
         });
      } catch (error) {
   ...
   async getSubscriptionStatus(req: AuthRequest, res: Response): Promise<any> {
      try {
         ...
         if (!subscription) {
            return res.json({
               ...
               data: {
                  ...
                  paywallMode: true,
                  message:
                     'No subscription found. Please subscribe to access features.',
               },
            });
         }
         ...
         if (subscription.status === 'trial' && isTrialExpired) {
            ...
            paywallMode = true;
            message =
               'Trial period has expired. Please upgrade to continue using all features.';
            ...
            if (isInGracePeriod) {
               ...
               paywallMode = true;
               message =
                  'Subscription has expired. Please renew to restore full access.';
            }
            ...
            ['expired', 'canceled', 'suspended'].includes(subscription.status)
         ) {
            paywallMode = true;
            message =
               'Subscription is not active. Please subscribe or reactivate to access features.';
         }
...
export const subscriptionManagementController =
   new SubscriptionManagementController();
