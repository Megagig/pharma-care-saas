...
export class AdminDashboardController {
   ...
   async updateWorkspaceSubscription(
      ...
      try {
         ...
         if (planId) {
            ...
            subscription.features = Object.keys(plan.features).filter(
               (key) =>
         ...
         // Log admin action
         console.log(
            `Admin ${req.user?.email} updated subscription for workspace ${workspace.name}`,
            {
               workspaceId,
   ...
   async cancelInvitation(req: AuthRequest, res: Response): Promise<any> {
      try {
         ...
         // Log admin action
         console.log(
            `Admin ${req.user?.email} canceled invitation ${invitationId}`,
            {
               invitationId,
               reason,
               adminId: req.user?._id,
