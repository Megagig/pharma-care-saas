...
export class AuditController {
      ...
      try {
         ...
         logs.forEach((log) => {
            ...
            summary.bySeverity[log.severity] =
               (summary.bySeverity[log.severity] || 0) + 1;

            // Action stats
            summary.byAction[log.action] =
               (summary.byAction[log.action] || 0) + 1;
            ...
            // Top IPs
            summary.topIPs[log.ipAddress] =
               (summary.topIPs[log.ipAddress] || 0) + 1;
         });
   ...
   private calculateOverallRiskScore(logs: any[]): number {
      ...
      const totalRiskScore = logs.reduce(
         (sum, log) => sum + (log.riskScore || 0),
         0
      );
   ...
   private calculateComplianceScore(summary: any): number {
      ...
      const suspiciousRate =
         summary.totalLogs > 0
            ? (summary.suspiciousActivities / summary.totalLogs) * 100
            : 0;
      ...
      const highRiskRate =
         summary.totalLogs > 0
            ? (summary.highRiskActivities / summary.totalLogs) * 100
            : 0;
   ...
   private analyzeSecurityPatterns(alerts: any[]): any {
      ...
      alerts.forEach((alert) => {
         if (alert.errorMessage && alert.userEmail) {
            patterns.repeatedFailures[alert.userEmail] =
               (patterns.repeatedFailures[alert.userEmail] || 0) + 1;
         }

         // Track suspicious IPs
         if (alert.suspicious) {
            patterns.suspiciousIPs[alert.ipAddress] =
               (patterns.suspiciousIPs[alert.ipAddress] || 0) + 1;
         }
      });
      ...
      if (highFailureUsers.length > 0) {
         patterns.recommendations.push(
            `Review accounts with repeated failures: ${highFailureUsers.join(', ')}`
         );
      }
      ...
      if (suspiciousIPList.length > 0) {
         patterns.recommendations.push(
            `Consider blocking suspicious IPs: ${suspiciousIPList.join(', ')}`
         );
      }

      if (alerts.length > 50) {
         patterns.recommendations.push(
            'High volume of security alerts detected. Review security policies.'
         );
      }
