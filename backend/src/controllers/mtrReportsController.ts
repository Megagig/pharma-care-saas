...
export const getMTRSummaryReport = async (req: AuthRequest, res: Response) => {
   try {
      const { startDate, endDate, pharmacistId, reviewType, priority } =
         req.query;
      ...
      if (pharmacistId)
         additionalFilters.pharmacistId = new mongoose.Types.ObjectId(
         ...
         { $match: matchStage },
         {
            $group: {
               ...
               adherenceImprovedCount: {
                  $sum: {
                     $cond: ['$clinicalOutcomes.adherenceImproved', 1, 0],
                  },
               },
               adverseEventsReducedCount: {
                  $sum: {
                     $cond: ['$clinicalOutcomes.adverseEventsReduced', 1, 0],
                  },
               },
      ...
      sendSuccess(
         res,
         {
            summary: {
               ...
               avgCompletionTime:
                  Math.round((summary.avgCompletionTime || 0) * 100) / 100,
            },
            distributions: {
      ...
      sendError(
         res,
         'SERVER_ERROR',
         'Failed to generate MTR summary report',
         500
      );
   }
};
 ...
 */
export const getInterventionEffectivenessReport = async (
   req: AuthRequest,
   res: Response
) => {
   try {
         ...
         res,
         {
            summary: {
               ...
               overallAcceptanceRate:
                  Math.round(overallAcceptanceRate * 100) / 100,
            },
            effectiveness: {
      ...
      console.error(
         'Error generating intervention effectiveness report:',
         error
      );
      sendError(
         res,
         'SERVER_ERROR',
         'Failed to generate intervention effectiveness report',
         500
      );
   }
};
 ...
 */
export const getPharmacistPerformanceReport = async (
   req: AuthRequest,
   res: Response
) => {
   try {
      ...
      if (pharmacistId) {
         pharmacistFilter.pharmacistId = new mongoose.Types.ObjectId(
            pharmacistId as string
         );
      }
      ...
      const combinedStats = pharmacistMTRStats.map((mtrStat) => {
         ...
         return {
            ...
            interventionAcceptanceRate:
               interventionStat?.interventionAcceptanceRate || 0,
         };
      });

      // Calculate quality scores
      const qualityScores = combinedStats
         .map((stat) => {
            ...
            const efficiencyScore = Math.max(
               0,
               100 - ((stat.avgCompletionTime || 14) / 14) * 100
            );
      ...
      sendError(
         res,
         'SERVER_ERROR',
         'Failed to generate pharmacist performance report',
         500
      );
   }
};
 ...
 */
export const getQualityAssuranceReport = async (
   req: AuthRequest,
   res: Response
) => {
   try {
      ...
      const documentationQuality = await MedicationTherapyReview.aggregate([
         { $match: matchStage },
         {
            $addFields: {
               hasCompletePlan: {
                  $and: [
                     { $ne: ['$plan', null] },
                     {
                        $gt: [
                           {
                              $size: { $ifNull: ['$plan.recommendations', []] },
                           },
                           0,
                        ],
                     },
               ...
               hasMedications: {
                  $gt: [{ $size: { $ifNull: ['$medications', []] } }, 0],
               },
               hasProblems: {
                  $gt: [{ $size: { $ifNull: ['$problems', []] } }, 0],
               },
            },
         },
         {
      ...
      sendSuccess(
         res,
         {
            ...
            qualityMetrics: {
               ...
               avgProblemResolutionRate:
                  problemPatterns.length > 0
                     ? problemPatterns.reduce(
                          (sum, p) => sum + p.resolutionRate,
                          0
                       ) / problemPatterns.length
                     : 0,
            },
         },
      ...
      sendError(
         res,
         'SERVER_ERROR',
         'Failed to generate quality assurance report',
         500
      );
   }
};
 ...
 */
export const getOutcomeMetricsReport = async (
   req: AuthRequest,
   res: Response
) => {
   try {
         ...
         { $match: matchStage },
         {
            $group: {
               ...
               qualityOfLifeImprovedCount: {
                  $sum: {
                     $cond: ['$clinicalOutcomes.qualityOfLifeImproved', 1, 0],
                  },
               },
               clinicalParametersImprovedCount: {
                  $sum: {
                     $cond: [
                        '$clinicalOutcomes.clinicalParametersImproved',
                        1,
                        0,
                     ],
                  },
               ...
               totalCostSavings: { $sum: '$clinicalOutcomes.costSavings' },
               avgProblemsPerReview: {
                  $avg: '$clinicalOutcomes.problemsResolved',
               },
               avgMedicationsPerReview: {
                  $avg: '$clinicalOutcomes.medicationsOptimized',
               },
            },
         },
      ]);
      ...
      const adherenceImprovementRate =
         outcomes.totalReviews > 0
            ? (outcomes.adherenceImprovedCount / outcomes.totalReviews) * 100
            : 0;

      const adverseEventReductionRate =
         outcomes.totalReviews > 0
      ...
      sendSuccess(
         res,
         {
            summary: {
               ...outcomes,
               adherenceImprovementRate:
                  Math.round(adherenceImprovementRate * 100) / 100,
               adverseEventReductionRate:
                  Math.round(adverseEventReductionRate * 100) / 100,
            },
            outcomesByType,
            trends: {
      ...
      sendError(
         res,
         'SERVER_ERROR',
         'Failed to generate outcome metrics report',
         500
      );
   }
};
