   ...
   try {
      ...
      res.json({
         ...
         data: {
            ...
            primaryLocation:
               workspace.locations?.find((loc) => loc.isPrimary) || null,
         },
      });
   } catch (error) {
   ...
   try {
      ...
      if (!plan || !plan.features?.multiLocationDashboard) {
         res.status(403).json({
            success: false,
            message:
               'Multi-location feature not available in your current plan',
      ...
      logger.info(
         `Location created: ${newLocation.name} for workspace ${workspace.name}`
      );
...
export const updateLocation = async (
   ...
   try {
      ...
      // Find the location
      const locationIndex = workspace.locations?.findIndex(
         (loc) => loc.id === locationId
      );
      ...
      logger.info(
         `Location updated: ${locationId} for workspace ${workspace.name}`
      );
...
export const deleteLocation = async (
   ...
   try {
      ...
      logger.info(
         `Location deleted: ${locationId} for workspace ${workspace.name}`
      );
...
export const setPrimaryLocation = async (
   ...
   try {
      ...
      const updateOperations =
         workspace.locations?.map((loc, index) => ({
      ...
      logger.info(
         `Primary location set: ${locationId} for workspace ${workspace.name}`
      );
...
export const bulkUpdateLocations = async (
   ...
   try {
      ...
      const existingLocationIds =
         workspace.locations?.map((loc) => loc.id) || [];
      const updateLocationIds = locations.map((loc) => loc.id);

      const invalidIds = updateLocationIds.filter(
      ...
      const updatedLocations =
         workspace.locations?.map((existingLoc) => {
            const updateData = locations.find(
               (loc) => loc.id === existingLoc.id
            );
            if (updateData) {
               return {
                  ...existingLoc,
      ...
      logger.info(
         `Bulk location update completed for workspace ${workspace.name}`
      );
