...
export const getFollowUpReminders = async (
   ...
   try {
      ...
      return sendSuccess(
         res,
         followUp.reminders,
         'Follow-up reminders retrieved successfully'
      );
   } catch (error) {
      ...
      return sendError(
         res,
         'SERVER_ERROR',
         'Failed to get follow-up reminders',
         500
      );
   }
};
...
export const cancelScheduledReminder = async (
   ...
   try {
      ...
      const reminderIndex = followUp.reminders.findIndex(
         (r: any) => r._id?.toString() === reminderId
      );
      if (reminderIndex === -1) {
      ...
      const mockInteractions = [
         {
            ...
            clinicalSignificance:
               'Monitor INR closely and watch for signs of bleeding',
         },
      ];
 ...
 */
export const notifyHighSeverityDTP = async (
   req: AuthenticatedRequest,
   res: Response
) => {
   try {
      ...
      const problem = await DrugTherapyProblem.findById(problemId).populate(
         'patientId',
      ...
      if (!problem) {
         return sendError(
            res,
            'NOT_FOUND',
            'Drug therapy problem not found',
            404
         );
      }
      ...
      if (!['critical', 'major'].includes(problem.severity)) {
         return sendError(
            res,
            'BAD_REQUEST',
            'Notifications are only sent for critical or major severity problems',
            400
         );
      }
      ...
      return sendSuccess(
         res,
         null,
         'High severity DTP notification sent successfully'
      );
   } catch (error) {
      ...
      return sendError(
         res,
         'SERVER_ERROR',
         'Failed to send high severity DTP notification',
         500
      );
   }
};
