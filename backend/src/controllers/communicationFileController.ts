...
export class CommunicationFileController {
    ...
    static async uploadFile(req: AuthenticatedRequest, res: Response): Promise<void> {
        try {
            ...
            const processResult = await FileUploadService.processUploadedFile(req.file);
            ...
            if (req.file?.path) {
                try {
                    ...
                    logger.error('Failed to cleanup file after error', { error: cleanupError });
                }
            }
    ...
    static async deleteFile(req: AuthenticatedRequest, res: Response): Promise<void> {
        try {
            ...
            if (message.senderId.toString() !== userId && req.user?.role !== 'admin') {
                res.status(403).json({ error: 'Access denied. Only file owner or admin can delete files' });
                return;
            }
     ...
     * Get file metadata
     */
    static async getFileMetadata(req: AuthenticatedRequest, res: Response): Promise<void> {
        try {
            const { fileId } = req.params;
            const userId = req.user?._id;
            const workplaceId = req.user?.workplaceId;
     ...
     */
    static async listConversationFiles(req: AuthenticatedRequest, res: Response): Promise<void> {
        try {
            ...
            if (fileType) {
                query['content.attachments.mimeType'] = new RegExp(fileType as string, 'i');
            }
