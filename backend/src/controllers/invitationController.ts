...
export const createInvitation = async (
   ...
   try {
      ...
      if (workspace.currentSubscriptionId) {
         ...
         if (subscriptionPlan) {
            ...
            if (subscriptionPlan.features.teamSize) {
               ...
               const totalPotentialSize =
                  currentTeamSize + pendingInvitationsCount + 1;
      ...
      const invitation = new Invitation({
         ...
         metadata: {
            inviterName:
               inviter?.firstName + ' ' + inviter?.lastName || 'Unknown',
...
export const getWorkspaceInvitations = async (
   ...
   try {
      ...
      if (
         status &&
         ['active', 'expired', 'used', 'canceled'].includes(status as string)
      ) {
         query.status = status;
      }
   ...
   try {
      ...
      if (
         ...
         res.status(403).json({
            success: false,
            message:
               'You can only cancel invitations you created or if you are the workspace owner',
         });
         return;
      }
...
export const acceptInvitation = async (
   ...
   try {
      ...
      if (
         user.workplaceId &&
         user.workplaceId.toString() === workspace._id.toString()
      ) {
         res.status(409).json({
            success: false,
      ...
      if (user.workplaceId) {
         res.status(409).json({
            success: false,
            message:
               'You are already a member of another workspace. Please leave your current workspace first.',
            data: {
      ...
      try {
         ...
         emailService
            .sendInvitationAcceptedNotification(inviterData.email, {
...
export const acceptInvitationPublic = async (
   ...
   try {
      ...
      try {
         ...
         // Add user to workspace team members
         await Workplace.findByIdAndUpdate(workspace._id, {
...
export const getInvitationAnalytics = async (
   ...
   try {
      ...
      const analytics =
         await invitationCronService.getInvitationAnalytics(workspaceId);
      ...
      res.json({
         ...
         data: {
            ...
            teamSize: {
               ...
               limit: teamSizeLimit,
               remaining: teamSizeLimit
                  ? Math.max(0, teamSizeLimit - currentTeamSize)
                  : null,
            },
         },
      });
