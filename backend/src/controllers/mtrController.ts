...
export const createMTRSession = asyncHandler(
   async (req: AuthRequest, res: Response) => {
      ...
      if (req.user?.role === 'super_admin') {
         console.log(
            'Super admin bypassing tenant check for MTR patient access'
         );
      } else {
         checkTenantAccess(
      ...
      const session = new MedicationTherapyReview({
         ...
         reviewReason,
         patientConsent:
            req.user?.role === 'super_admin' ? true : patientConsent, // Auto-approve consent for super_admin
...
export const getMTROutcomes = asyncHandler(
   async (req: AuthRequest, res: Response) => {
      ...
      sendSuccess(
         res,
         { analytics },
         'Outcome analytics generated successfully'
      );
   }
);
         ...
         for (let j = i + 1; j < medications.length; j++) {
            ...
            if (
               ...
               interactions.push({
                  ...
                  recommendation:
                     'Consider alternative therapy or adjust dosing',
               });
            }
         }
      }
