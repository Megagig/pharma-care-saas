...
export const getUsageComparison = async (
   ...
   try {
      ...
      const allPlans = await SubscriptionPlan.find({ isActive: true }).sort({
         priceNGN: 1,
      });

      // Get current usage
      const currentUsage = await WorkspaceStatsService.getUsageStats(
         workspace._id
      );
      ...
      const comparison = allPlans.map((planOption) => {
         ...
         const currentTierRank = plan
            ? tierRanking[plan.tier as keyof typeof tierRanking] || 0
            : 0;
         const planTierRank =
            tierRanking[planOption.tier as keyof typeof tierRanking] || 0;
         ...
         return {
            ...
            usage: {
               patients: {
                  ...
                  withinLimit:
                     planLimits.patients === null ||
                     currentUsage.patientsCount <= planLimits.patients,
                  percentage: planLimits.patients
                     ? Math.round(
                          (currentUsage.patientsCount / planLimits.patients) *
                             100
                       )
               ...
               users: {
                  ...
                  withinLimit:
                     planLimits.users === null ||
                     currentUsage.usersCount <= planLimits.users,
                  percentage: planLimits.users
                     ? Math.round(
                          (currentUsage.usersCount / planLimits.users) * 100
                       )
                     : null,
               },
               storage: {
                  ...
                  withinLimit:
                     planLimits.storage === null ||
                     (currentUsage.storageUsed || 0) <= planLimits.storage,
                  percentage: planLimits.storage
                     ? Math.round(
                          ((currentUsage.storageUsed || 0) /
                             planLimits.storage) *
                             100
                       )
                     : null,
...
async function getUsageTrends(workspaceId: any): Promise<any> {
   ...
   try {
      const currentStats =
         await WorkspaceStatsService.getUsageStats(workspaceId);
   ...
   try {
      ...
      return {
         summary: {
            ...
            resourcesAtWarning: Object.values(usageWithLimits.usage).filter(
               (resource: any) =>
                  resource.percentage && resource.percentage >= 90
            ).length,
            resourcesAtLimit: Object.values(usageWithLimits.usage).filter(
               (resource: any) =>
...
function getRecommendedPlan(comparison: any[], currentUsage: any): any {
   ...
   const suitablePlans = comparison.filter((plan) => {
      return (
         plan.usage.patients.withinLimit &&
   ...
   return suitablePlans.reduce((cheapest, current) => {
      return current.plan.priceNGN < cheapest.plan.priceNGN
         ? current
         : cheapest;
   });
}
