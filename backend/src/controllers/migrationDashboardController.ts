...
export class MigrationDashboardController {
   ...
   static async resolveAlert(req: Request, res: Response): Promise<void> {
      try {
         ...
         logger.error('Failed to resolve migration alert', {
            error,
      ...
      try {
         ...
         if (!['daily', 'weekly', 'on_demand'].includes(type as string)) {
            res.status(400).json({
               success: false,
               message:
                  'Invalid report type. Must be daily, weekly, or on_demand',
            });
            return;
         }

         const report = await monitoringService.generateReport(
            type as 'daily' | 'weekly' | 'on_demand'
         );
            ...
            data: {
               ...
               details: {
                  ...
                  workspacesWithSubscriptions:
                     metrics.workspacesWithSubscriptions,
