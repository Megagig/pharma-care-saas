   ...
   try {
      ...
      if (!note) {
         ...
         if (noteId && mongoose.Types.ObjectId.isValid(noteId)) {
            ...
            if (foundNote) {
               console.log(
                  `Found note by direct ID for deletion: ${foundNote._id}`
               );
               note = foundNote;
            }
         }
      ...
      if (confidentialNotes.length > 0) {
         ...
         if (!canModifyConfidential) {
            res.status(403).json({
               success: false,
               message:
                  'Insufficient permissions to modify some confidential notes',
      ...
      if (updates.isConfidential === true) {
         if (!confidentialNoteService.canCreateConfidentialNotes(req.user!)) {
            res.status(403).json({
               success: false,
               message:
                  'Insufficient permissions to mark notes as confidential',
            });
            return;
         }
      }
      ...
      if (confidentialNotes.length > 0) {
         ...
         if (!canDeleteConfidential) {
            res.status(403).json({
               success: false,
               message:
                  'Insufficient permissions to delete some confidential notes',
               confidentialNoteCount: confidentialNotes.length,
            });
            return;
         }
