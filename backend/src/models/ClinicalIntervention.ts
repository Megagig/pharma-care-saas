...
export interface IClinicalInterventionModel
   extends mongoose.Model<IClinicalIntervention> {
   generateNextInterventionNumber(
      workplaceId: mongoose.Types.ObjectId
   ): Promise<string>;
   findActive(
      workplaceId?: mongoose.Types.ObjectId
   ): mongoose.Query<IClinicalIntervention[], IClinicalIntervention>;
   findOverdue(
      workplaceId?: mongoose.Types.ObjectId
   ): mongoose.Query<IClinicalIntervention[], IClinicalIntervention>;
   findByPatient(
...
const clinicalInterventionSchema = new Schema(
   {
      ...
      implementationNotes: {
         ...
         maxlength: [
            2000,
            'Implementation notes cannot exceed 2000 characters',
         ],
      },
      ...
      outcomes: {
         ...
         clinicalParameters: [
            {
               ...
               improvementPercentage: {
                  ...
                  min: [
                     -100,
                     'Improvement percentage cannot be less than -100',
                  ],
         ...
         adverseEffects: {
            ...
            maxlength: [
               1000,
               'Adverse effects description cannot exceed 1000 characters',
            ],
         },
         additionalIssues: {
            type: String,
            ...
            maxlength: [
               1000,
               'Additional issues description cannot exceed 1000 characters',
            ],
         },
         successMetrics: {
            problemResolved: {
...
clinicalInterventionSchema.pre('save', function (this: IClinicalIntervention) {
   ...
   if (
      (this.status === 'implemented' || this.status === 'completed') &&
      !this.outcomes?.patientResponse
   ) {
      throw new Error(
         'Patient response outcome is required for implemented/completed interventions'
      );
   }
...
clinicalInterventionSchema.statics.generateNextInterventionNumber =
...
export default mongoose.model<
   ...
   IClinicalInterventionModel
>('ClinicalIntervention', clinicalInterventionSchema);
