...
medicationTherapyReviewSchema.virtual('completionPercentage').get(function (this: IMedicationTherapyReview) {
...
medicationTherapyReviewSchema.virtual('durationDays').get(function (this: IMedicationTherapyReview) {
...
medicationTherapyReviewSchema.virtual('isOverdue').get(function (this: IMedicationTherapyReview) {
...
// Instance methods
medicationTherapyReviewSchema.methods.getCompletionPercentage = function (this: IMedicationTherapyReview): number {
    const steps = Object.values(this.steps);
    const completedSteps = steps.filter(step => step.completed).length;
    return Math.round((completedSteps / steps.length) * 100);
};

medicationTherapyReviewSchema.methods.getNextStep = function (this: IMedicationTherapyReview): string | null {
    const stepOrder = [
        'patientSelection',
        'medicationHistory',
        'therapyAssessment',
...
medicationTherapyReviewSchema.methods.canComplete = function (this: IMedicationTherapyReview): boolean {
...
medicationTherapyReviewSchema.methods.generateReviewNumber = function (this: IMedicationTherapyReview): string {
...
// Pre-save middleware
medicationTherapyReviewSchema.pre('save', function (this: IMedicationTherapyReview) {
...
medicationTherapyReviewSchema.statics.generateNextReviewNumber = async function (
    workplaceId: mongoose.Types.ObjectId
): Promise<string> {
    const year = new Date().getFullYear();
    const month = (new Date().getMonth() + 1).toString().padStart(2, '0');
