...
export const createMTRSessionSchema = [
   ...
   body('reviewType')
      ...
      .withMessage(
         'Review type must be initial, follow_up, annual, or targeted'
      ),
...
export const updateMTRSessionSchema = [
   body('status')
      ...
      .withMessage(
         'Status must be in_progress, completed, cancelled, or on_hold'
      ),
...
export const createProblemSchema = [
   body('category')
      ...
      .withMessage(
         'Category must be indication, effectiveness, safety, or adherence'
      ),
   ...
   body('clinicalSignificance')
      ...
      .isLength({ min: 10, max: 1000 })
      .withMessage(
         'Clinical significance must be between 10 and 1000 characters'
      ),

   body('evidenceLevel')
      .isIn(['definite', 'probable', 'possible', 'unlikely'])
      .withMessage(
         'Evidence level must be definite, probable, possible, or unlikely'
      ),
...
export const updateProblemSchema = [
   body('status')
      ...
      .withMessage(
         'Status must be identified, addressed, monitoring, resolved, or not_applicable'
      ),
   ...
   ...createProblemSchema.filter(
      (rule) => !rule.toString().includes('notEmpty')
...
export const createInterventionSchema = [
   body('type')
      ...
      .withMessage(
         'Type must be recommendation, counseling, monitoring, communication, or education'
      ),
   ...
   body('category')
      ...
      .withMessage(
         'Category must be medication_change, adherence_support, monitoring_plan, or patient_education'
      ),
   ...
   body('targetAudience')
      ...
      .withMessage(
         'Target audience must be patient, prescriber, caregiver, or healthcare_team'
      ),

   body('communicationMethod')
      .isIn(['verbal', 'written', 'phone', 'email', 'fax', 'in_person'])
      .withMessage(
         'Communication method must be verbal, written, phone, email, fax, or in_person'
      ),
   ...
   body('urgency')
      ...
      .withMessage(
         'Urgency must be immediate, within_24h, within_week, or routine'
      ),
...
export const updateInterventionSchema = [
   body('outcome')
      ...
      .withMessage(
         'Outcome must be accepted, rejected, modified, pending, or not_applicable'
      ),
   ...
   ...createInterventionSchema.filter(
      (rule) => !rule.toString().includes('notEmpty')
   ), // Allow optional updates
];
...
export const createFollowUpSchema = [
   body('type')
      ...
      .withMessage(
         'Type must be phone_call, appointment, lab_review, adherence_check, or outcome_assessment'
      ),
...
export const updateFollowUpSchema = [
   body('status')
      ...
      .withMessage(
         'Status must be scheduled, in_progress, completed, missed, rescheduled, or cancelled'
      ),
   ...
   body('outcome.status')
      ...
      .withMessage(
         'Outcome status must be successful, partially_successful, or unsuccessful'
      ),
