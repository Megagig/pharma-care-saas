...
export const strategyBusinessRules: BusinessRuleValidation[] = [
   {
      ...
      message:
         'Custom strategies require detailed description and rationale (minimum 20 characters each)',
   ...
   body('issueDescription')
      ...
      .custom((value) => {
         ...
         if (suspiciousPatterns.some((pattern) => pattern.test(value))) {
            throw new Error(
               'Issue description contains potentially unsafe content'
            );
         }
...
export const interventionParamsSchema: ValidationChain[] = [
   param('id').isMongoId().withMessage('Valid intervention ID is required'),
];
...
export const linkMTRSchema: ValidationChain[] = [
   body('mtrId').isMongoId().withMessage('Valid MTR ID is required'),
];
