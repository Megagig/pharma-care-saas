   ...
   allergies: z
      .array(
         z.object({
            ...
            severity: z
               .enum(SEVERITY_LEVELS as [string, ...string[]])
               .optional(),
            notedAt: z
               .string()
               .datetime()
...
// For updates, we create a schema where optional fields properly handle empty values
// and required fields maintain their validation while allowing partial updates
export const updatePatientSchema = z
   .object({
      // Demographics - allow updates without requiring all fields
      firstName: z
         .string()
         .min(1, 'First name is required')
         .max(50, 'First name cannot exceed 50 characters')
      ...
      // Demographics (optional)
      otherNames: z.string().max(100).trim().optional().nullable(),
      dob: z
         .string()
         .datetime()
         .optional()
      ...
      gender: z
         ...
         .nullable(),
      maritalStatus: z
         .enum(MARITAL_STATUS as [string, ...string[]])
         .optional()
         .nullable(),
      ...
      bloodGroup: z
         ...
         .nullable(),
      genotype: z
         .enum(GENOTYPES as [string, ...string[]])
         .optional()
         .nullable(),
      weightKg: z.number().positive().max(1000).optional().nullable(),

      // Complex nested objects - these should be handled separately if needed
      allergies: z
         .array(
            z.object({
               substance: z.string().min(1).max(100).trim(),
      ...
      carePlan: z
         .object({
            goals: z
               .array(z.string().min(5).max(200).trim())
               .max(10)
               .optional(),
            objectives: z
               .array(z.string().min(5).max(300).trim())
               .max(15)
               .optional(),
            followUpDate: z
               .string()
