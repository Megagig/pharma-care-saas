...
grep "migration" /var/log/pharmacare/combined.log | tail -50
```
...
3. **Reset Password:**
   ...
   curl -X POST https://api.pharmacare.com/api/auth/forgot-password \
...
grep "workspace context" /var/log/pharmacare/combined.log
...
grep "invitation email" /var/log/pharmacare/combined.log
...
curl -X POST https://api.resend.com/emails \
  ...
  -H "Content-Type: application/json" \
  -d '{"from": "test@pharmacare.com", "to": "test@example.com", "subject": "Test", "html": "Test"}'
```
...
3. **Recreate Invitation:**
   ...
   curl -X DELETE https://api.pharmacare.com/api/invitations/INVITATION_ID \
     -H "Authorization: Bearer OWNER_TOKEN"

   curl -X POST https://api.pharmacare.com/api/workspaces/WORKSPACE_ID/invitations \
     -H "Authorization: Bearer OWNER_TOKEN" \
     -H "Content-Type: application/json" \
...
2. **Cancel Unnecessary Invitations:**
   ...
   curl -X GET https://api.pharmacare.com/api/workspaces/WORKSPACE_ID/invitations \
     -H "Authorization: Bearer OWNER_TOKEN"
...
3. **Upgrade Plan:**
   ...
   curl -X GET https://api.pharmacare.com/api/subscription-plans \
...
# Check webhook logs
grep "webhook" /var/log/pharmacare/combined.log | tail -20
...
2. **Trigger Webhook Manually:**
   ...
   curl -X POST https://api.pharmacare.com/api/webhooks/paystack \
     -H "Content-Type: application/json" \
     -d '{
...
3. **Refresh Plan Features:**
   ...
   curl -X POST https://api.pharmacare.com/api/admin/refresh-plans \
...
grep "trial expiry" /var/log/pharmacare/combined.log
...
3. **Enable Paywall Mode:**
   ```bash
   curl -X POST https://api.pharmacare.com/api/subscriptions/workspace/WORKSPACE_ID/paywall/enable \
     -H "Authorization: Bearer ADMIN_TOKEN"
   ```
...
curl -X GET https://api.pharmacare.com/api/auth/permissions \
...
3. **Clear Permission Cache:**
   ...
   # Clear user session
   curl -X POST https://api.pharmacare.com/api/auth/logout \
     -H "Authorization: Bearer USER_TOKEN"
   ```
...
```bash
# Check user role
db.users.findOne({ email: "admin@pharmacare.com" }, { role: 1, status: 1 })
...
grep "usage update" /var/log/pharmacare/combined.log
...
1. **Manually Recalculate Statistics:**
   ...
   curl -X POST https://api.pharmacare.com/api/usage/recalculate \
     -H "Authorization: Bearer OWNER_TOKEN"
   ```
...
curl -X POST https://api.pharmacare.com/api/patients \
  -H "Authorization: Bearer TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"name": "Test Patient"}'
```
...
dig TXT pharmacare.com | grep -i spf
dig TXT _dmarc.pharmacare.com
...
1. **Configure SPF/DKIM/DMARC:**
   ...
   # DMARC: "v=DMARC1; p=quarantine; rua=mailto:dmarc@pharmacare.com"
   ```
...
# Check email service logs
grep "template" /var/log/pharmacare/combined.log
```
...
curl -w "@curl-format.txt" -o /dev/null -s https://api.pharmacare.com/api/auth/profile
...
```bash
# Check migration logs
grep "migration" /var/log/pharmacare/combined.log | grep -i error
...
curl -X GET https://api.pharmacare.com/api/auth/profile \
  -H "Authorization: Bearer TOKEN" \
  -H "Origin: https://app.pharmacare.com"
...
```bash
# Check webhook logs
grep "webhook" /var/log/pharmacare/combined.log
...
- **Technical Support:** tech-support@pharmacare.com
- **Emergency Hotline:** +234-800-PHARMA-1
- **DevOps Team:** devops@pharmacare.com
- **Database Admin:** dba@pharmacare.com
